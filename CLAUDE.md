...
|------|--------|---------------------|
| `specs/original/` | Downloaded from GitHub Releases (robinmordasiewicz/f5xc-api-fixed) | Change download config, not outputs |
...
| `.github_release` | Managed by download script (GitHub release version tracking) | Never edit manually |
...
|----------|---------|--------------|
| `GITHUB_TOKEN` | GitHub API authentication | Higher rate limits (5000/hr vs 60/hr), download specs |
...
| File | Purpose |
|------|---------|
| `config/download.yaml` | Download source configuration (GitHub Releases) |
...
**Source Repository**: [robinmordasiewicz/f5xc-api-fixed](https://github.com/robinmordasiewicz/f5xc-api-fixed)

**Release Format**:
- Versioning: `vYYYY.MM.DD-N` (date-based with sequence)
- Asset: `f5xc-api-fixed-v{version}.zip` (5.7 MB compressed)
- Contents: 268 domain specs in `domains/` directory

**Rate Limits**:
- Unauthenticated: 60 requests/hour
- Authenticated (GITHUB_TOKEN): 5000 requests/hour
- CI/CD: Uses built-in `secrets.GITHUB_TOKEN` automatically
...
**Caching Mechanism**:
- Version tracking: `.github_release` (JSON metadata with version, timestamp, asset info)
- Replaces: `.etag` (legacy HTTP ETag caching)
- Format:
  ```json
  {
    ...
    "asset_name": "f5xc-api-fixed-v2026.01.22-2.zip",
    "asset_size": 5971024,
    "downloaded_at": "2026-01-25T15:45:00Z"
  }
  ```

**Benefits**:
- Pre-validated specifications (fixed schemas, corrected refs)
- Faster than HTTP download (5.7 MB vs 25 MB)
- Version-based caching (more reliable than ETag)
- Full control via source repository
- Easy rollback to any release
- Native GitHub Actions integration

**Rollback** (if needed):
Edit `config/download.yaml` to use legacy HTTP source:
```yaml
source:
  type: http_download
...
### 4. Version Caching May Skip Downloads

**Issue**: `make download` reports "No changes" unexpectedly
**Detection**: Check `.github_release` modification date and version
