...
description: Anreicherungsmetadaten für HTTP-Load-Balancer-Schemas
sidebar:
  order: 2
i18n:
...
Anreicherungsmetadaten für HTTP-Load-Balancer-Schemas. Siehe [OpenAPI-Erweiterungen](../development/#openapi-extensions) für Erweiterungsdefinitionen.

## Angereicherte Schemas

### Muster

Alle Schemas, die `viewshttp_loadbalancer.*SpecType` entsprechen, erhalten Anreicherungen:

- `viewshttp_loadbalancerCreateSpecType`
- `viewshttp_loadbalancerReplaceSpecType`
- `viewshttp_loadbalancerGetSpecType`

## Minimalkonfigurations-Metadaten

Die `x-f5xc-minimum-configuration`-Erweiterung stellt umfassende CLI-Metadaten für die Erstellung minimal funktionsfähiger HTTP-Load-Balancer bereit.
...
**Hinweis**: Die OneOf-Gruppe des Load-Balancer-Typs wird bezeichnet als:
...
Felder, die mit `x-f5xc-server-default: true` gekennzeichnet sind, erhalten ihren `default`-Wert vom F5 XC API-Server, wenn sie in Anfragen ausgelassen werden. HTTP-Load-Balancer verfügen über eine große Anzahl serverseitig angewendeter Standardwerte, da die meisten Sicherheitsfunktionen und Protokolloptionen standardmäßig einen deaktivierten oder sicheren Zustand aufweisen.

### HTTPS Auto-Cert-Standardwerte

Wenn `https_auto_cert` angegeben ist, wendet der Server diese Standardwerte für ausgelassene Felder an:
...
| `tls_config.default_security` | `{}` | object | TLS 1.2+ mit starken Chiffren |
| `no_mtls` | `{}` | object | Mutual TLS deaktiviert |
| `default_header` | `{}` | object | Standard-Servernamen-Header-Behandlung |
...
### Sicherheitsfunktions-Standardwerte

Alle Sicherheitsfunktionen sind standardmäßig deaktiviert, wenn sie ausgelassen werden:

| Feld | Standardwert | Beschreibung |
|------|-------------|--------------|
| `disable_waf` | `{}` | Web Application Firewall deaktiviert |
...
| `disable_threat_mesh` | `{}` | Threat Mesh deaktiviert |
| `disable_malicious_user_detection` | `{}` | Erkennung bösartiger Benutzer deaktiviert |

### DDoS- und Zugriffskontroll-Standardwerte

| Feld | Standardwert | Beschreibung |
|------|-------------|--------------|
| `l7_ddos_protection.mitigation_block` | `{}` | DDoS-Verkehr blockieren (Standard-Mitigationsaktion) |
...
| `l7_ddos_protection.ddos_policy_none` | `{}` | Keine DDoS-Richtlinienreferenz |
| `no_challenge` | `{}` | Keine Client-Challenge |
...
| Feld | Standardwert | Typ | Beschreibung |
|------|-------------|-----|--------------|
| `advertise_on_public_default_vip` | `{}` | object | Auf öffentlicher Standard-VIP bewerben |
...
| `system_default_timeouts` | `{}` | object | System-Standard-Timeouts verwenden |
| `service_policies_from_namespace` | `{}` | object | Service-Richtlinien vom Namespace erben |
...
Felder, die mit `x-f5xc-conflicts-with` gekennzeichnet sind, zeigen OneOf-Muster an. Nur ein Feld aus jeder Gruppe kann angegeben werden.
...
| `advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | Wie der Load Balancer beworben wird |
| `load_balancing_algorithm` | `round_robin`, `least_request`, `ring_hash`, `random` | Algorithmus zur Verkehrsverteilung |
...
| `server_name_header` | `default_header`, `append_server_name_header`, `pass_through_server_name_header` | Servernamen-Header-Behandlung |
...
| `bot_defense` | `disable_bot_defense`, `enable_bot_defense` | Bot-Erkennung und -Mitigation |
...
### DDoS-Schutzgruppen (4 Gruppen)

| Gruppenname | Felder | Beschreibung |
|-------------|--------|--------------|
| `ddos_mitigation` | `mitigation_block`, `mitigation_challenge`, `mitigation_none` | DDoS-Mitigationsaktion |
| `ddos_rps_threshold` | `default_rps_threshold`, `custom_rps_threshold` | Anfragen-pro-Sekunde-Schwellenwert |
| `ddos_clientside_action` | `clientside_action_none`, `clientside_action_javascript`, `clientside_action_captcha` | Clientseitige DDoS-Validierungsaktion |
| `ddos_policy` | `ddos_policy_none`, `ddos_policy_ref` | DDoS-Richtlinienreferenz oder keine |

### Weitere Einstellungsgruppen (6 Gruppen)

| Gruppenname | Felder | Beschreibung |
|-------------|--------|--------------|
| `challenge` | `no_challenge`, `js_challenge`, `captcha_challenge` | Client-Challenge-Typ zur Bot-Erkennung |
...
| `timeouts` | `system_default_timeouts`, `custom_timeouts` | System-Standard- oder benutzerdefinierte Timeouts verwenden |
| `service_policies_source` | `service_policies_from_namespace`, `active_service_policies` | Service-Richtlinien vom Namespace oder aktive Liste |
...
## OneOf-Varianten-Empfehlungen (Zukünftige Erweiterung)

**Status**: Noch nicht auf http_loadbalancer-Schemas angewendet.

Im Gegensatz zu Healthcheck, das `x-f5xc-recommended-oneof-variant` enthält, um die häufigste Wahl für OneOf-Gruppen anzugeben, fehlt diese Erweiterung bei http_loadbalancer derzeit.
...
1. Standardauswahlen der F5 XC-Konsole beobachten (z. B. welcher lb_type in der Benutzeroberfläche vorausgewählt ist)
2. Konfiguration zu `config/discovered_defaults.yaml` hinzufügen
3. Anreicherungs-Pipeline erneut ausführen, um die Erweiterung anzuwenden

**Erwartete Struktur nach Hinzufügung**:
```yaml
viewshttp_loadbalancerCreateSpecType:
  x-f5xc-recommended-oneof-variant:
...
Diese Erweiterung würde es nachgelagerten Tools ermöglichen, die am häufigsten verwendete Variante vorauszuwählen, wenn Benutzern Konfigurationsoptionen präsentiert werden.

## Einschränkungs-Metadaten

Felder, die mit `x-f5xc-constraints` gekennzeichnet sind, enthalten Validierungseinschränkungen, die aus der API-Analyse ermittelt wurden.
...
| `spec.l7_ddos_protection.mitigation` | `mitigation_block`, `mitigation_challenge`, `mitigation_none` | `mitigation_block` | Layer-7-DDoS-Mitigationsaktion |
...
| `spec.challenge` | `no_challenge`, `js_challenge`, `captcha_challenge` | `no_challenge` | Client-Challenge-Typ |
| `spec.advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | `advertise_on_public_default_vip` | Load-Balancer-Bewerbung |

## Integration geführter Workflows

Die Anreicherungs-Pipeline fügt `x-f5xc-guided-workflows`-Metadaten auf Spec-Ebene hinzu. Diese Workflows bieten schrittweise Bereitstellungsanleitungen, die von KI-Assistenten und CLI-Tools genutzt werden.

### HTTP-Load-Balancer bereitstellen

Ein 5-Schritte-Workflow zur Erstellung eines vollständig konfigurierten HTTP-Load-Balancers mit Backend-Origin-Pool (definiert in `config/guided_workflows.yaml`):

| Schritt | Aktion | Ressource | Erforderliche Felder | Optional |
|---------|--------|-----------|---------------------|----------|
| 1 | Origin Pool erstellen | `origin_pool` | `name`, `origin_servers`, `port` | Nein |
...
| 5 | Bereitstellung verifizieren | - | - | Nein |
...
Ein 7-Schritte-Workflow zur Erstellung eines HTTPS-Load-Balancers mit SSL/TLS-Terminierung:

| Schritt | Aktion | Ressource | Erforderliche Felder | Optional |
|---------|--------|-----------|---------------------|----------|
| 1 | SSL-Zertifikat hochladen | `certificate` | `name`, `certificate_chain`, `private_key` | Nein |
| 2 | Origin Pool erstellen | `origin_pool` | `name`, `origin_servers` | Nein |
...
| 7 | HTTPS-Bereitstellung verifizieren | - | - | Nein |

**Voraussetzungen**: Gültiger Namespace im Ziel-Tenant, SSL-Zertifikat und privater Schlüssel, Backend-Anwendung erreichbar.

### Workflow-Nutzung

Nachgelagerte Tools verwenden diese Workflows, um Benutzer durch mehrstufige Bereitstellungen zu führen:

- **xcsh CLI**: Präsentiert Workflows als interaktive geführte Befehle (z. B. `xcsh deploy http-lb`)
- **VS Code-Erweiterung**: Zeigt Workflow-Schritte in einem Seitenleisten-Panel mit Fortschrittsverfolgung an
- **KI-Assistenten**: Folgen den Workflow-Schritten sequenziell, erstellen jede Ressource und validieren Abhängigkeiten

## OpenAPI-Erweiterungsreferenz

Diese herstellerspezifischen Erweiterungen werden dem Standard-OpenAPI-Schema hinzugefügt, um F5 XC-spezifische Metadaten zu übermitteln.

### x-f5xc-cli-domain

**Typ**: `string`

Gibt die Domain-Klassifizierung für CLI- und Tool-Organisation an.
...
Stellt umfassende Metadaten für die Erstellung minimal funktionsfähiger Konfigurationen bereit. Enthält:
- Beschreibung
- Erforderliche Felder mit Einschränkungen
...
Listet Feldnamen auf, die sich mit dem aktuellen Feld gegenseitig ausschließen, was auf OneOf-Muster hinweist.
...
Stellt Validierungseinschränkungen bereit, einschließlich Array-Größenbegrenzungen, Ganzzahlbereiche, Eindeutigkeitsanforderungen und Erkennungs-Metadaten.
...
Stellt konkrete Feldbeispiele für Dokumentation und Tools bereit.
...
Gibt kontextspezifische Anforderungen mit Flags an: `minimum_config`, `create`, `update`, `read`.
...
| Datei | Inhalt |
|-------|--------|
| `docs/specifications/api/virtual.json` | Alle http_loadbalancer-Schemas mit Anreicherungen |
...
| Datei | Zweck |
|-------|-------|
| `config/minimum_configs.yaml` | Quelle der Minimalkonfigurations-Metadaten |
| `config/constraint_patterns.yaml` | Definitionen von Einschränkungsmustern |
| `config/guided_workflows.yaml` | Definitionen der geführten Workflow-Schritte |
| `config/domain_descriptions.yaml` | Beschreibungs-Metadaten auf Domain-Ebene |
...
- [Leitfaden zu Einschränkungs-Metadaten](../constraint-metadata/) - Detaillierte Dokumentation zur Einschränkungsvalidierung

## Änderungsprotokoll

| Version | Datum | Änderungen |
|---------|-------|------------|
| 2.0.46 | 18.04.2026 | Serverseitig angewendete Standardwerte, Integration geführter Workflows und erweiterter Minimalkonfigurationsabschnitt hinzugefügt |
| 2.0.45 | 20.01.2026 | Erste Dokumentation der http_loadbalancer-Anreicherungen |
