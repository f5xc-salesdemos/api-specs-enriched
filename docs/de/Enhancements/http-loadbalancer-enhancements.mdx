...
Alle Schemas, die auf `viewshttp_loadbalancer.*SpecType` passen, erhalten Anreicherungen:

- `viewshttp_loadbalancerCreateSpecType`
- `viewshttp_loadbalancerReplaceSpecType`
- `viewshttp_loadbalancerGetSpecType`

## Metadaten für Minimalkonfiguration

Die `x-f5xc-minimum-configuration`-Erweiterung bietet umfassende CLI-Metadaten zur Erstellung minimal funktionsfähiger HTTP-Load-Balancer.
...
**Hinweis**: Die OneOf-Gruppe des Load-Balancer-Typs wird wie folgt bezeichnet:
...
Felder, die mit `x-f5xc-server-default: true` gekennzeichnet sind, erhalten ihren `default`-Wert vom F5 XC API-Server, wenn sie in Anfragen weggelassen werden. Der HTTP-Load-Balancer verfügt über eine große Anzahl serverseitig angewendeter Standardwerte, da die meisten Sicherheitsfunktionen und Protokolloptionen standardmäßig auf einen deaktivierten oder sicheren Zustand gesetzt sind.
...
| `disable_client_side_defense` | `{}` | Clientseitiger Schutz deaktiviert |
...
| Feld | Standardwert | Beschreibung |
|------|--------------|--------------|
| `l7_ddos_protection.mitigation_block` | `{}` | DDoS-Datenverkehr blockieren (Standard-Mitigationsmaßnahme) |
...
Felder, die mit `x-f5xc-conflicts-with` gekennzeichnet sind, weisen auf OneOf-Muster hin. Aus jeder Gruppe kann nur ein Feld angegeben werden.
...
| `client_side_defense` | `disable_client_side_defense`, `enable_client_side_defense` | Clientseitiger Schutz |
...
| Gruppenname | Felder | Beschreibung |
|-------------|--------|--------------|
| `ddos_mitigation` | `mitigation_block`, `mitigation_challenge`, `mitigation_none` | DDoS-Mitigationsmaßnahme |
...
Im Gegensatz zu Healthcheck, das `x-f5xc-recommended-oneof-variant` enthält, um die gängigste Wahl für OneOf-Gruppen anzuzeigen, fehlt http_loadbalancer derzeit diese Erweiterung.
...
1. Beobachten Sie die Standardauswahlen der F5 XC-Konsole (z. B. welcher lb_type in der Benutzeroberfläche vorausgewählt ist)
2. Fügen Sie die Konfiguration zu `config/discovered_defaults.yaml` hinzu
3. Führen Sie die Anreicherungspipeline erneut aus, um die Erweiterung anzuwenden
...
| `spec.l7_ddos_protection.mitigation` | `mitigation_block`, `mitigation_challenge`, `mitigation_none` | `mitigation_block` | Layer-7-DDoS-Mitigationsmaßnahme |
...
- **KI-Assistenten**: Folgen den Workflow-Schritten sequentiell, erstellen jede Ressource und validieren Abhängigkeiten
...
Gibt die Domänenklassifikation für CLI- und Tooling-Organisation an.
...
Bietet umfassende Metadaten zur Erstellung minimal funktionsfähiger Konfigurationen. Enthält:
- Beschreibung
...
Listet Feldnamen auf, die sich mit dem aktuellen Feld gegenseitig ausschließen und OneOf-Muster anzeigen.
...
Bietet Validierungseinschränkungen einschließlich Array-Größenbegrenzungen, Integer-Bereiche, Eindeutigkeitsanforderungen und Discovery-Metadaten.
...
Bietet alternative Beschreibungslängen für verschiedene Anwendungsfälle:
...
Bietet konkrete Feldbeispiele für Dokumentation und Tooling.
...
| `config/guided_workflows.yaml` | Geführte Workflow-Schritt-Definitionen |
...
|---------|-------|------------|
| 2.0.46 | 2026-04-18 | Serverseitig angewendete Standardwerte, Integration geführter Workflows und erweiterten Abschnitt zur Minimalkonfiguration hinzugefügt |
| 2.0.45 | 2026-01-20 | Erstdokumentation der http_loadbalancer-Anreicherungen |
