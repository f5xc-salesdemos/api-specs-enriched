...
## Metadaten zur Minimalkonfiguration

Die Erweiterung `x-f5xc-minimum-configuration` stellt umfassende CLI-Metadaten für die Erstellung minimal funktionsfähiger HTTP-Load-Balancer bereit.
...
Felder, die mit `x-f5xc-server-default: true` gekennzeichnet sind, erhalten ihren `default`-Wert vom F5-XC-API-Server, wenn sie in Anfragen weggelassen werden. Der HTTP-Load-Balancer hat eine große Anzahl serverseitig angewandter Standardwerte, da die meisten Sicherheitsfunktionen und Protokolloptionen standardmäßig auf einen deaktivierten oder sicheren Zustand gesetzt sind.

### HTTPS-Auto-Cert-Standardwerte

Wenn `https_auto_cert` angegeben wird, wendet der Server diese Standardwerte für weggelassene Felder an:
...
| `coalescing_options.default_coalescing` | `{}` | object | Standard-HTTP/2-Verbindungsbündelung |
...
| Feld | Standardwert | Beschreibung |
|------|-------------|--------------|
| `l7_ddos_protection.mitigation_block` | `{}` | DDoS-Datenverkehr blockieren (Standard-Mitigationsaktion) |
...
### Weitere serverseitig angewandte Standardwerte
...
Felder, die mit `x-f5xc-conflicts-with` gekennzeichnet sind, weisen auf OneOf-Muster hin. Nur ein Feld aus jeder Gruppe kann angegeben werden.
...
| `advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | Wie der Load Balancer bekanntgemacht wird |
| `load_balancing_algorithm` | `round_robin`, `least_request`, `ring_hash`, `random` | Algorithmus zur Datenverkehrsverteilung |
...
| `coalescing` | `default_coalescing`, `disable_coalescing`, `enable_for_same_origin` | HTTP/2-Verbindungsbündelungsoptionen |
...
| `user_identification` | `user_id_client_ip`, `user_identification` | Methode zur Benutzeridentifikation |
...
Im Gegensatz zu healthcheck, das `x-f5xc-recommended-oneof-variant` enthält, um die häufigste Auswahl für OneOf-Gruppen anzugeben, fehlt dem http_loadbalancer diese Erweiterung derzeit.
...
1. F5-XC-Konsolenauswahlen beobachten (z. B. welcher lb_type in der UI vorausgewählt ist)
...
**Erwartete Struktur nach Hinzufügen**:
```yaml
viewshttp_loadbalancerCreateSpecType:
  x-f5xc-recommended-oneof-variant:
...
Diese Erweiterung würde nachgelagerten Tools ermöglichen, die am häufigsten verwendete Variante vorauszuwählen, wenn Benutzern Konfigurationsoptionen präsentiert werden.
...
| `spec.https_auto_cert.coalescing_options` | `default_coalescing`, `disable_coalescing`, `enable_for_same_origin` | `default_coalescing` | HTTP/2-Verbindungsbündelung |
...
Die Anreicherungspipeline fügt `x-f5xc-guided-workflows`-Metadaten auf Spec-Ebene hinzu. Diese Workflows bieten schrittweise Bereitstellungsanleitungen, die von KI-Assistenten und CLI-Tools verwendet werden.
...
**Voraussetzungen**: Gültiger Namespace im Ziel-Tenant, Backend-Anwendung erreichbar über IP oder DNS, SSL-Zertifikat (optional für HTTPS).
...
- **VS-Code-Erweiterung**: Zeigt Workflow-Schritte in einem Seitenleisten-Panel mit Fortschrittsverfolgung
...
Gibt die Domänenklassifizierung für CLI- und Tooling-Organisation an.
...
Wenn `true`, gibt an, dass der zugehörige `default`-Wert vom F5-XC-API-Server durchgesetzt wird. Felder mit dieser Erweiterung können sicher aus API-Anfragen weggelassen werden – der Server wendet den Standardwert automatisch an.
...
Stellt Validierungseinschränkungen bereit, einschließlich Array-Größenlimits, Ganzzahlbereiche, Eindeutigkeitsanforderungen und Erkennungsmetadaten.
...
Stellt konkrete Feldbeispiele für Dokumentation und Tooling bereit.
...
Gibt kontextspezifische Anforderungen mittels Flags an: `minimum_config`, `create`, `update`, `read`.
...
| `config/constraint_patterns.yaml` | Einschränkungsmuster-Definitionen |
...
- [Entwicklungshandbuch – OpenAPI-Erweiterungen](../development/#openapi-extensions) – Erweiterungsdefinitionen und -nutzung
...
- [Leitfaden für Einschränkungsmetadaten](../constraint-metadata/) – Detaillierte Dokumentation zur Einschränkungsvalidierung

## Änderungsprotokoll

| Version | Datum | Änderungen |
|---------|-------|------------|
| 2.0.46 | 2026-04-18 | Serverseitig angewandte Standardwerte, Integration geführter Workflows und erweiterter Abschnitt zur Minimalkonfiguration hinzugefügt |
