...
- **Terraform**: Constraints zur Planungszeit durchsetzen, nicht zur Ausführungszeit
- **IDE-Erweiterungen**: Echtzeit-Validierung und Autovervollständigungsvorschläge
- **API-Validatoren**: Vorab-Validierung vor der Übermittlung zur Reduzierung von API-Fehlern

### Abdeckungsstatistiken

- **9.851 Constraints** über alle F5 XC API-Spezifikationen hinweg
- **50+ Mustertypen** (String, Array, Numerisch)
- **95 % Konfidenz** bei Feldern mit hoher Priorität (Namen, Ports, Bezeichner)
...
| *(Constraint-Schlüssel)* | verschieden | Typspezifische Constraint-Schlüssel auf oberster Ebene (z. B. `minLength`, `maxItems`, `minimum`) |
| `metadata` | object | Quellnachverfolgung, Konfidenz, Validierungszeitstempel |
...
| `fqdn` | Vollständig qualifizierter Domainname | `api.example.com` |
...
| `json` | JSON-String (parsierbar) | `{"key": "value"}` |
| `yaml` | YAML-String (parsierbar) | `key: value` |
| `base64` | Base64-kodierter String | `SGVsbG8gV29ybGQ=` |
| `hex` | Hexadezimal-String | `48656c6c6f` |
...
- Bindestriche erlaubt, jedoch nicht am Anfang/Ende (RFC 1123)
...
**FQDNs** (`format: "fqdn"`):
- Vollständig qualifizierte Domainnamen müssen 1–253 Zeichen lang sein
...
- ✅ `80` (HTTP)
- ✅ `443` (HTTPS)
- ✅ `8080` (gebräuchlicher alternativer HTTP-Port)
...
- Konfidenzwert >= 0,9
- Muster wurde gegen Discovery-Daten validiert ODER
- Muster entspricht anerkannten Standards (RFC, ISO usw.)
...
Constraints werden nach einem 3-stufigen Prioritätssystem abgestimmt:
...
2. **DISCOVERY** (Konfidenz: 0,99)
   ...
   - Überschreibt inferierte Constraints

3. **INFERRED** (Konfidenz: 0,50–0,95)
   - Musterabgleich anhand von Feldnamen
...
### Muster mit hoher Konfidenz (0,95–0,99)
...
### Muster mit mittlerer Konfidenz (0,80–0,94)
...
- **Probleme**: Constraint-Genauigkeitsprobleme melden unter https://github.com/f5xc-salesdemos/api-specs-enriched/issues
- **Dokumentation**: Vollständige API-Dokumentation unter https://f5xc-salesdemos.github.io/api-specs-enriched/
- **MCP-Integration**: Siehe `f5xc-api-mcp`-Repository für MCP-Server-Integrationsbeispiele
