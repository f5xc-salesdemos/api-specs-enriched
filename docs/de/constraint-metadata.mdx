...
4. [Natürlichsprachliche Beschreibungen](#natürlichsprachliche-beschreibungen)
...
Die `x-f5xc-constraints`-Erweiterung stellt deterministisches Wissen über Feldvalidierungs-Constraints bereit und ermöglicht:

- **KI-Assistenten**: Gültige Konfigurationen ohne Benutzereingabe für Constraint-Details generieren
- **CLI-Tools**: Genaue Eingabehinweise, Validierung und Fehlermeldungen bereitstellen
...
- **API-Validatoren**: Vor-Absendung-Validierung zur Reduzierung von API-Fehlern
...
| `json` | JSON-String (parsbar) | `{"key": "value"}` |
| `yaml` | YAML-String (parsbar) | `key: value` |
...
- DNS-Label-Namen müssen 1-63 Zeichen lang sein und aus Kleinbuchstaben und Ziffern bestehen
...
- Müssen ein Schema enthalten (http/https)
...
**Origin-Pools**:
- Das Array der Origin-Pools erfordert mindestens 1 Element, maximal 50 Elemente
...
### Beispiel 2: Portnummer-Validierung
...
3. **ABGELEITET** (Konfidenz: 0,50-0,95)
   - Musterbasiert aus Feldnamen abgeglichen
   - Niedrigste Priorität, wird durch Discovery oder Explizit überschrieben
...
def validate_cli_input(field_name, user_value, constraints):
    ...
    # Länge prüfen (Constraint-Schlüssel sind auf oberster Ebene)
...
func generateTerraformSchema(property map[string]interface{}) *schema.Schema {
    ...
    // Constraint-Schlüssel sind auf oberster Ebene (kein verschachtelter "string"-Wrapper)
...
| Muster | Feldnamen-Regex | Constraint-Typ | Beispiel |
|--------|-----------------|----------------|----------|
...
- **MCP-Integration**: Siehe das `f5xc-api-mcp`-Repository für Beispiele zur MCP-Server-Integration
