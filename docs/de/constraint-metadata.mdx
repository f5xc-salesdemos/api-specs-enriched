...
8. [Leitfaden zur Werkzeug-Integration](#tool-integration-guide)

---

## Übersicht

Die Erweiterung `x-f5xc-constraints` stellt deterministisches Wissen über Feld-Validierungsconstraints bereit und ermöglicht Folgendes:

- **KI-Assistenten**: Gültige Konfigurationen generieren, ohne den Benutzer nach Constraint-Details zu fragen
- **CLI-Werkzeuge**: Präzise Eingabehinweise, Validierung und Fehlermeldungen bereitstellen
...
- **API-Validatoren**: Vorab-Validierung vor dem Senden zur Reduzierung von API-Fehlern

### Abdeckungsstatistiken

- **9.851 Constraints** über alle F5 XC API-Spezifikationen hinweg
- **50+ Mustertypen** (Zeichenkette, Array, numerisch)
...
| `category` | string | Constraint-Kategorie zur Gruppierung und Berichterstattung |
| *(Constraint-Schlüssel)* | verschiedene | Typspezifische Constraint-Schlüssel, auf oberster Ebene aufgeführt (z. B. `minLength`, `maxItems`, `minimum`) |
| `metadata` | object | Quellverfolgung, Konfidenz, Validierungszeitstempel |
...
| `fqdn` | Vollqualifizierter Domänenname | `api.example.com` |
...
### Zeichenketten-Format-Constraints
...
**FQDNs** (`format: "fqdn"`):
- Vollqualifizierte Domänennamen müssen 1–253 Zeichen umfassen
...
- Schema (http/https) muss enthalten sein
...
**Ungültige Werte**:
- ❌ `0` (unterhalb des Minimums)
- ❌ `65536` (oberhalb des Maximums)
- ❌ `-1` (negativ)
...
- ❌ `[Array mit 51 Elementen]` (oberhalb des Maximums)
...
- Konfidenzwert >= 0,9
- Das Muster anhand von Discovery-Daten validiert wurde ODER
- Das Muster anerkannten Standards entspricht (RFC, ISO usw.)
...
if constraint.get("deterministic"):
    ...
    # Benutzer mit Constraint-Hinweisen nach Eingabe fragen
...
Constraints werden nach einer 3-stufigen Priorität abgestimmt:

1. **EXISTING** (Konfidenz: 1,0)
   - Explizit in der ursprünglichen OpenAPI-Spezifikation gesetzt
...
3. **INFERRED** (Konfidenz: 0,50–0,95)
   - Aus Feldnamen per Mustererkennung abgeleitet
...
## Leitfaden zur Werkzeug-Integration
...
def generate_resource_name(schema_property):
    ...
    # Gültigen Namen generieren, der dem Muster entspricht
