...
4. [Beschreibungen in natürlicher Sprache](#natural-language-descriptions)
...
8. [Leitfaden zur Werkzeugintegration](#tool-integration-guide)

---

## Übersicht

Die Erweiterung `x-f5xc-constraints` liefert deterministisches Wissen über Feldvalidierungs-Constraints und ermöglicht:

- **KI-Assistenten**: Gültige Konfigurationen generieren, ohne den Benutzer nach Constraint-Details zu fragen
- **CLI-Werkzeuge**: Genaue Eingabehinweise, Validierung und Fehlermeldungen bereitstellen
- **Terraform**: Constraints zur Planungszeit durchsetzen, nicht erst zur Anwendungszeit
- **IDE-Erweiterungen**: Echtzeit-Validierung und Autocomplete-Vorschläge
- **API-Validatoren**: Validierung vor der Übermittlung zur Reduzierung von API-Fehlern

### Abdeckungsstatistiken

- **9.851 Constraints** über alle F5 XC API-Spezifikationen hinweg
- **Mehr als 50 Mustertypen** (Zeichenkette, Array, numerisch)
- **95 % Konfidenz** bei Feldern hoher Priorität (Namen, Ports, Bezeichner)
- **3-stufige Abstimmung**: EXISTING > DISCOVERY > INFERRED
...
| `deterministic` | boolean | Hochkonfidenz-Flag (Konfidenz >= 0,9) für die KI-Generierung |
| `category` | string | Constraint-Kategorie für Gruppierung und Berichterstattung |
| *(Constraint-Schlüssel)* | verschiedene | Typspezifische Constraint-Schlüssel, auf oberster Ebene abgeflacht (z. B. `minLength`, `maxItems`, `minimum`) |
...
| `json` | JSON-Zeichenkette (parsierbar) | `{"key": "value"}` |
| `yaml` | YAML-Zeichenkette (parsierbar) | `key: value` |
| `base64` | Base64-kodierte Zeichenkette | `SGVsbG8gV29ybGQ=` |
| `hex` | Hexadezimale Zeichenkette | `48656c6c6f` |
...
### Zeichenketten-Formatbeschränkungen
...
**FQDNs** (`format: "fqdn"`):
- Vollqualifizierte Domainnamen müssen 1–253 Zeichen lang sein
...
- Muss ein Schema (http/https) enthalten
- Beispiel: `https://api.example.com/v1`, `http://localhost:8080`

**IPv4-Adressen** (`format: "ipv4"`):
- IPv4-Adressen müssen in Punkt-Dezimal-Notation angegeben werden
...
**Portnummern**:
- Portnummern müssen ganzzahlig zwischen 1 und 65535 liegen
- Standardports: 80 (HTTP), 443 (HTTPS), 22 (SSH)

**VLAN-IDs**:
- VLAN-IDs müssen ganzzahlig zwischen 1 und 4094 liegen (802.1Q-Standard)
...
- ✅ `80` (HTTP)
- ✅ `443` (HTTPS)
- ✅ `8080` (gängiger alternativer HTTP-Port)
...
| 0,70–0,89 | **Mittlere Konfidenz** | Empfehlend, Benutzerbestätigung empfohlen |
...
- Das Muster mit etablierten Standards übereinstimmt (RFC, ISO usw.)

**Verwendung bei der KI-Generierung**:
```python
if constraint.get("deterministic"):
    # Wert automatisch generieren, ohne den Benutzer zu fragen
    value = generate_from_pattern(constraint)
else:
    # Benutzer mit Constraint-Hinweisen um Eingabe bitten
...
Constraints werden mithilfe einer 3-stufigen Priorität abgestimmt:
...
3. **INFERRED** (Konfidenz: 0,50–0,95)
   - Musterbasiert aus Feldnamen abgeleitet
...
## Leitfaden zur Werkzeugintegration
...
def validate_cli_input(field_name, user_value, constraints):
    ...
    # Länge prüfen (Constraint-Schlüssel befinden sich auf der obersten Ebene)
...
func generateTerraformSchema(property map[string]interface{}) *schema.Schema {
    ...
    // Constraint-Schlüssel befinden sich auf der obersten Ebene (kein verschachtelter "string"-Wrapper)
...
|---------|------------------|-----------------|---------|
| DNS-Label | `\bname$` | Zeichenkette (1–63 Zeichen, dns-label) | `my-service` |
| Namespace | `\bnamespace$` | Zeichenkette (1–63 Zeichen, dns-label) | `production` |
| Port | `\bport$` | Zahl (1–65535) | `443` |
| VLAN-ID | `\b(vlan)?_?id$` | Zahl (1–4094) | `100` |
| UUID | `\buuid$` | Zeichenkette (36 Zeichen, uuid-Format) | `550e8400-...` |
| E-Mail | `\bemail$` | Zeichenkette (max. 254 Zeichen, E-Mail) | `user@example.com` |
| Zeitstempel | `\b(timestamp\|created_at\|updated_at)$` | Zeichenkette (ISO 8601) | `2026-01-19T12:00:00Z` |

### Mittelkonfidenz-Muster (0,80–0,94)

| Muster | Feldname-Regex | Constraint-Typ | Beispiel |
|---------|------------------|-----------------|---------|
| Benutzername | `\b(user\|username)$` | Zeichenkette (1–64 Zeichen, alphanumerisch) | `admin` |
| Anzeigename | `\b(display_?)?name$` | Zeichenkette (1–256 Zeichen, Freitext) | `My Service` |
| Beschreibung | `\b(comment\|description\|note)$` | Zeichenkette (max. 4096 Zeichen) | `Service description` |
| Pfad | `\bpath$` | Zeichenkette (max. 2048 Zeichen) | `/api/v1/resources` |

---

## Support und Feedback

- **Probleme**: Melden Sie Probleme mit der Constraint-Genauigkeit unter https://github.com/f5xc-salesdemos/api-specs-enriched/issues
...
**Musterabdeckung**: Mehr als 50 Typen
