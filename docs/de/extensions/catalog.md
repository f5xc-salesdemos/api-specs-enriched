...
# Katalog der Anreicherungserweiterungen
...
- **Upstream-Durchreichung** — Erweiterungen, die F5 in den Quellspezifikationen
  ausgibt und die wir unverändert beibehalten (`x-ves-proto-*`, `x-displayname`, etc.).
  Der Transparenz halber dokumentiert, aber nicht von diesem Repository gesteuert.
...
solange die `### x-name`-Überschrift existiert und das Flag
...
- **Applied at:** info
- **Purpose:** CLI-weiter Metadatenblock (Toolname, Versionshinweise, Domain-Gruppierung).
- **Consumers:** CLI
- **Value type:** object
...
- **Applied at:** info
- **Purpose:** ETag des Upstream-Quellspezifikations-Release-Assets.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Branding-/Terminologie-Glossarblock, der auf jede Domain-Spezifikation angewendet wird.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** info
- **Purpose:** Beobachtete Antwortzeit (ms) der abgefragten API während der Discovery.
- **Consumers:** multiple
- **Value type:** number
...
- **Applied at:** info
- **Purpose:** Kuratierte Best-Practice-Empfehlungen für eine Domain.
- **Consumers:** multiple
- **Value type:** array
...
- **Applied at:** info
- **Purpose:** Domain-spezifische Akronym-Expansionstabelle.
- **Consumers:** multiple
- **Value type:** object
...
- **Applied at:** schema
- **Purpose:** Minimaler Feldsatz, der erforderlich ist, um diese Ressource erfolgreich per POST/PUT zu erstellen.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** schema property
- **Purpose:** Deklarative Validierungsregeln, abgeleitet aus Upstream-Protobuf-`ves.io.schema.rules`.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** schema property
- **Purpose:** Shell-Vervollständigungshinweise (statische Enum oder dynamischer Befehl).
- **Consumers:** CLI
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Standardwert(e), die in generierter Dokumentation und UIs angezeigt werden sollen.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Listet benannte Feature-Kombinationen auf, die diese Eigenschaft erfordern.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/utils/minimum_configuration_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Bedingte Anforderungen (z. B. erforderlich, wenn ein Geschwisterfeld den Wert X hat).
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Deprecation-Hinweis mit Empfehlung für den Ersatz.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Empfohlener Produktionswert für ein Feld, bei dem der Server-Standardwert suboptimal ist.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{}`
- **Injected by:** scripts/utils/default_value_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Listet Geschwistereigenschaften auf, die nicht gleichzeitig mit dieser gesetzt werden können.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
...
- **Applied at:** schema property
- **Purpose:** Dokumentiert feldübergreifende Abhängigkeiten, bei denen ein Feld ein anderes als gesetzt voraussetzt.
...
- **Applied at:** schema property
- **Purpose:** Numerische / String-Einschränkungen, abgeleitet aus Live-API-Abfragen oder statischen Mustern.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** schema property
- **Purpose:** Deklariert, ob ein Feld innerhalb seines Geltungsbereichs eindeutig sein muss.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Purpose:** Benennt die Felder im Operationskörper, die für den Erfolg bereitgestellt werden müssen.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
...
- **Applied at:** operation
- **Purpose:** Klassifiziert den Wirkungsradius einer Operation (low/medium/high/critical).
- **Consumers:** multiple
- **Value type:** string
...
- **Applied at:** operation
- **Purpose:** Ob CLI/UI den Benutzer vor der Ausführung zur Bestätigung auffordern soll.
- **Consumers:** multiple
- **Value type:** boolean
...
- **Applied at:** operation
- **Purpose:** Beobachtete Rate-Limit-Header / Verhalten, das von der Live-API ermittelt wurde.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** info
- **Purpose:** Mindestens erforderliche F5 XC-Abonnementstufe.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Kennzeichnet eine Domain als Preview-/Beta-Feature.
- **Consumers:** multiple
- **Value type:** boolean
- **Value schema:** `{"type": "boolean"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Icon-Bezeichner, der beim Rendern dieser Domain in einer UI verwendet wird.
- **Consumers:** Web UI
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Querverweise zu anderen Domains, die häufig zusammen mit dieser verwendet werden.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/merge_specs.py
