---
title: Mejoras del HTTP LoadBalancer
description: Metadatos de enriquecimiento para esquemas del balanceador de carga HTTP
sidebar:
  order: 2
i18n:
...
Metadatos de enriquecimiento para esquemas del balanceador de carga HTTP. Consulte [Extensiones OpenAPI](../development/#openapi-extensions) para las definiciones de extensiones.

## Esquemas enriquecidos

### Patrón

Todos los esquemas que coinciden con `viewshttp_loadbalancer.*SpecType` reciben enriquecimientos:
...
La extensión `x-f5xc-minimum-configuration` proporciona metadatos CLI completos para crear balanceadores de carga HTTP mínimos viables.
...
### Tipo de balanceador de carga (OneOf obligatorio)
...
| `spec.http_https` | Tanto HTTP como HTTPS |

**Nota**: El grupo OneOf del tipo de balanceador de carga se denomina como:
- `lb_type` en archivos de configuración (forma abreviada en `config/minimum_configs.yaml`)
- `loadbalancer_type` en extensiones nativas de la especificación OpenAPI (nombre de campo nativo)
...
Los campos marcados con `x-f5xc-server-default: true` tienen su valor `default` aplicado por el servidor API de F5 XC cuando se omiten en las solicitudes. El balanceador de carga HTTP tiene una gran cantidad de valores predeterminados aplicados por el servidor porque la mayoría de las funciones de seguridad y opciones de protocolo tienen como valor predeterminado un estado deshabilitado o seguro.

### Valores predeterminados de HTTPS Auto-Cert
...
|-------|---------------|------|-------------|
| `port` | `443` | integer | Puerto de escucha HTTPS |
| `http_redirect` | `false` | boolean | Redirección de HTTP a HTTPS |
| `add_hsts` | `false` | boolean | Encabezado HTTP Strict Transport Security |
| `tls_config.default_security` | `{}` | object | TLS 1.2+ con cifrados fuertes |
| `no_mtls` | `{}` | object | TLS mutuo deshabilitado |
| `default_header` | `{}` | object | Manejo predeterminado del encabezado de nombre de servidor |
| `enable_path_normalize` | `{}` | object | Normalización de ruta habilitada |
| `default_loadbalancer` | `{}` | object | Configuración predeterminada del balanceador de carga |
| `header_transformation_type.legacy_header_transformation` | `{}` | object | Transformación de encabezados heredada |
| `connection_idle_timeout` | `120000` | integer | Tiempo de espera de conexión inactiva en milisegundos (2 minutos) |
| `http_protocol_options.http_protocol_enable_v1_v2` | `{}` | object | HTTP/1.1 y HTTP/2 habilitados |
| `coalescing_options.default_coalescing` | `{}` | object | Coalescencia de conexiones HTTP/2 predeterminada |
...
|-------|---------------|-------------|
| `disable_waf` | `{}` | Firewall de aplicaciones web deshabilitado |
| `disable_bot_defense` | `{}` | Defensa contra bots deshabilitada |
| `disable_rate_limit` | `{}` | Limitación de tasa deshabilitada |
...
### Valores predeterminados de DDoS y control de acceso

| Campo | Valor predeterminado | Descripción |
|-------|---------------|-------------|
| `l7_ddos_protection.mitigation_block` | `{}` | Bloquear tráfico DDoS (acción de mitigación predeterminada) |
| `l7_ddos_protection.default_rps_threshold` | `{}` | Usar umbral de RPS predeterminado |
| `l7_ddos_protection.clientside_action_none` | `{}` | Sin validación DDoS del lado del cliente |
| `l7_ddos_protection.ddos_policy_none` | `{}` | Sin referencia a política DDoS |
...
| Campo | Valor predeterminado | Tipo | Descripción |
|-------|---------------|------|-------------|
| `advertise_on_public_default_vip` | `{}` | object | Publicar en VIP público predeterminado |
| `round_robin` | `{}` | object | Algoritmo de balanceo de carga round-robin |
| `add_location` | `true` | boolean | Agregar encabezado de ubicación a las respuestas |
| `system_default_timeouts` | `{}` | object | Usar tiempos de espera predeterminados del sistema |
| `service_policies_from_namespace` | `{}` | object | Heredar políticas de servicio del espacio de nombres |
| `default_sensitive_data_policy` | `{}` | object | Usar política de datos sensibles predeterminada |

## Grupos de campos mutuamente excluyentes
...
| `advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | Cómo publicar el balanceador de carga |
...
| Nombre del grupo | Campos | Descripción |
|-----------|--------|-------------|
| `path_normalize` | `enable_path_normalize`, `disable_path_normalize` | Normalización de ruta habilitada o deshabilitada |
...
| Nombre del grupo | Campos | Descripción |
|-----------|--------|-------------|
| `waf` | `disable_waf`, `enable_waf` | Firewall de aplicaciones web |
| `bot_defense` | `disable_bot_defense`, `enable_bot_defense` | Detección y mitigación de bots |
| `rate_limit` | `disable_rate_limit`, `enable_rate_limit` | Limitación de tasa |
...
| `ddos_policy` | `ddos_policy_none`, `ddos_policy_ref` | Referencia a política DDoS o ninguna |

### Otros grupos de configuración (6 grupos)

| Nombre del grupo | Campos | Descripción |
|-----------|--------|-------------|
| `challenge` | `no_challenge`, `js_challenge`, `captcha_challenge` | Tipo de desafío al cliente para detección de bots |
| `user_identification` | `user_id_client_ip`, `user_identification` | Método de identificación de usuario |
...
A diferencia de healthcheck, que incluye `x-f5xc-recommended-oneof-variant` para indicar la opción más común para grupos OneOf, http_loadbalancer actualmente carece de esta extensión.
...
1. Observar las selecciones predeterminadas de la consola F5 XC (por ejemplo, qué lb_type está preseleccionado en la interfaz)
2. Agregar la configuración a `config/discovered_defaults.yaml`
...
Esta extensión permitiría a las herramientas posteriores preseleccionar la variante más comúnmente utilizada al presentar opciones de configuración a los usuarios.
...
| `spec.https_auto_cert.connection_idle_timeout` | 1000 | 3600000 | 120000 | Tiempo de espera de conexión inactiva (milisegundos, 1s a 1h) |
...
| `spec.l7_ddos_protection.rps_threshold` | `default_rps_threshold`, `custom_rps_threshold` | `default_rps_threshold` | Umbral de RPS para detección DDoS |
...
| `spec.advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | `advertise_on_public_default_vip` | Publicación del balanceador de carga |
...
| Paso | Acción | Recurso | Campos obligatorios | Opcional |
|------|--------|----------|-----------------|----------|
| 1 | Crear pool de origen | `origin_pool` | `name`, `origin_servers`, `port` | No |
| 2 | Configurar verificación de salud | `healthcheck` | `name`, `http_health_check` | Sí |
| 3 | Adjuntar verificación de salud | - | - | Sí |
| 4 | Crear balanceador de carga HTTP | `http_loadbalancer` | `name`, `domains`, `http.port` | No |
| 5 | Verificar despliegue | - | - | No |

**Requisitos previos**: Espacio de nombres válido en el tenant de destino, aplicación backend accesible por IP o DNS, certificado SSL (opcional para HTTPS).
...
| 3 | Configurar verificación de salud | `healthcheck` | - | Sí |
...
**Requisitos previos**: Espacio de nombres válido en el tenant de destino, certificado SSL y clave privada, aplicación backend accesible.

### Consumo de flujos de trabajo

Las herramientas posteriores utilizan estos flujos de trabajo para guiar a los usuarios a través de despliegues de múltiples pasos:

- **xcsh CLI**: Presenta los flujos de trabajo como comandos guiados interactivos (por ejemplo, `xcsh deploy http-lb`)
- **Extensión VS Code**: Muestra los pasos del flujo de trabajo en un panel lateral con seguimiento de progreso
...
- Descripción
- Campos obligatorios con restricciones
...
Cuando es `true`, indica que el valor `default` acompañante es aplicado por el servidor API de F5 XC. Los campos con esta extensión pueden omitirse de forma segura en las solicitudes API; el servidor aplica el valor predeterminado automáticamente.
...
Lista los nombres de campos que son mutuamente excluyentes con el campo actual, indicando patrones OneOf.
...
Indica requisitos específicos del contexto usando indicadores: `minimum_config`, `create`, `update`, `read`.
...
| Archivo | Contenido |
|------|---------|
| `docs/specifications/api/virtual.json` | Todos los esquemas de http_loadbalancer con enriquecimientos |
...
- [Mejoras de Healthcheck](./healthcheck-enhancements/) - Enriquecimientos del esquema de verificación de salud (incluye `x-f5xc-recommended-oneof-variant`)
- [Mejoras de Origin Pool](./originpool-enhancements/) - Enriquecimientos del esquema de pool de origen
...
|---------|------|---------|
| 2.0.46 | 2026-04-18 | Se agregaron valores predeterminados aplicados por el servidor, integración de flujos de trabajo guiados y sección ampliada de configuración mínima |
