...
Los campos marcados con `x-f5xc-server-default: true` tienen su valor `default` aplicado por el servidor API de F5 XC cuando se omiten en las solicitudes. El balanceador de carga HTTP tiene una gran cantidad de valores predeterminados aplicados por el servidor porque la mayoría de las funciones de seguridad y opciones de protocolo tienen por defecto un estado deshabilitado o seguro.
...
| `default_header` | `{}` | object | Manejo predeterminado del encabezado de nombre del servidor |
...
Todas las funciones de seguridad están deshabilitadas por defecto cuando se omiten:
...
| Campo | Valor Predeterminado | Tipo | Descripción |
|-------|---------------|------|-------------|
| `advertise_on_public_default_vip` | `{}` | object | Anunciar en VIP público predeterminado |
...
| Nombre del Grupo | Campos | Descripción |
|-----------|--------|-------------|
| `http_protocol` | `http_protocol_enable_v1_only`, `http_protocol_enable_v1_v2`, `http_protocol_enable_v2_only` | Versiones de protocolo HTTP |
...
| `server_name_header` | `default_header`, `append_server_name_header`, `pass_through_server_name_header` | Manejo del encabezado de nombre del servidor |
...
| `spec.https_auto_cert.http_protocol_options` | `http_protocol_enable_v1_only`, `http_protocol_enable_v1_v2`, `http_protocol_enable_v2_only` | `http_protocol_enable_v1_v2` | Versiones de protocolo HTTP |
...
El pipeline de enriquecimiento agrega metadatos `x-f5xc-guided-workflows` a nivel de especificación. Estos flujos de trabajo proporcionan orientación paso a paso para el despliegue, consumida por asistentes de IA y herramientas CLI.
...
| 2 | Configurar Health Check | `healthcheck` | `name`, `http_health_check` | Sí |
| 3 | Adjuntar Health Check | - | - | Sí |
...
| 3 | Configurar Health Check | `healthcheck` | - | Sí |
...
- **xcsh CLI**: Presenta flujos de trabajo como comandos guiados interactivos (por ejemplo, `xcsh deploy http-lb`)
- **Extensión de VS Code**: Muestra los pasos del flujo de trabajo en un panel lateral con seguimiento de progreso
...
Cuando es `true`, indica que el valor `default` acompañante es aplicado por el servidor API de F5 XC. Los campos con esta extensión pueden omitirse de forma segura en las solicitudes de API — el servidor aplica el valor predeterminado automáticamente.
...
|---------|------|---------|
| 2.0.46 | 2026-04-18 | Agregados valores predeterminados aplicados por el servidor, integración de flujos de trabajo guiados y sección ampliada de configuración mínima |
