...
**Versión**: 3.3.0
**Estado**: Producción
**Consumidores**: Asistentes de IA, herramientas CLI, proveedores Terraform, extensiones IDE, validadores de API
...
- **Asistentes de IA**: Generar configuraciones válidas sin intervención del usuario para detalles de restricciones
...
- **Extensiones IDE**: Validación en tiempo real y sugerencias de autocompletado
...
| *(claves de restricción)* | varios | Claves de restricción específicas del tipo aplanadas en el nivel superior (por ejemplo, `minLength`, `maxItems`, `minimum`) |
...
**URLs** (`format: "url"`):
- Las URLs deben ser URIs válidos según RFC 3986
...
2. **DESCUBRIMIENTO** (Confianza: 0.99)
   ...
   - Anula las restricciones inferidas

3. **INFERIDO** (Confianza: 0.50-0.95)
   - Coincidencia de patrones a partir de nombres de campos
   - Prioridad más baja, anulado por descubrimiento o explícito
...
### Integración con Proveedor Terraform
...
### Integración con Extensiones IDE
