...
description: >-
  Metadatos completos de restricciones para generación determinista por IA y
  validación
sidebar:
  order: 2
i18n:
...
**Propósito**: Metadatos completos de restricciones para generación determinista por IA, validación CLI y aplicación de esquemas Terraform.
...
7. [Integración con descubrimiento](#integración-con-descubrimiento)
...
La extensión `x-f5xc-constraints` proporciona conocimiento determinista sobre las restricciones de validación de campos, permitiendo:

- **Asistentes de IA**: Generar configuraciones válidas sin solicitar al usuario detalles de restricciones
...
| `category` | string | Categoría de restricción para agrupamiento e informes |
...
| `metadata` | object | Seguimiento de origen, confianza, marca temporal de validación |
...
| `date-time` | Marca temporal ISO 8601 | `2026-01-19T12:00:00Z` |
...
- Los nombres de etiqueta DNS deben tener de 1 a 63 caracteres alfanuméricos en minúsculas
...
**Marcas temporales** (`format: "date-time"`):
- Las marcas temporales deben estar en formato ISO 8601
...
- ❌ `my_service` (no se permite guion bajo)
...
| < 0.50 | **Muy baja** | No incluido en restricciones |
...
2. **DESCUBRIMIENTO** (Confianza: 0.99)
   ...
   - Sobrescribe restricciones inferidas

3. **INFERIDO** (Confianza: 0.50-0.95)
   - Coincidencia de patrones a partir de nombres de campos
   - Prioridad más baja, sobrescrito por descubrimiento o explícito
...
### Integración con proveedores de Terraform
...
| Patrón | Regex del nombre de campo | Tipo de restricción | Ejemplo |
|--------|---------------------------|---------------------|---------|
...
| Marca temporal | `\b(timestamp\|created_at\|updated_at)$` | String (ISO 8601) | `2026-01-19T12:00:00Z` |
...
- **Incidencias**: Reporte problemas de precisión en restricciones en https://github.com/f5xc-salesdemos/api-specs-enriched/issues
