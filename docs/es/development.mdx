...
| `docs/specifications/api/` | Specs de dominio generados | Ignorado por Git |
...
2. La verificación de ETag determina si los specs de F5 cambiaron
3. El pipeline procesa y enriquece los specs
...
| Nuevo spec de dominio añadido | Menor | 1.0.0 → 1.1.0 |
...
- **Valores enum reales**: Valores reales vistos en producción
- **Comportamientos por defecto**: Valores aplicados por el servidor cuando se omiten campos
...
El descubrimiento añade extensiones `x-discovered-*` a los specs:
...
|-----------|---------|-------------|
| `x-f5xc-server-default` | El servidor de la API F5 XC aplica este valor cuando el campo se omite | El campo es opcional; omitirlo produce el comportamiento por defecto documentado |
| `x-f5xc-recommended-value` | La consola web de F5 XC pre-rellena este valor para nuevos recursos | El campo no tiene valor por defecto del servidor pero este valor representa la configuración típica |
...
| `x-f5xc-required-for` | Pipeline de enriquecimiento | Estado requerido según contexto |
...
| `x-f5xc-recommended-value` | Valor pre-rellenado en la consola de F5 XC |
...
Cuando es `true`, indica que el valor `default` acompañante es aplicado por el servidor de la API F5 XC. Los campos con esta extensión pueden omitirse de forma segura en las solicitudes a la API — el servidor aplica el valor por defecto automáticamente.
...
Especifica un valor que la consola web de F5 XC utiliza como valor por defecto pre-rellenado. Este valor no es aplicado por el servidor pero representa la configuración inicial típica para nuevos recursos creados a través de la consola.
...
**Fuente**: Se deriva automáticamente de las extensiones nativas de F5 `x-ves-oneof-field-*` durante el enriquecimiento del pipeline.

### Implicaciones de Reglas de Validación
...
Algunos recursos aceptan specs vacíos porque el servidor aplica valores por defecto:
...
### Añadir Valores por Defecto Descubiertos

1. Crear recurso en F5 XC con spec mínimo vía API
2. Leer de vuelta el recurso para ver los valores aplicados por el servidor
...
### Errores de Linting en Specs Generados
...
La versión se deriva de las etiquetas de git (por ejemplo, `v2.0.38`), eliminando las condiciones de carrera que causaban conflictos de merge en PRs concurrentes.
...
Los specs generados están ignorados por git:
...
| Variable | Propósito |
|----------|---------|
| `F5XC_API_URL` | URL de la API del tenant F5 XC |
...
| `CHANGELOG.md` | Registro de cambios auto-generado |
