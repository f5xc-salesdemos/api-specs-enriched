...
`scripts/utils/extension_constants.py` se aplica mediante
...
- **Inyectadas aquí** — extensiones que nuestros enriquecedores añaden (`x-f5xc-*` y
  ...
  descubrimiento). Estas son las que las herramientas posteriores deben consumir.
- **Paso directo del origen** — extensiones que F5 emite en las especificaciones
...
- **Inyección futura** — aún no emitidas; se documentan aquí en el momento en que
  un enriquecedor comienza a producirlas (no aplicable en la población inicial).
...
breve siempre que el encabezado `### x-name` exista y la marca
`Pass-through from upstream:` esté presente con valor `yes` o `no`.
...
- **Applied at:** info
- **Purpose:** Bloque de metadatos globales del CLI (nombre de la herramienta, indicaciones de versión, agrupación por dominio).
- **Consumers:** CLI
- **Value type:** object
...
- **Applied at:** info
- **Purpose:** Marca de tiempo de la especificación fuente del origen a partir de la cual se construyó el archivo enriquecido.
- **Consumers:** multiple
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** ETag del activo de lanzamiento de la especificación fuente del origen.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Marca de tiempo de cuándo se ejecutó el pase de descubrimiento de la API en vivo.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string", "format": "date-time"}`
...
- **Applied at:** info
- **Purpose:** Guía curada de mejores prácticas para un dominio.
- **Consumers:** multiple
- **Value type:** array
...
- **Applied at:** schema
- **Purpose:** Conjunto mínimo viable de campos requeridos para realizar exitosamente un POST/PUT de este recurso.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** info
- **Purpose:** Proporciona metadatos de restricción, recomendación y clasificación de namespace para un recurso.
- **Consumers:** multiple
- **Value type:** object
...
- **Applied at:** schema
- **Purpose:** Nombre del tipo de recurso Terraform que corresponde a este esquema.
- **Consumers:** Terraform
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema
- **Purpose:** Nombre de visualización legible para un esquema de recurso (anula la generación automática).
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Purpose:** Descripción enriquecida de la propiedad que complementa la `description` del origen.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Reglas de validación declarativas derivadas de las `ves.io.schema.rules` del protobuf del origen.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** schema property
- **Purpose:** Lista las combinaciones de características nombradas que requieren esta propiedad.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/utils/minimum_configuration_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Aviso de deprecación con guía de reemplazo.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Valor recomendado para producción en un campo donde el valor predeterminado del servidor es subóptimo.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{}`
- **Injected by:** scripts/utils/default_value_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Lista las propiedades hermanas que no se pueden establecer junto con esta.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
...
- **Applied at:** schema property
- **Purpose:** Restricciones numéricas / de cadena derivadas del sondeo de la API en vivo o patrones estáticos.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Purpose:** Nombra los campos del cuerpo de la operación que deben proporcionarse para el éxito.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
...
- **Applied at:** operation
- **Purpose:** Clasifica el radio de impacto de una operación (low/medium/high/critical).
- **Consumers:** multiple
- **Value type:** string
...
- **Applied at:** operation
- **Purpose:** Si el CLI/UI debe solicitar confirmación al usuario antes de ejecutar.
- **Consumers:** multiple
- **Value type:** boolean
...
- **Applied at:** operation
- **Purpose:** Encabezados/comportamiento de límite de tasa observados de la API en vivo.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** operation
- **Purpose:** Catálogo de respuestas de error observadas durante el descubrimiento en vivo, con cargas de ejemplo.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** info
- **Purpose:** Categoría de agrupación de nivel superior para CLI / UI / documentación / Terraform para un dominio.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Recursos que requieren cuidado elevado (críticos para producción).
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Descripción breve (~60 caracteres) del dominio. También se aplica a nivel de propiedad para descripciones largas.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
...
- **Applied at:** info
- **Purpose:** Descripción media (~150 caracteres) del dominio. También se aplica a nivel de propiedad.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/property_description_short_enricher.py
...
- **Applied at:** info
- **Purpose:** Casos de uso nombrados que este dominio soporta.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Identificador de icono a utilizar al renderizar este dominio en una interfaz de usuario.
- **Consumers:** Web UI
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Enlaces cruzados a otros dominios comúnmente utilizados junto con este.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/merge_specs.py
...
## Paso directo del origen

### x-ves-proto-package

- **Applied at:** upstream
- **Purpose:** Preservado sin cambios de la especificación del origen F5.
- **Consumers:** N/A
- **Value type:** varies
- **Value schema:** N/A
...
- **Injected by:** upstream
- **Driven by config:** upstream
- **Example:** Consulte la documentación del origen F5.
- **Pass-through from upstream:** yes
