...
description: Estructura y contenido de la especificación de validación
sidebar:
  order: 3
i18n:
...
Este documento describe la estructura y el contenido de la especificación de validación publicada por el proyecto api-specs-enriched.

## Descripción general

La especificación de validación consolida las restricciones de validación de API, los valores predeterminados y los metadatos de configuración en un único archivo JSON. Esta especificación se genera a partir de las especificaciones OpenAPI enriquecidas y se publica junto a ellas.
...
Define los valores permitidos para los campos restringidos.
...
| `recommended` | Valores precompletados en la consola web de F5 XC | Análisis de la interfaz de usuario |
| `advanced_options` | Valores predeterminados anidados dentro de objetos advanced_options | Descubrimiento de API |
| `oneof_choices` | Selecciones de variantes OneOf predeterminadas | Comportamiento de la API |
...
|-----------|---------|----------|
| `x-f5xc-required-for` | Campos requeridos según el contexto | Propiedades de esquema |
| `x-f5xc-server-default` | Marca los valores predeterminados aplicados por el servidor | Propiedades de esquema |
| `x-f5xc-recommended-value` | Valor predeterminado recomendado | Propiedades de esquema |
| `x-f5xc-recommended-oneof-variant` | Variante OneOf recomendada | Definiciones de esquema |
| `x-f5xc-conditions` | Requisitos condicionales | Propiedades de esquema |
| `x-f5xc-minimum-configuration` | Ejemplos de configuración mínima | Definiciones de esquema |
| `x-f5xc-validation` | Restricciones derivadas del descubrimiento | Propiedades de esquema |
...
|-------------|--------------|---------|
| Cambios estructurales que rompen compatibilidad | Mayor | Renombrado de campos, secciones eliminadas |
| Nuevas reglas de validación o recursos | Menor | Nuevos valores predeterminados de recursos |
| Correcciones de errores o actualizaciones de descripción | Parche | Corrección de erratas |

## Prioridad de reconciliación

Cuando múltiples fuentes proporcionan restricciones, la prioridad de reconciliación es:

1. **Existing** - Restricciones en la especificación OpenAPI original (máxima prioridad)
2. **Discovery** - Restricciones provenientes del descubrimiento de API en vivo
3. **Inferred** - Restricciones derivadas de coincidencia de patrones (mínima prioridad)

## Documentación relacionada

- [Healthcheck Enhancements](./Enhancements/healthcheck-enhancements/) - Enriquecimientos del esquema de healthcheck
- [Origin Pool Enhancements](./Enhancements/originpool-enhancements/) - Enriquecimientos del esquema de origin pool
- [CLAUDE.md](../../CLAUDE.md) - Instrucciones para el asistente de IA
- [DEVELOPMENT.md](./development/) - Guía para desarrolladores

## Registro de cambios

| Versión | Fecha | Cambios |
|---------|------|---------|
| 2.1.2 | 2026-01-18 | Reescrito como referencia de API pura; añadidas las categorías oneof_recommended y nested_recommended |
