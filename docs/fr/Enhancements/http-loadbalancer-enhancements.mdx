---
title: Améliorations du HTTP LoadBalancer
description: Métadonnées d'enrichissement pour les schémas du répartiteur de charge HTTP
sidebar:
  order: 2
i18n:
...
Métadonnées d'enrichissement pour les schémas du répartiteur de charge HTTP. Voir [Extensions OpenAPI](../development/#openapi-extensions) pour les définitions des extensions.
...
L'extension `x-f5xc-minimum-configuration` fournit des métadonnées CLI complètes pour créer des répartiteurs de charge HTTP minimaux viables.

### Champs requis

| Champ | Contrainte | Description |
|-------|-----------|-------------|
| `metadata.name` | Format d'étiquette DNS : `[a-z0-9]([-a-z0-9]*[a-z0-9])?` | Nom de la ressource |
| `metadata.namespace` | Format d'étiquette DNS | Espace de noms |
| `spec.domains` | Tableau, min_items : 1 | Domaines à servir |

### Type de répartiteur de charge (OneOf requis)

L'un des types de répartiteur de charge suivants doit être spécifié :
...
**Remarque** : Le groupe OneOf du type de répartiteur de charge est désigné comme :
...
| `tls_config.default_security` | `{}` | object | TLS 1.2+ avec chiffrements robustes |
...
| `default_loadbalancer` | `{}` | object | Paramètres par défaut du répartiteur de charge |
...
Toutes les fonctionnalités de sécurité sont par défaut désactivées lorsqu'elles sont omises :

| Champ | Valeur par défaut | Description |
|-------|---------------|-------------|
| `disable_waf` | `{}` | Pare-feu d'application Web désactivé |
...
| Champ | Valeur par défaut | Type | Description |
|-------|---------------|------|-------------|
| `advertise_on_public_default_vip` | `{}` | object | Annoncer sur le VIP public par défaut |
| `round_robin` | `{}` | object | Algorithme de répartition de charge round-robin |
| `add_location` | `true` | boolean | Ajouter l'en-tête de localisation aux réponses |
| `system_default_timeouts` | `{}` | object | Utiliser les délais d'expiration par défaut du système |
...
|-----------|--------|-------------|
| `lb_type` | `http`, `https`, `https_auto_cert`, `http_https` | Type de protocole du répartiteur de charge |
| `advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | Comment annoncer le répartiteur de charge |
...
| `loadbalancer_choice` | `non_default_loadbalancer`, `default_loadbalancer` | Répartiteur de charge par défaut ou non |
...
| Nom du groupe | Champs | Description |
|-----------|--------|-------------|
| `waf` | `disable_waf`, `enable_waf` | Pare-feu d'application Web |
...
| `timeouts` | `system_default_timeouts`, `custom_timeouts` | Utiliser les délais par défaut du système ou personnalisés |
...
Contrairement à healthcheck, qui inclut `x-f5xc-recommended-oneof-variant` pour indiquer le choix le plus courant pour les groupes OneOf, http_loadbalancer ne dispose pas encore de cette extension.
...
| `spec.load_balancing_algorithm` | `round_robin`, `least_request`, `ring_hash`, `random` | `round_robin` | Algorithme de répartition de charge |
...
| `spec.advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | `advertise_on_public_default_vip` | Annonce du répartiteur de charge |

## Intégration des flux de travail guidés

Le pipeline d'enrichissement ajoute les métadonnées `x-f5xc-guided-workflows` au niveau de la spécification. Ces flux de travail fournissent des conseils de déploiement étape par étape consommés par les assistants IA et les outils CLI.

### Déployer un répartiteur de charge HTTP

Un flux de travail en 5 étapes pour créer un répartiteur de charge HTTP entièrement configuré avec un pool d'origine backend (défini dans `config/guided_workflows.yaml`) :

| Étape | Action | Ressource | Champs requis | Optionnel |
|------|--------|----------|-----------------|----------|
| 1 | Créer un pool d'origine | `origin_pool` | `name`, `origin_servers`, `port` | Non |
| 2 | Configurer la vérification de santé | `healthcheck` | `name`, `http_health_check` | Oui |
| 3 | Attacher la vérification de santé | - | - | Oui |
| 4 | Créer le répartiteur de charge HTTP | `http_loadbalancer` | `name`, `domains`, `http.port` | Non |
...
### Déployer un répartiteur de charge HTTPS avec TLS

Un flux de travail en 7 étapes pour créer un répartiteur de charge HTTPS avec terminaison SSL/TLS :

| Étape | Action | Ressource | Champs requis | Optionnel |
|------|--------|----------|-----------------|----------|
| 1 | Télécharger le certificat SSL | `certificate` | `name`, `certificate_chain`, `private_key` | Non |
| 2 | Créer un pool d'origine | `origin_pool` | `name`, `origin_servers` | Non |
| 3 | Configurer la vérification de santé | `healthcheck` | - | Oui |
| 4 | Configurer la politique WAF | `app_firewall` | - | Oui |
| 5 | Créer le répartiteur de charge HTTPS | `http_loadbalancer` | `name`, `domains`, `https.tls_parameters` | Non |
...
### Consommation des flux de travail

Les outils en aval utilisent ces flux de travail pour guider les utilisateurs à travers des déploiements multi-étapes :

- **CLI xcsh** : Présente les flux de travail comme des commandes guidées interactives (par ex., `xcsh deploy http-lb`)
- **Extension VS Code** : Affiche les étapes du flux de travail dans un panneau latéral avec suivi de la progression
- **Assistants IA** : Suivent les étapes du flux de travail séquentiellement, créant chaque ressource et validant les dépendances

## Référence des extensions OpenAPI

Ces extensions fournisseur sont ajoutées au schéma OpenAPI standard pour transmettre des métadonnées spécifiques à F5 XC.

### x-f5xc-cli-domain

**Type** : `string`

Indique la classification de domaine pour l'organisation CLI et des outils.
...
Lorsque `true`, indique que la valeur `default` accompagnante est appliquée par le serveur API F5 XC. Les champs avec cette extension peuvent être omis en toute sécurité des requêtes API — le serveur applique automatiquement la valeur par défaut.
...
Liste les noms de champs mutuellement exclusifs avec le champ actuel, indiquant des modèles OneOf.
...
Fournit des contraintes de validation incluant les limites de taille des tableaux, les plages d'entiers, les exigences d'unicité et les métadonnées de découverte.
...
Fournit des exemples concrets de champs pour la documentation et les outils.
...
Indique des exigences spécifiques au contexte à l'aide de drapeaux : `minimum_config`, `create`, `update`, `read`.
...
| `config/guided_workflows.yaml` | Définitions des étapes des flux de travail guidés |
...
- [Améliorations du Healthcheck](./healthcheck-enhancements/) - Enrichissements du schéma healthcheck (inclut `x-f5xc-recommended-oneof-variant`)
- [Améliorations du Origin Pool](./originpool-enhancements/) - Enrichissements du schéma origin pool
- [Guide des métadonnées de contraintes](../constraint-metadata/) - Documentation détaillée de la validation des contraintes

## Journal des modifications

| Version | Date | Modifications |
|---------|------|---------|
| 2.0.46 | 2026-04-18 | Ajout des valeurs par défaut appliquées par le serveur, intégration des flux de travail guidés et section de configuration minimale étendue |
