...
L'extension `x-f5xc-minimum-configuration` fournit des métadonnées CLI complètes pour la création de répartiteurs de charge HTTP minimaux viables.
...
| `header_transformation_type.legacy_header_transformation` | `{}` | object | Transformation d'en-têtes héritée |
...
| `advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | Comment publier le répartiteur de charge |
...
| `spec.advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | `advertise_on_public_default_vip` | Publication du répartiteur de charge |

## Intégration des workflows guidés

Le pipeline d'enrichissement ajoute les métadonnées `x-f5xc-guided-workflows` au niveau de la spécification. Ces workflows fournissent des conseils de déploiement étape par étape consommés par les assistants IA et les outils CLI.

### Déployer un répartiteur de charge HTTP

Un workflow en 5 étapes pour créer un répartiteur de charge HTTP entièrement configuré avec un pool d'origine backend (défini dans `config/guided_workflows.yaml`) :
...
Un workflow en 7 étapes pour créer un répartiteur de charge HTTPS avec terminaison SSL/TLS :
...
Les outils en aval utilisent ces workflows pour guider les utilisateurs à travers les déploiements en plusieurs étapes :

- **xcsh CLI** : Présente les workflows sous forme de commandes guidées interactives (par ex., `xcsh deploy http-lb`)
...
Indique la classification de domaine pour l'organisation CLI et l'outillage.
...
Lorsque `true`, indique que la valeur `default` associée est appliquée par le serveur API F5 XC. Les champs avec cette extension peuvent être omis en toute sécurité des requêtes API — le serveur applique automatiquement la valeur par défaut.
...
- `x-f5xc-description-medium` : 150 caractères maximum (info-bulles, résumés)
...
Indique les exigences spécifiques au contexte à l'aide de drapeaux : `minimum_config`, `create`, `update`, `read`.
...
| `config/guided_workflows.yaml` | Définitions des étapes des workflows guidés |
| `config/domain_descriptions.yaml` | Métadonnées de description au niveau du domaine |

### Schémas enrichis

- `viewshttp_loadbalancerCreateSpecType` - Schéma d'opération de création
- `viewshttp_loadbalancerReplaceSpecType` - Schéma d'opération de mise à jour
- `viewshttp_loadbalancerGetSpecType` - Schéma d'opération de lecture

## Documentation associée

- [Guide de développement - Extensions OpenAPI](../development/#openapi-extensions) - Définitions et utilisation des extensions
- [Améliorations du Healthcheck](./healthcheck-enhancements/) - Enrichissements des schémas de contrôle de santé (inclut `x-f5xc-recommended-oneof-variant`)
- [Améliorations de l'Origin Pool](./originpool-enhancements/) - Enrichissements des schémas de pool d'origine
