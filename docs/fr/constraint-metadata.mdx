...
description: >-
  Métadonnées de contraintes complètes pour la génération et la validation
  déterministes par IA
sidebar:
  order: 2
i18n:
...
**Objectif** : Métadonnées de contraintes complètes pour la génération déterministe par IA, la validation CLI et l'application du schéma Terraform.
...
7. [Intégration Discovery](#discovery-integration)
...
L'extension `x-f5xc-constraints` fournit une connaissance déterministe des contraintes de validation de champs, permettant :
...
- **Terraform** : Appliquer les contraintes au moment de la planification, et non lors de l'application
...
- Doivent inclure un schéma (http/https)
...
**Délais d'expiration** :
- Les délais d'expiration doivent être compris entre 1 et 3600 secondes (maximum 1 heure)

### Contraintes de tableau

**Pools d'origine** :
- Le tableau des pools d'origine requiert au moins 1 élément, maximum 50 éléments
- Chaque élément doit être unique

**Étiquettes** :
- Le tableau d'étiquettes autorise entre 0 et 100 éléments
...
| 0,70-0,89 | **Confiance moyenne** | Indicatif, recommander la confirmation de l'utilisateur |
| 0,50-0,69 | **Faible confiance** | Informatif uniquement, nécessite la validation de l'utilisateur |
...
## Intégration Discovery

### Priorité des sources

Les contraintes sont réconciliées selon une priorité à 3 niveaux :

1. **EXISTING** (Confiance : 1,0)
   - Défini explicitement dans la spécification OpenAPI d'origine
   - Jamais remplacé

2. **DISCOVERY** (Confiance : 0,99)
   - Extrait des réponses de l'API F5 XC en direct
   - Inclut les règles de validation issues de x-ves-validation-rules
   - Remplace les contraintes inférées

3. **INFERRED** (Confiance : 0,50-0,95)
   - Correspondance par motif à partir des noms de champs
   - Priorité la plus basse, remplacé par discovery ou explicite

### Sources de données Discovery
...
2. **Catalogue d'erreurs** : Messages d'erreur d'API indiquant des violations de contraintes
   - « le nom doit comporter entre 1 et 63 caractères » → minLength/maxLength
   - « le port doit être compris entre 1 et 65535 » → minimum/maximum
...
**Fournir une validation des saisies** :
```python
...
- **Problèmes** : Signaler les problèmes de précision des contraintes à l'adresse https://github.com/f5xc-salesdemos/api-specs-enriched/issues
- **Documentation** : Documentation complète de l'API disponible à l'adresse https://f5xc-salesdemos.github.io/api-specs-enriched/
- **Intégration MCP** : Consulter le dépôt `f5xc-api-mcp` pour des exemples d'intégration de serveur MCP
