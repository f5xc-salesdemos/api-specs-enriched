...
2. [Structure de l'extension](#structure-de-lextension)
3. [Types de contraintes](#types-de-contraintes)
4. [Descriptions en langage naturel](#descriptions-en-langage-naturel)
5. [Exemples de validation](#exemples-de-validation)
6. [Scores de confiance](#scores-de-confiance)
7. [Intégration de la découverte](#intégration-de-la-découverte)
8. [Guide d'intégration des outils](#guide-dintégration-des-outils)
...
- **Assistants IA** : Générer des configurations valides sans intervention de l'utilisateur pour les détails des contraintes
- **Outils CLI** : Fournir des indications de saisie précises, une validation et des messages d'erreur
- **Terraform** : Appliquer les contraintes au moment de la planification, et non de l'exécution
...
- Les noms d'étiquettes DNS doivent comporter 1 à 63 caractères alphanumériques minuscules
- Les tirets sont autorisés mais pas en début/fin (RFC 1123)
...
**FQDN** (`format: "fqdn"`) :
- Les noms de domaine pleinement qualifiés doivent comporter 1 à 253 caractères
...
**Numéros de port** :
- Les numéros de port doivent être des entiers compris entre 1 et 65535
...
- ❌ `-my-service` (tiret en début)
- ❌ `my-service-` (tiret en fin)
...
| 0.70-0.89 | **Confiance moyenne** | Consultatif, confirmation de l'utilisateur recommandée |
| 0.50-0.69 | **Faible confiance** | Informatif uniquement, validation de l'utilisateur requise |
...
2. **Catalogue d'erreurs** : Messages d'erreur d'API indiquant des violations de contraintes
   - "name must be 1-63 characters" → minLength/maxLength
   - "port must be between 1 and 65535" → minimum/maximum

3. **Validation des réponses** : Réponses d'API réussies démontrant des valeurs valides
...
### Intégration avec les outils CLI
...
### Intégration avec le fournisseur Terraform
...
### Intégration avec les extensions IDE
...
|-------|----------------------|---------------------|---------|
| Étiquette DNS | `\bname$` | String (1-63 caractères, dns-label) | `my-service` |
| Espace de noms | `\bnamespace$` | String (1-63 caractères, dns-label) | `production` |
| Port | `\bport$` | Number (1-65535) | `443` |
| Identifiant VLAN | `\b(vlan)?_?id$` | Number (1-4094) | `100` |
| UUID | `\buuid$` | String (36 caractères, format uuid) | `550e8400-...` |
| E-mail | `\bemail$` | String (max 254 caractères, email) | `user@example.com` |
...
| Motif | Regex du nom de champ | Type de contrainte | Exemple |
|-------|----------------------|---------------------|---------|
| Nom d'utilisateur | `\b(user\|username)$` | String (1-64 caractères, alphanumérique) | `admin` |
| Nom d'affichage | `\b(display_?)?name$` | String (1-256 caractères, texte libre) | `My Service` |
| Description | `\b(comment\|description\|note)$` | String (max 4096 caractères) | `Service description` |
| Chemin | `\bpath$` | String (max 2048 caractères) | `/api/v1/resources` |
