...
**Objectif** : Métadonnées de contraintes complètes pour la génération IA déterministe, la validation CLI et l'application des schémas Terraform.
...
- **Outils CLI** : Fournir des indications de saisie précises, la validation et des messages d'erreur
- **Terraform** : Appliquer les contraintes au moment de la planification, pas au moment de l'application
- **Extensions IDE** : Validation en temps réel et suggestions d'auto-complétion
- **Validateurs d'API** : Validation avant soumission pour réduire les erreurs API

### Statistiques de couverture

- **9 851 contraintes** sur toutes les spécifications API F5 XC
- **Plus de 50 types de motifs** (chaîne, tableau, numérique)
- **95 % de confiance** sur les champs hautement prioritaires (noms, ports, identifiants)
...
| `category` | string | Catégorie de contrainte pour le regroupement et le reporting |
...
- Les noms d'étiquettes DNS doivent comporter de 1 à 63 caractères alphanumériques minuscules
- Les traits d'union sont autorisés mais pas en début/fin (RFC 1123)
...
**FQDN** (`format: "fqdn"`) :
- Les noms de domaine pleinement qualifiés doivent comporter de 1 à 253 caractères
...
**UUID** (`format: "uuid"`) :
- Les UUID doivent respecter le format RFC 4122
...
**Étiquettes** :
- Le tableau d'étiquettes autorise de 0 à 100 éléments
...
- ❌ `-my-service` (trait d'union au début)
- ❌ `my-service-` (trait d'union à la fin)
...
|-------|----------------|-------------|
| 0.90-1.00 | **Confiance élevée** | Indicateur déterministe activé, utiliser pour la génération IA |
| 0.70-0.89 | **Confiance moyenne** | Consultatif, confirmation utilisateur recommandée |
| 0.50-0.69 | **Confiance faible** | Informatif uniquement, validation utilisateur requise |
...
- Le motif correspond à des standards bien établis (RFC, ISO, etc.)
...
2. **Catalogue d'erreurs** : Messages d'erreur API indiquant des violations de contraintes
...
## Annexe : Catalogue des motifs de contraintes

### Motifs à haute confiance (0.95-0.99)

| Motif | Regex du nom de champ | Type de contrainte | Exemple |
|-------|----------------------|---------------------|---------|
| Étiquette DNS | `\bname$` | String (1-63 car., dns-label) | `my-service` |
| Espace de noms | `\bnamespace$` | String (1-63 car., dns-label) | `production` |
| Port | `\bport$` | Number (1-65535) | `443` |
| Identifiant VLAN | `\b(vlan)?_?id$` | Number (1-4094) | `100` |
| UUID | `\buuid$` | String (36 car., format uuid) | `550e8400-...` |
| E-mail | `\bemail$` | String (max 254 car., email) | `user@example.com` |
| Horodatage | `\b(timestamp\|created_at\|updated_at)$` | String (ISO 8601) | `2026-01-19T12:00:00Z` |

### Motifs à confiance moyenne (0.80-0.94)

| Motif | Regex du nom de champ | Type de contrainte | Exemple |
|-------|----------------------|---------------------|---------|
| Nom d'utilisateur | `\b(user\|username)$` | String (1-64 car., alphanumérique) | `admin` |
| Nom d'affichage | `\b(display_?)?name$` | String (1-256 car., texte libre) | `My Service` |
| Description | `\b(comment\|description\|note)$` | String (max 4096 car.) | `Service description` |
| Chemin | `\bpath$` | String (max 2048 car.) | `/api/v1/resources` |
...
- **Intégration MCP** : Consultez le dépôt `f5xc-api-mcp` pour des exemples d'intégration du serveur MCP
