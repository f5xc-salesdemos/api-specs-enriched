...
description: >-
  Flux de travail de développement pour le pipeline d'enrichissement de l'API F5
  XC
sidebar:
  order: 1
i18n:
...
Ce guide décrit le flux de travail de développement pour le pipeline d'enrichissement de l'API F5 XC.
...
|-----------|---------|------------|
| `specs/original/` | Spécifications source F5 | Ignoré par Git |
...
- Les champs réellement requis/optionnels
- Les valeurs d'énumération à partir des réponses en production
...
6. La release GitHub est créée avec le journal des modifications
...
- **Contraintes non documentées** : Champs requis non marqués dans OpenAPI
- **Valeurs d'énumération réelles** : Valeurs réelles observées en production
- **Comportements par défaut** : Valeurs appliquées par le serveur lorsque les champs sont omis
- **Modèles de réponse** : Structures de données réelles
...
| `x-f5xc-recommended-oneof-variant` | La console F5 XC présélectionne ce variant OneOf | Identifie le choix typique lorsque plusieurs options mutuellement exclusives existent |
| `x-f5xc-conflicts-with` | Liste les autres propriétés qui ne peuvent pas être utilisées conjointement avec ce champ | La propriété fait partie d'un groupe OneOf ; seule une des propriétés en conflit peut être spécifiée |

### Extensions de champs requis
...
| `x-f5xc-recommended-oneof-variant` | Variant OneOf recommandé |
...
Lorsque `true`, indique que la valeur `default` associée est appliquée par le serveur de l'API F5 XC. Les champs avec cette extension peuvent être omis en toute sécurité des requêtes API — le serveur applique la valeur par défaut automatiquement.
...
Spécifie une valeur que la console web F5 XC utilise comme valeur pré-remplie par défaut. Cette valeur n'est pas appliquée par le serveur mais représente la configuration de départ typique pour les nouvelles ressources créées via la console.
...
**Type** : `object` (correspondance entre nom de groupe et nom de variant)

Pour les schémas avec des groupes de champs mutuellement exclusifs, identifie quel variant est le choix par défaut ou le plus courant. La clé est le nom du groupe OneOf et la valeur est le nom du champ du variant recommandé.
...
L'enrichisseur inspecte `x-ves-validation-rules` pour déduire le statut requis :

| Règle | Implication |
|------|-------------|
| `ves.io.schema.rules.message.required: "true"` | Le champ est requis |
| `ves.io.schema.rules.uint32.gte: N` | Si N >= 1 et pas de valeur par défaut serveur, le champ est requis |
...
### Erreurs de linting sur les spécifications générées
...
La version est dérivée des tags git (par ex., `v2.0.38`), éliminant les conditions de concurrence qui causaient des conflits de fusion sur les PR concurrentes.
