...
description: Flux de développement pour le pipeline d'enrichissement d'API F5 XC
sidebar:
  order: 1
i18n:
...
Ce guide décrit le flux de développement pour le pipeline d'enrichissement d'API F5 XC.
...
| `specs/discovered/` | Résultat de la découverte d'API | Suivi (openapi.json, session.json) |
...
- Les validations de motif
- Les exemples de réponse

### 2. Flux de publication

Les publications sont automatisées :

1. Le calendrier quotidien (6h00 UTC) ou une poussée vers la branche principale déclenche le flux de travail
2. La vérification ETag détermine si les spécifications F5 ont changé
3. Le pipeline traite et enrichit les spécifications
4. La version est calculée à partir des balises git : `git describe --tags --abbrev=0`
5. Un commit direct + une balise sont créés (sans PR d'incrémentation de version)
6. Une publication GitHub est créée avec le journal des modifications
...
|-----------|-----------|---------|
| `[major]` dans le commit | Majeur | 1.0.0 → 2.0.0 |
| `BREAKING CHANGE` dans le commit | Majeur | 1.0.0 → 2.0.0 |
| Nouvelle spécification de domaine ajoutée | Mineur | 1.0.0 → 1.1.0 |
...
- **Les contraintes non documentées** : Les champs requis non marqués dans OpenAPI
- **Les valeurs d'énumération réelles** : Les vraies valeurs observées en production
- **Les comportements par défaut** : Les valeurs appliquées par le serveur lorsque des champs sont omis
- **Les motifs de réponse** : Les formes réelles des données
...
### Contenu du paquet de publication
...
### Déclenchement manuel du flux de travail
...
| `x-f5xc-recommended-value` | La console web F5 XC pré-remplit cette valeur pour les nouvelles ressources | Le champ n'a pas de valeur par défaut serveur, mais cette valeur représente une configuration typique |
...
| `x-f5xc-conflicts-with` | Liste les autres propriétés ne pouvant pas être utilisées conjointement avec ce champ | La propriété fait partie d'un groupe OneOf ; une seule des propriétés conflictuelles peut être spécifiée |
...
### Extensions de valeur par défaut et OneOf
...
Pour les schémas avec des groupes de champs mutuellement exclusifs, identifie quelle variante est le choix par défaut ou le plus courant. La clé est le nom du groupe OneOf et la valeur est le nom du champ de variante recommandé.
...
- Les outils CLI peuvent avertir des combinaisons de champs conflictuels
- Les assistants IA peuvent générer des configurations OneOf correctes
- Les extensions IDE peuvent proposer une autocomplétion tenant compte des conflits
...
| `ves.io.schema.rules.uint32.gte: N` | Si N >= 1 et qu'il n'y a pas de valeur par défaut serveur, le champ est requis |
...
Corriger les problèmes dans la configuration d'enrichissement/normalisation, et non dans les fichiers de sortie.

### Système de versionnage

La version est dérivée des balises git (par exemple, `v2.0.38`), éliminant les conditions de concurrence qui causaient des conflits de fusion sur les PR concurrentes.
...
Ajouter une exclusion dans `.pre-commit-config.yaml` :
...
| `make rebuild` | Reconstruction rapide (ignorer le téléchargement) |
...
| `config/spectral.yaml` | Règles de lint |
| `scripts/utils/version_calculator.py` | Calcul de version basé sur les balises |

**Remarque** : La version est dérivée des balises git (par exemple, `v2.0.38`), et non d'un fichier `.version`.
