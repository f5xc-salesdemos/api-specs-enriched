...
5. Commit direct + tag créé (pas de PR de montée de version)
6. GitHub Release créée avec le changelog
7. Documentation déployée sur GitHub Pages

**Règles de montée de version** :

| Condition | Type de montée | Exemple |
|-----------|-----------|---------|
...
Le pipeline d'enrichissement utilise des extensions propriétaires pour intégrer les métadonnées de validation et de valeurs par défaut.

### Catégories d'extensions

| Extension | Signification | Implication |
|-----------|---------|-------------|
| `x-f5xc-server-default` | Le serveur API F5 XC applique cette valeur lorsque le champ est omis | Le champ est optionnel ; l'omettre produit le comportement par défaut documenté |
| `x-f5xc-recommended-value` | La console web F5 XC pré-remplit cette valeur pour les nouvelles ressources | Le champ n'a pas de valeur par défaut côté serveur mais cette valeur représente une configuration typique |
...
| `x-f5xc-conflicts-with` | Exclusion mutuelle avec d'autres propriétés OneOf |

#### x-f5xc-server-default

**Type** : `boolean`

Lorsque `true`, indique que la valeur `default` associée est appliquée par le serveur API F5 XC. Les champs avec cette extension peuvent être omis en toute sécurité des requêtes API — le serveur applique la valeur par défaut automatiquement.
...
Déclare les relations d'exclusion mutuelle entre les membres d'un groupe OneOf. Dérivé automatiquement des extensions `x-ves-oneof-field-*`. Cela permet aux outils en aval (Terraform, CLI, MCP) de valider les conflits au niveau du schéma plutôt qu'à l'exécution.
...
- Les extensions IDE peuvent fournir une auto-complétion tenant compte des conflits
...
| `ves.io.schema.rules.uint32.gte: N` | Si N >= 1 et pas de valeur par défaut serveur, le champ est obligatoire |
...
| `healthcheck` | `jitter: 0`, `jitter_percent: 0`, valeurs par défaut imbriquées de http_health_check |
...
Le pipeline s'exécute à chaque commit pour assurer la cohérence. Pour les commits de travail en cours :
...
La version est dérivée des tags git (par exemple, `v2.0.38`), éliminant les conditions de concurrence qui causaient des conflits de fusion sur les PR concurrentes.
...
**Note** : La version est dérivée des tags git (par exemple, `v2.0.38`), et non d'un fichier `.version`.
