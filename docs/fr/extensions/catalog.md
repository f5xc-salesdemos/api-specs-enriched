...
- **Transmission amont** — extensions émises par F5 dans les spécifications
  sources que nous préservons sans modification (`x-ves-proto-*`, `x-displayname`, etc.).
...
- **Injection future** — pas encore émises ; documentées ici dès qu'un
  enrichisseur commence à les produire (non applicable lors du peuplement
  initial).
...
succinct tant que l'en-tête `### x-name` existe et que le drapeau
`Pass-through from upstream:` est présent avec la valeur `yes` ou `no`.
...
## Injectées — niveau spécification (section info)

### x-f5xc-cli-domain

- **Applied at:** info
- **Purpose:** Identifie le slug de domaine CLI (par ex. `http_loadbalancer`) pour une spécification enrichie.
- **Consumers:** CLI
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** Bloc de métadonnées globales CLI (nom de l'outil, indications de version, regroupement par domaine).
- **Consumers:** CLI
- **Value type:** object
...
- **Applied at:** info
- **Purpose:** Horodatage de la spécification source amont à partir de laquelle le fichier enrichi a été construit.
- **Consumers:** multiple
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** Version sémantique apposée sur la spécification enrichie par le pipeline.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Bloc de glossaire de marque/terminologie appliqué à chaque spécification de domaine.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** info
- **Purpose:** Horodatage du moment où la passe de découverte de l'API en direct a été exécutée.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string", "format": "date-time"}`
...
- **Applied at:** info
- **Purpose:** Recommandations de bonnes pratiques curatées pour un domaine.
- **Consumers:** multiple
- **Value type:** array
...
- **Applied at:** info
- **Purpose:** Flux de travail guidés étape par étape nommés pour accomplir des tâches courantes dans un domaine.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
...
- **Applied at:** schema
- **Purpose:** Ensemble minimal viable de champs requis pour réussir un POST/PUT de cette ressource.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** info
- **Purpose:** Fournit les métadonnées de contrainte, recommandation et classification d'espace de noms pour une ressource.
- **Consumers:** multiple
- **Value type:** object
...
- **Applied at:** schema
- **Purpose:** Nom du type de ressource Terraform qui correspond à ce schéma.
- **Consumers:** Terraform
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Valeur par défaut assignée par le serveur lorsque le client omet la propriété.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{}`
...
- **Applied at:** schema property
- **Purpose:** Documente les dépendances inter-champs où un champ requiert qu'un autre soit défini.
...
- **Applied at:** operation
- **Purpose:** Indique si le CLI/UI doit demander à l'utilisateur de confirmer avant l'exécution.
- **Consumers:** multiple
- **Value type:** boolean
...
- **Applied at:** operation
- **Purpose:** En-têtes / comportement de limitation de débit observés à partir de l'API en direct.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** operation
- **Purpose:** Catalogue des réponses d'erreur observées lors de la découverte en direct, avec des charges utiles d'exemple.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** info
- **Purpose:** Catégorie de regroupement de haut niveau CLI / UI / docs / Terraform pour un domaine.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Slug de section / regroupement de navigation de la documentation pour les documents rendus.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
