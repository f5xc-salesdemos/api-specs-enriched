---
title: Origin Pool संवर्द्धन
description: Origin pool स्कीमा के लिए एनरिचमेंट मेटाडेटा
sidebar:
  order: 3
i18n:
...
origin_pool-संबंधित स्कीमा के लिए एनरिचमेंट मेटाडेटा। एक्सटेंशन परिभाषाओं के लिए [OpenAPI Extensions](../development/#openapi-extensions) देखें।

## संवर्द्धित स्कीमा

### पैटर्न

`origin_pool.*SpecType` से मेल खाने वाली सभी स्कीमा को संवर्द्धन प्राप्त होता है:
...
`x-f5xc-server-default: true` के साथ चिह्नित फ़ील्ड की `default` वैल्यू F5 XC API सर्वर द्वारा तब लागू की जाती है जब अनुरोधों से छोड़ दी जाती है।
...
| `endpoint_selection` | `DISTRIBUTED` | enum | सभी एंडपॉइंट्स का उपयोग करें (लोकल + रिमोट) |

### advanced_options डिफ़ॉल्ट

जब `advanced_options` निर्दिष्ट नहीं किया जाता, तो सर्वर ऐसे व्यवहार करता है जैसे ये वैल्यू सेट की गई हों:
...
F5 XC वेब UI उन वैल्यू से भिन्न वैल्यू पूर्व-चयनित करता है जो API फ़ील्ड छोड़ने पर लागू करता है।
...
| `LB_OVERRIDE` | पैरेंट लोड बैलेंसर से हैश पॉलिसी इनहेरिट | UI डिफ़ॉल्ट |

### endpoint_selection

| वैल्यू | विवरण | नोट्स |
|-------|-------------|-------|
| `DISTRIBUTED` | रिमोट और लोकल दोनों एंडपॉइंट्स पर विचार करें | सर्वर डिफ़ॉल्ट |
| `LOCAL_ONLY` | केवल लोकल एंडपॉइंट्स का उपयोग | |
| `LOCAL_PREFERRED` | लोकल को प्राथमिकता, अनुपलब्ध होने पर रिमोट पर फ़ॉलबैक | |

## OneOf फ़ील्ड समूह

पारस्परिक रूप से अनन्य फ़ील्ड समूह। प्रत्येक समूह से केवल एक फ़ील्ड निर्दिष्ट की जा सकती है:
...
| 4 | लोडबैलेंसर एल्गोरिथम | `spec.loadbalancer_algorithm` | enum | `ROUND_ROBIN` |
...
| `docs/specifications/api/openapi.json` | सभी स्कीमा के साथ मर्ज्ड स्पेसिफिकेशन |
...
ऑरिजिन सर्वर विभिन्न प्रकार के वेरिएंट का उपयोग करके निर्दिष्ट किए जा सकते हैं। प्रकार यह निर्धारित करता है कि कौन से अतिरिक्त फ़ील्ड आवश्यक हैं।
...
1. **साइट संसाधन खोज** - `site` संसाधन स्कीमा और site_locator पैटर्न का दस्तावेज़ीकरण
2. **नेटवर्क चॉइस Enum** - `network_choice` प्रतिबंधित वैल्यू का दस्तावेज़ीकरण (site_local_inside, site_local_outside, आदि)
3. **SNAT पूल विकल्प** - SNAT पूल कॉन्फ़िगरेशन पैटर्न का दस्तावेज़ीकरण
4. **क्रॉस-संसाधन संदर्भ** - origin_pool साइट संसाधनों को कैसे संदर्भित करता है, इसे परिभाषित करें

### ट्रैकिंग

- कॉन्फ़िगरेशन प्लेसहोल्डर: `config/discovered_defaults.yaml` → `origin_pool.origin_server_types.enums`
- संबंधित स्प्रिंट: साइट संसाधन स्कीमा संवर्द्धन

## संबंधित दस्तावेज़ीकरण

- [विकास गाइड - OpenAPI Extensions](../development/#openapi-extensions) - एक्सटेंशन परिभाषाएँ और उपयोग
- [वैलिडेशन स्पेसिफिकेशन](../validation-spec/) - validation.json प्रारूप और संरचना
- [हेल्थचेक संवर्द्धन](./healthcheck-enhancements/) - हेल्थचेक स्कीमा संवर्द्धन
...
| 2.1.0 | 2026-01-18 | validation.json में एकीकृत डिफ़ॉल्ट संरचना में अपडेट किया |
