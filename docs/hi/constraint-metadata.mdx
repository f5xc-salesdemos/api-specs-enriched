...
description: नियतात्मक AI जेनरेशन और वैलिडेशन के लिए व्यापक कंस्ट्रेंट मेटाडेटा
sidebar:
  order: 2
i18n:
...
**उद्देश्य**: नियतात्मक AI जेनरेशन, CLI वैलिडेशन, और Terraform स्कीमा प्रवर्तन के लिए व्यापक कंस्ट्रेंट मेटाडेटा।

**एक्सटेंशन**: `x-f5xc-constraints`
**संस्करण**: 3.3.0
**स्थिति**: प्रोडक्शन
**उपभोक्ता**: AI सहायक, CLI उपकरण, Terraform प्रदाता, IDE एक्सटेंशन, API वैलिडेटर
...
5. [वैलिडेशन उदाहरण](#validation-examples)
6. [विश्वास स्कोर](#confidence-scores)
7. [डिस्कवरी एकीकरण](#discovery-integration)
8. [उपकरण एकीकरण गाइड](#tool-integration-guide)

---

## अवलोकन

`x-f5xc-constraints` एक्सटेंशन फ़ील्ड वैलिडेशन कंस्ट्रेंट के बारे में नियतात्मक ज्ञान प्रदान करता है, जो निम्नलिखित को सक्षम बनाता है:

- **AI सहायक**: कंस्ट्रेंट विवरण के लिए उपयोगकर्ता इनपुट के बिना वैध कॉन्फ़िगरेशन जेनरेट करें
- **CLI उपकरण**: सटीक इनपुट संकेत, वैलिडेशन, और त्रुटि संदेश प्रदान करें
- **Terraform**: अप्लाई समय पर नहीं, प्लान समय पर कंस्ट्रेंट लागू करें
- **IDE एक्सटेंशन**: रीयल-टाइम वैलिडेशन और ऑटोकंप्लीट सुझाव
- **API वैलिडेटर**: API त्रुटियों को कम करने के लिए सबमिशन-पूर्व वैलिडेशन

### कवरेज आँकड़े

- सभी F5 XC API स्पेक में **9,851 कंस्ट्रेंट**
- **50+ पैटर्न प्रकार** (स्ट्रिंग, ऐरे, न्यूमेरिक)
- उच्च-प्राथमिकता फ़ील्ड (नाम, पोर्ट, आइडेंटिफ़ायर) पर **95% विश्वास**
...
| `deterministic` | boolean | उच्च-विश्वास फ़्लैग (confidence >= 0.9) AI जेनरेशन के लिए |
| `category` | string | समूहन और रिपोर्टिंग के लिए कंस्ट्रेंट श्रेणी |
| *(कंस्ट्रेंट कीज़)* | विभिन्न | प्रकार-विशिष्ट कंस्ट्रेंट कीज़ शीर्ष स्तर पर फ़्लैट किए गए (जैसे, `minLength`, `maxItems`, `minimum`) |
| `metadata` | object | स्रोत ट्रैकिंग, विश्वास, वैलिडेशन टाइमस्टैम्प |
...
| `base64` | Base64-एनकोडेड स्ट्रिंग | `SGVsbG8gV29ybGQ=` |
...
### स्ट्रिंग फ़ॉर्मेट कंस्ट्रेंट

**DNS लेबल** (`format: "dns-label"`):
- DNS लेबल नाम 1-63 लोअरकेस अल्फ़ान्यूमेरिक अक्षरों के होने चाहिए
- हाइफ़न अनुमत हैं लेकिन शुरुआत/अंत में नहीं (RFC 1123)
...
**FQDN** (`format: "fqdn"`):
- पूर्ण योग्य डोमेन नाम 1-253 अक्षरों के होने चाहिए
- बिंदु-विभाजित DNS लेबल
...
**UUID** (`format: "uuid"`):
- UUID को RFC 4122 फ़ॉर्मेट का पालन करना चाहिए
...
**पोर्ट नंबर**:
- पोर्ट नंबर 1 और 65535 के बीच पूर्णांक होने चाहिए
- मानक पोर्ट: 80 (HTTP), 443 (HTTPS), 22 (SSH)

**VLAN ID**:
- VLAN ID 1 और 4094 के बीच पूर्णांक होने चाहिए (802.1Q मानक)

**टाइमआउट**:
- टाइमआउट 1-3600 सेकंड (अधिकतम 1 घंटा) होने चाहिए

### ऐरे कंस्ट्रेंट

**ऑरिजिन पूल**:
- ऑरिजिन पूल की ऐरे में कम से कम 1 आइटम, अधिकतम 50 आइटम आवश्यक हैं
- प्रत्येक आइटम अद्वितीय होना चाहिए

**टैग**:
- टैग ऐरे 0-100 आइटम की अनुमति देती है
...
### उदाहरण 1: DNS लेबल नाम वैलिडेशन
...
- ❌ `[51 आइटम वाली ऐरे]` (अधिकतम से अधिक)
...
| सीमा | व्याख्या | उपयोग |
|-------|----------------|-------|
| 0.90-1.00 | **उच्च विश्वास** | नियतात्मक फ़्लैग सक्षम, AI जेनरेशन के लिए उपयोग करें |
| 0.70-0.89 | **मध्यम विश्वास** | सलाहकार, उपयोगकर्ता पुष्टि की सिफ़ारिश |
| 0.50-0.69 | **कम विश्वास** | केवल सूचनात्मक, उपयोगकर्ता वैलिडेशन आवश्यक |
| < 0.50 | **बहुत कम** | कंस्ट्रेंट में शामिल नहीं |

### नियतात्मक फ़्लैग

`deterministic` बूलियन फ़्लैग तब सेट होता है जब:
- विश्वास स्कोर >= 0.9
- पैटर्न को डिस्कवरी डेटा के विरुद्ध मान्य किया गया हो या
- पैटर्न अच्छी तरह से स्थापित मानकों (RFC, ISO, आदि) से मेल खाता हो

**AI जेनरेशन में उपयोग**:
```python
...
कंस्ट्रेंट को 3-स्तरीय प्राथमिकता का उपयोग करके समाधान किया जाता है:

1. **EXISTING** (विश्वास: 1.0)
   - मूल OpenAPI स्पेक में स्पष्ट रूप से सेट किया गया
   - कभी ओवरराइड नहीं किया जाता

2. **DISCOVERY** (विश्वास: 0.99)
   - लाइव F5 XC API प्रतिक्रियाओं से निकाला गया
   - x-ves-validation-rules से वैलिडेशन नियम शामिल हैं
   - अनुमानित कंस्ट्रेंट को ओवरराइड करता है
...
डिस्कवरी कंस्ट्रेंट निम्नलिखित से निकाले जाते हैं:

1. **x-ves-validation-rules**: नेटिव F5 वैलिडेशन एक्सटेंशन
...
2. **त्रुटि कैटलॉग**: कंस्ट्रेंट उल्लंघन दर्शाने वाले API त्रुटि संदेश
   - "name must be 1-63 characters" → minLength/maxLength
   - "port must be between 1 and 65535" → minimum/maximum

3. **प्रतिक्रिया वैलिडेशन**: वैध मान प्रदर्शित करने वाली सफल API प्रतिक्रियाएँ
   - देखे गए मान सीमाएँ
...
| पैटर्न | फ़ील्ड नाम Regex | कंस्ट्रेंट प्रकार | उदाहरण |
|---------|------------------|-----------------|---------|
| उपयोगकर्ता नाम | `\b(user\|username)$` | String (1-64 अक्षर, अल्फ़ान्यूमेरिक) | `admin` |
| प्रदर्शन नाम | `\b(display_?)?name$` | String (1-256 अक्षर, मुक्त पाठ) | `My Service` |
...
- **समस्याएँ**: कंस्ट्रेंट सटीकता समस्याओं की रिपोर्ट https://github.com/f5xc-salesdemos/api-specs-enriched/issues पर करें
- **दस्तावेज़ीकरण**: पूर्ण API दस्तावेज़ीकरण https://f5xc-salesdemos.github.io/api-specs-enriched/ पर उपलब्ध है
- **MCP एकीकरण**: MCP सर्वर एकीकरण उदाहरणों के लिए `f5xc-api-mcp` रिपॉज़िटरी देखें

---

**संस्करण**: 3.3.0
**अंतिम अपडेट**: 2026-01-19
**समाधित कंस्ट्रेंट**: 9,851
