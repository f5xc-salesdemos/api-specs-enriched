...
description: एनरिच्ड OpenAPI स्पेसिफिकेशन में प्रत्येक x-* एक्सटेंशन का प्रामाणिक स्रोत
i18n:
...
प्रत्येक `x-*` एक्सटेंशन का प्रामाणिक स्रोत जो
`docs/specifications/api/*.json` में दिखाई देता है।
...
- **यहाँ इंजेक्ट किए गए** — हमारे एनरिचर्स द्वारा जोड़े गए एक्सटेंशन (`x-f5xc-*` और
  `x-ves-cli-*` / `x-ves-field-*` / `x-ves-operation-*` / डिस्कवरी
  वेरिएंट)। ये वे हैं जिनका डाउनस्ट्रीम टूल्स को उपभोग करना चाहिए।
- **अपस्ट्रीम पास-थ्रू** — F5 द्वारा स्रोत स्पेक्स में उत्सर्जित एक्सटेंशन
  जिन्हें हम अपरिवर्तित रखते हैं (`x-ves-proto-*`, `x-displayname`, आदि)।
  पारदर्शिता के लिए प्रलेखित लेकिन इस रिपॉजिटरी द्वारा नियंत्रित नहीं।
- **भविष्य में इंजेक्ट होने वाले** — अभी तक उत्सर्जित नहीं; जैसे ही कोई
  एनरिचर इन्हें उत्पन्न करना शुरू करता है, यहाँ प्रलेखित किए जाते हैं (प्रारंभिक
  भरने के समय लागू नहीं)।

## प्रविष्टि स्कीमा

नीचे प्रत्येक प्रविष्टि का ठीक यही आकार है। `tests/test_extension_catalog.py`
में समानता परीक्षण अनुभाग बॉडी के संक्षिप्त होने को तब तक सहन करता है जब तक
`### x-name` हेडर मौजूद है और `Pass-through from upstream:` फ्लैग `yes` या
`no` मान के साथ उपस्थित है।
...
- **Applied at:** info
- **Purpose:** पाइपलाइन द्वारा एनरिच्ड स्पेक पर लगाया गया सिमेंटिक वर्शन।
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** डिस्कवरी के दौरान जाँचे गए लाइव API का बेस URL।
- **Consumers:** multiple
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** डिस्कवरी के दौरान जाँचे गए API के लिए देखा गया प्रतिक्रिया समय (ms)।
- **Consumers:** multiple
- **Value type:** number
...
- **Applied at:** info
- **Purpose:** प्रति-डोमेन संक्षिप्तीकरण विस्तार तालिका।
- **Consumers:** multiple
- **Value type:** object
...
- **Applied at:** schema
- **Purpose:** किसी संसाधन स्कीमा के लिए मानव-पठनीय प्रदर्शन नाम (स्वचालित जनरेशन को ओवरराइड करता है)।
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Purpose:** एनरिच्ड प्रॉपर्टी विवरण जो अपस्ट्रीम `description` को पूरक बनाता है।
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** किसी प्रॉपर्टी के लिए अनेक उदाहरणात्मक मान।
- **Consumers:** multiple
- **Value type:** array
...
- **Applied at:** schema property
- **Purpose:** शेल कम्प्लीशन संकेत (स्थिर enum या गतिशील कमांड)।
- **Consumers:** CLI
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** जनरेट किए गए डॉक्स और UI में दिखाने के लिए डिफ़ॉल्ट मान।
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** HTTP ऑपरेशन (POST/PUT/...) की सूची देता है जिनके लिए यह प्रॉपर्टी आवश्यक है।
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** नामित फ़ीचर संयोजनों की सूची देता है जिनके लिए यह प्रॉपर्टी आवश्यक है।
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/utils/minimum_configuration_enricher.py
...
- **Applied at:** schema property
- **Purpose:** सशर्त आवश्यकताएँ (जैसे जब सहोदर फ़ील्ड X के बराबर हो तब आवश्यक)।
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** प्रतिस्थापन मार्गदर्शन के साथ बहिष्करण सूचना।
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** जब क्लाइंट प्रॉपर्टी को छोड़ देता है तब सर्वर द्वारा असाइन किया गया डिफ़ॉल्ट मान।
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{}`
...
- **Applied at:** schema property
- **Purpose:** सहोदर प्रॉपर्टीज़ की सूची देता है जिन्हें इसके साथ सेट नहीं किया जा सकता।
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
...
- **Applied at:** schema property
- **Purpose:** क्रॉस-फ़ील्ड निर्भरताओं को प्रलेखित करता है जहाँ एक फ़ील्ड के लिए दूसरे का सेट होना आवश्यक है।
...
- **Applied at:** schema property
- **Purpose:** लाइव-API प्रोबिंग या स्थिर पैटर्न से व्युत्पन्न संख्यात्मक / स्ट्रिंग बाधाएँ।
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** schema property
- **Purpose:** घोषित करता है कि कोई फ़ील्ड अपने दायरे के भीतर अद्वितीय होना चाहिए या नहीं।
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** operation
- **Purpose:** किसी ऑपरेशन के प्रभाव क्षेत्र को वर्गीकृत करता है (low/medium/high/critical)।
- **Consumers:** multiple
- **Value type:** string
...
- **Applied at:** operation
- **Purpose:** क्या CLI/UI को निष्पादन से पहले उपयोगकर्ता से पुष्टि का अनुरोध करना चाहिए।
- **Consumers:** multiple
- **Value type:** boolean
...
- **Applied at:** operation
- **Purpose:** ऑपरेशन के देखने योग्य दुष्प्रभावों (restart, reconfigure, आदि) की सूची देता है।
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/utils/operation_metadata_enricher.py
...
- **Applied at:** operation
- **Purpose:** लाइव API से प्रकट रेट-लिमिट हेडर / व्यवहार।
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** operation
- **Purpose:** लाइव डिस्कवरी के दौरान देखी गई त्रुटि प्रतिक्रियाओं की सूची, नमूना पेलोड के साथ।
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** info
- **Purpose:** किसी डोमेन के लिए शीर्ष-स्तरीय CLI / UI / docs / Terraform समूहीकरण श्रेणी।
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** वे संसाधन जिन्हें उन्नत सावधानी की आवश्यकता है (प्रोडक्शन-क्रिटिकल)।
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** संक्षिप्त (~60 अक्षर) डोमेन विवरण। लंबे विवरण के लिए प्रॉपर्टी स्तर पर भी लागू होता है।
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
...
- **Applied at:** info
- **Purpose:** दीर्घ (~500 अक्षर) डोमेन विवरण।
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
...
- **Applied at:** info
- **Purpose:** इस डोमेन के साथ सामान्यतः उपयोग किए जाने वाले अन्य डोमेन के क्रॉस-लिंक।
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** रेंडर किए गए डॉक्स के लिए दस्तावेज़ अनुभाग / नेवीगेशन समूहीकरण स्लग।
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Injected by:** upstream
- **Driven by config:** upstream
- **Example:** F5 अपस्ट्रीम डॉक्स देखें।
- **Pass-through from upstream:** yes
