---
title: Miglioramenti dell'HTTP LoadBalancer
description: Metadati di arricchimento per gli schemi dell'HTTP load balancer
sidebar:
  order: 2
i18n:
...
Metadati di arricchimento per gli schemi dell'HTTP load balancer. Consulta le [Estensioni OpenAPI](../development/#openapi-extensions) per le definizioni delle estensioni.
...
L'estensione `x-f5xc-minimum-configuration` fornisce metadati CLI completi per la creazione di HTTP load balancer minimamente funzionanti.
...
- `loadbalancer_type` nelle estensioni native della specifica OpenAPI (nome nativo del campo)
...
I campi contrassegnati con `x-f5xc-server-default: true` hanno il loro valore `default` applicato dal server API F5 XC quando vengono omessi dalle richieste. L'HTTP load balancer ha un gran numero di valori predefiniti applicati dal server perché la maggior parte delle funzionalità di sicurezza e delle opzioni di protocollo sono impostate per default su uno stato disabilitato o sicuro.

### Valori predefiniti per HTTPS Auto-Cert
...
| `default_header` | `{}` | object | Gestione predefinita dell'header del nome server |
...
| `header_transformation_type.legacy_header_transformation` | `{}` | object | Trasformazione legacy degli header |
...
| `disable_bot_defense` | `{}` | Difesa dai bot disabilitata |
| `disable_rate_limit` | `{}` | Limitazione della frequenza disabilitata |
| `disable_api_discovery` | `{}` | Scoperta delle API disabilitata |
| `disable_api_testing` | `{}` | Test delle API disabilitato |
| `disable_api_definition` | `{}` | Definizione delle API disabilitata |
...
### Valori predefiniti per DDoS e controllo degli accessi

| Campo | Valore predefinito | Descrizione |
|-------|---------------|-------------|
| `l7_ddos_protection.mitigation_block` | `{}` | Blocca il traffico DDoS (azione di mitigazione predefinita) |
| `l7_ddos_protection.default_rps_threshold` | `{}` | Usa la soglia RPS predefinita |
...
| `user_id_client_ip` | `{}` | Identifica gli utenti tramite IP client |
| `disable_trust_client_ip_headers` | `{}` | Non fidarsi degli header IP del client |

### Altri valori predefiniti applicati dal server

| Campo | Valore predefinito | Tipo | Descrizione |
|-------|---------------|------|-------------|
| `advertise_on_public_default_vip` | `{}` | object | Pubblicizza sul VIP pubblico predefinito |
| `round_robin` | `{}` | object | Algoritmo di bilanciamento round-robin |
| `add_location` | `true` | boolean | Aggiungi header di localizzazione alle risposte |
| `system_default_timeouts` | `{}` | object | Usa i timeout predefiniti del sistema |
| `service_policies_from_namespace` | `{}` | object | Eredita le policy di servizio dal namespace |
| `default_sensitive_data_policy` | `{}` | object | Usa la policy predefinita per i dati sensibili |
...
| `advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | Come pubblicizzare il load balancer |
...
| `server_name_header` | `default_header`, `append_server_name_header`, `pass_through_server_name_header` | Gestione dell'header del nome server |
...
| `bot_defense` | `disable_bot_defense`, `enable_bot_defense` | Rilevamento e mitigazione dei bot |
| `rate_limit` | `disable_rate_limit`, `enable_rate_limit` | Limitazione della frequenza |
| `api_discovery` | `disable_api_discovery`, `enable_api_discovery` | Scoperta delle API |
| `api_testing` | `disable_api_testing`, `enable_api_testing` | Test delle API |
| `api_definition` | `disable_api_definition`, `enable_api_definition` | Definizione delle API |
...
| `ddos_policy` | `ddos_policy_none`, `ddos_policy_ref` | Riferimento alla policy DDoS o nessuna |

### Gruppi di altre impostazioni (6 gruppi)

| Nome gruppo | Campi | Descrizione |
|-----------|--------|-------------|
| `challenge` | `no_challenge`, `js_challenge`, `captcha_challenge` | Tipo di sfida per il client per il rilevamento dei bot |
| `user_identification` | `user_id_client_ip`, `user_identification` | Metodo di identificazione dell'utente |
| `client_ip_headers` | `disable_trust_client_ip_headers`, `enable_trust_client_ip_headers` | Fidarsi o meno degli header IP del client |
| `timeouts` | `system_default_timeouts`, `custom_timeouts` | Usa timeout predefiniti del sistema o personalizzati |
...
| `sensitive_data_policy` | `default_sensitive_data_policy`, `custom_sensitive_data_policy` | Usa policy predefinita o personalizzata per i dati sensibili |

## Raccomandazioni per le varianti OneOf (estensione futura)
...
1. Osservare le selezioni predefinite della console F5 XC (ad esempio, quale lb_type è preselezionato nell'interfaccia utente)
...
Questa estensione consentirebbe agli strumenti a valle di preselezionare la variante più comunemente utilizzata quando si presentano le opzioni di configurazione agli utenti.
...
|-------|---------|---------|---------|-------------|
| `spec.https_auto_cert.port` | 1 | 65535 | 443 | Numero di porta HTTPS |
...
| `spec.https_auto_cert.header_transformation_type` | `legacy_header_transformation`, `proper_header_transformation`, `preserve_case_header_transformation` | `legacy_header_transformation` | Trasformazione degli header HTTP |
...
| `spec.l7_ddos_protection.mitigation` | `mitigation_block`, `mitigation_challenge`, `mitigation_none` | `mitigation_block` | Azione di mitigazione DDoS di livello 7 |
...
| `spec.challenge` | `no_challenge`, `js_challenge`, `captcha_challenge` | `no_challenge` | Tipo di sfida per il client |
| `spec.advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | `advertise_on_public_default_vip` | Pubblicizzazione del load balancer |

## Integrazione dei flussi di lavoro guidati

La pipeline di arricchimento aggiunge i metadati `x-f5xc-guided-workflows` a livello di spec. Questi flussi di lavoro forniscono una guida passo-passo per il deployment, utilizzata da assistenti AI e strumenti CLI.

### Deploy dell'HTTP Load Balancer

Un flusso di lavoro in 5 passaggi per creare un HTTP load balancer completamente configurato con origin pool di backend (definito in `config/guided_workflows.yaml`):

| Passaggio | Azione | Risorsa | Campi obbligatori | Opzionale |
|------|--------|----------|-----------------|----------|
| 1 | Crea Origin Pool | `origin_pool` | `name`, `origin_servers`, `port` | No |
| 2 | Configura Health Check | `healthcheck` | `name`, `http_health_check` | Sì |
| 3 | Collega Health Check | - | - | Sì |
| 4 | Crea HTTP Load Balancer | `http_loadbalancer` | `name`, `domains`, `http.port` | No |
| 5 | Verifica il deployment | - | - | No |
...
### Deploy dell'HTTPS Load Balancer con TLS

Un flusso di lavoro in 7 passaggi per creare un HTTPS load balancer con terminazione SSL/TLS:

| Passaggio | Azione | Risorsa | Campi obbligatori | Opzionale |
|------|--------|----------|-----------------|----------|
| 1 | Carica certificato SSL | `certificate` | `name`, `certificate_chain`, `private_key` | No |
| 2 | Crea Origin Pool | `origin_pool` | `name`, `origin_servers` | No |
| 3 | Configura Health Check | `healthcheck` | - | Sì |
| 4 | Configura Policy WAF | `app_firewall` | - | Sì |
| 5 | Crea HTTPS Load Balancer | `http_loadbalancer` | `name`, `domains`, `https.tls_parameters` | No |
| 6 | Collega Policy WAF | - | - | Sì |
| 7 | Verifica il deployment HTTPS | - | - | No |
...
- **xcsh CLI**: Presenta i flussi di lavoro come comandi guidati interattivi (ad esempio, `xcsh deploy http-lb`)
- **Estensione VS Code**: Mostra i passaggi del flusso di lavoro in un pannello laterale con monitoraggio dell'avanzamento
...
Fornisce metadati completi per la creazione di configurazioni minimamente funzionanti. Include:
- Descrizione
...
Quando è `true`, indica che il valore `default` associato viene applicato dal server API F5 XC. I campi con questa estensione possono essere omessi in sicurezza dalle richieste API — il server applica automaticamente il valore predefinito.
...
Fornisce lunghezze alternative delle descrizioni per diversi casi d'uso:
...
- [Miglioramenti dell'Healthcheck](./healthcheck-enhancements/) - Arricchimenti dello schema healthcheck (include `x-f5xc-recommended-oneof-variant`)
- [Miglioramenti dell'Origin Pool](./originpool-enhancements/) - Arricchimenti dello schema origin pool
...
|---------|------|---------|
| 2.0.46 | 2026-04-18 | Aggiunti valori predefiniti applicati dal server, integrazione dei flussi di lavoro guidati e sezione ampliata sulla configurazione minima |
