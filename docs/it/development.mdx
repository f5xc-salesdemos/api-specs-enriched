...
**Regole per il bump di versione**:
...
## Discovery nel dettaglio

### Cos'è la discovery?

La discovery esplora l'API live di F5 XC per trovare:

- **Vincoli non documentati**: Campi obbligatori non indicati nell'OpenAPI
...
| Estensione | Scopo |
|------------|-------|
| `x-f5xc-server-default: true` | Indica i valori predefiniti applicati dal server |
| `x-f5xc-recommended-value` | Valore pre-compilato dalla console F5 XC |
...
Quando è `true`, indica che il valore `default` associato viene applicato dal server API F5 XC. I campi con questa estensione possono essere tranquillamente omessi dalle richieste API — il server applica automaticamente il valore predefinito.
...
Specifica un valore che la console web F5 XC utilizza come predefinito pre-compilato. Questo valore non è applicato dal server, ma rappresenta la configurazione iniziale tipica per le nuove risorse create tramite la console.
...
Dichiara relazioni di mutua esclusività tra i membri di un gruppo OneOf. Derivato automaticamente dalle estensioni `x-ves-oneof-field-*`. Questo permette agli strumenti a valle (Terraform, CLI, MCP) di validare i conflitti a livello di schema anziché a runtime.
...
L'arricchitore ispeziona `x-ves-validation-rules` per dedurre lo stato di obbligatorietà:

| Regola | Implicazione |
|--------|--------------|
| `ves.io.schema.rules.message.required: "true"` | Il campo è obbligatorio |
| `ves.io.schema.rules.uint32.gte: N` | Se N >= 1 e non c'è un valore predefinito del server, il campo è obbligatorio |
...
1. Creare la risorsa in F5 XC con specifica minimale tramite API
...
La pipeline viene eseguita ad ogni commit per garantire la coerenza. Per i commit di lavoro in corso:
...
Aggiungere l'esclusione a `.pre-commit-config.yaml`:
...
| `make rebuild` | Rebuild rapido (salta il download) |
| `make download` | Scarica le specifiche (cache ETag) |
