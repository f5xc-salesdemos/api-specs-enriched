...
- Campi obbligatori/opzionali effettivi
- Valori enum dalle risposte live
- Valori predefiniti
- Validazioni dei pattern
- Esempi di risposta
...
1. Schedulazione giornaliera (6:00 UTC) o push su main attiva il workflow
...
5. Commit diretto + tag creato (nessun PR per il bump di versione)
6. GitHub Release creata con changelog
7. Documentazione pubblicata su GitHub Pages
...
## Discovery in dettaglio

### Cos'è la Discovery?

La Discovery esplora l'API live di F5 XC per trovare:
...
### Contenuti del pacchetto di rilascio
...
|------------|-------------|--------------|
| `x-f5xc-server-default` | Il server API F5 XC applica questo valore quando il campo viene omesso | Il campo è opzionale; ometterlo produce il comportamento predefinito documentato |
| `x-f5xc-recommended-value` | La console web F5 XC pre-popola questo valore per le nuove risorse | Il campo non ha un valore predefinito del server ma questo valore rappresenta la configurazione tipica |
...
| `x-f5xc-conflicts-with` | Elenca le altre proprietà che non possono essere usate insieme a questo campo | La proprietà fa parte di un gruppo OneOf; solo una delle proprietà in conflitto può essere specificata |
...
Specifica un valore che la console web F5 XC utilizza come predefinito pre-popolato. Questo valore non è applicato dal server ma rappresenta la configurazione di partenza tipica per le nuove risorse create tramite la console.
...
L'arricchitore ispeziona `x-ves-validation-rules` per inferire lo stato di obbligatorietà:
...
### Pre-commit impiega troppo tempo

La pipeline viene eseguita ad ogni commit per garantire la coerenza. Per i commit in corso d'opera:
...
La versione è derivata dai tag git (ad es., `v2.0.38`), eliminando le condizioni di race che causavano conflitti di merge su PR concorrenti.
...
| `make download` | Download delle specifiche (cache ETag) |
...
| `make discover` | Discovery API (richiesta VPN) |
...
**Nota**: La versione è derivata dai tag git (ad es., `v2.0.38`), non da un file `.version`.
