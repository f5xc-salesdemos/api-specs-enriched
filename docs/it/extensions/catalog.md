...
Fonte di verità per ogni estensione `x-*` presente in
...
- **Pass-through dall'upstream** — estensioni emesse da F5 nelle specifiche sorgente
  e da noi preservate senza modifiche (`x-ves-proto-*`, `x-displayname`, ecc.).
...
- **Applied at:** info
- **Purpose:** Blocco di metadati a livello CLI (nome dello strumento, indicazioni di versione, raggruppamento per dominio).
- **Consumers:** CLI
- **Value type:** object
...
- **Applied at:** info
- **Purpose:** Blocco glossario di branding/terminologia applicato a ciascuna specifica di dominio.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** info
- **Purpose:** URL base dell'API live sondata durante il discovery.
- **Consumers:** multiple
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** Workflow guidati passo-passo per completare attività comuni in un dominio.
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
...
- **Applied at:** schema
- **Purpose:** Insieme minimo di campi necessari per eseguire con successo un POST/PUT su questa risorsa.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** schema property
- **Purpose:** Suggerimenti di completamento shell (enum statico o comando dinamico).
- **Consumers:** CLI
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Requisiti condizionali (es. obbligatorio quando un campo fratello ha valore X).
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Valore consigliato per la produzione per un campo dove il valore predefinito del server è subottimale.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{}`
- **Injected by:** scripts/utils/default_value_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Per i blocchi `oneOf`, indica quale variante è consigliata.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/default_value_enricher.py
...
- **Applied at:** schema property
- **Purpose:** Documenta le dipendenze tra campi dove un campo richiede che un altro sia impostato.
...
- **Applied at:** schema property
- **Purpose:** Vincoli numerici / di stringa derivati dal sondaggio dell'API live o da pattern statici.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** schema property
- **Purpose:** Dichiara se un campo deve essere univoco all'interno del proprio ambito.
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** info
- **Purpose:** Categoria di raggruppamento di primo livello per CLI / UI / documentazione / Terraform per un dominio.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Descrizione breve (~60 caratteri) del dominio. Si applica anche a livello di proprietà per descrizioni lunghe.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
...
- **Applied at:** info
- **Purpose:** Descrizione media (~150 caratteri) del dominio. Si applica anche a livello di proprietà.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/property_description_short_enricher.py
...
- **Applied at:** info
- **Purpose:** Descrizione lunga (~500 caratteri) del dominio.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
...
- **Applied at:** info
- **Purpose:** Identificativo dell'icona da utilizzare nel rendering di questo dominio in una UI.
- **Consumers:** Web UI
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** Slug della sezione documentazione / raggruppamento di navigazione per la documentazione renderizzata.
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** upstream
- **Purpose:** Preservato senza modifiche dalla specifica upstream F5.
- **Consumers:** N/A
- **Value type:** varies
- **Value schema:** N/A
