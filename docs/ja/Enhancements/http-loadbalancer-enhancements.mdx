...
`x-f5xc-minimum-configuration` 拡張は、最小限の有効な HTTP ロードバランサーを作成するための包括的な CLI メタデータを提供します。
...
| `spec.domains` | 配列、min_items: 1 | サービスを提供するドメイン |
...
`x-f5xc-server-default: true` でマークされたフィールドは、リクエストから省略された場合に F5 XC API サーバーによって `default` 値が適用されます。HTTP ロードバランサーには多数のサーバー適用デフォルトがあります。これは、ほとんどのセキュリティ機能やプロトコルオプションがデフォルトで無効または安全な状態になっているためです。
...
| `tls_config.default_security` | `{}` | object | TLS 1.2+ と強力な暗号化 |
...
`x-f5xc-conflicts-with` でマークされたフィールドは OneOf パターンを示します。各グループから 1 つのフィールドのみを指定できます。
...
| `mtls` | `no_mtls`, `use_mtls` | 相互 TLS の有効化または無効化 |
...
| グループ名 | フィールド | 説明 |
|-----------|--------|-------------|
| `path_normalize` | `enable_path_normalize`, `disable_path_normalize` | パス正規化の有効化または無効化 |
...
| `ddos_rps_threshold` | `default_rps_threshold`, `custom_rps_threshold` | 毎秒リクエスト数しきい値 |
...
| グループ名 | フィールド | 説明 |
|-----------|--------|-------------|
| `challenge` | `no_challenge`, `js_challenge`, `captcha_challenge` | ボット検出用のクライアントチャレンジタイプ |
...
## OneOf バリアントの推奨（将来の拡張）

**ステータス**: http_loadbalancer スキーマにはまだ適用されていません。

ヘルスチェックには OneOf グループで最も一般的な選択肢を示す `x-f5xc-recommended-oneof-variant` が含まれていますが、http_loadbalancer には現在この拡張がありません。
...
`x-f5xc-constraints` でマークされたフィールドには、API 分析から発見されたバリデーション制約が含まれています。
...
エンリッチメントパイプラインは、スペックレベルで `x-f5xc-guided-workflows` メタデータを追加します。これらのワークフローは、AI アシスタントや CLI ツールが使用するステップバイステップのデプロイメントガイダンスを提供します。

### HTTP ロードバランサーのデプロイ

バックエンドオリジンプール付きの完全に構成された HTTP ロードバランサーを作成するための 5 ステップのワークフローです（`config/guided_workflows.yaml` で定義）：
...
**前提条件**: ターゲットテナントに有効な名前空間があること、バックエンドアプリケーションが IP または DNS で到達可能であること、SSL 証明書（HTTPS の場合はオプション）。

### TLS 付き HTTPS ロードバランサーのデプロイ

SSL/TLS ターミネーション付き HTTPS ロードバランサーを作成するための 7 ステップのワークフローです：
...
**前提条件**: ターゲットテナントに有効な名前空間があること、SSL 証明書と秘密鍵、バックエンドアプリケーションが到達可能であること。

### ワークフローの利用

ダウンストリームツールはこれらのワークフローを使用して、マルチステップのデプロイメントをユーザーにガイドします：

- **xcsh CLI**: ワークフローをインタラクティブなガイドコマンドとして表示（例：`xcsh deploy http-lb`）
- **VS Code 拡張機能**: サイドバーパネルにワークフローステップを進捗追跡付きで表示
...
最小限の有効な構成を作成するための包括的なメタデータを提供します。以下を含みます：
- 説明
- 制約付きの必須フィールド
- 相互排他グループ
- 構成例（YAML、JSON）
...
`true` の場合、付随する `default` 値が F5 XC API サーバーによって適用されることを示します。この拡張が付いたフィールドは API リクエストから安全に省略できます — サーバーが自動的にデフォルトを適用します。
...
配列サイズ制限、整数範囲、一意性要件、および検出メタデータを含むバリデーション制約を提供します。
...
さまざまなユースケース向けに代替の説明の長さを提供します：
- `x-f5xc-description-short`: 最大 60 文字（CLI カラム、バッジ）
- `x-f5xc-description-medium`: 最大 150 文字（ツールチップ、要約）
...
| `config/constraint_patterns.yaml` | 制約パターンの定義 |
| `config/guided_workflows.yaml` | ガイドワークフローステップの定義 |
...
- [ヘルスチェックの拡張](./healthcheck-enhancements/) - ヘルスチェックスキーマのエンリッチメント（`x-f5xc-recommended-oneof-variant` を含む）
- [オリジンプールの拡張](./originpool-enhancements/) - オリジンプールスキーマのエンリッチメント
- [制約メタデータガイド](../constraint-metadata/) - 詳細な制約バリデーションドキュメント

## 変更履歴

| バージョン | 日付 | 変更内容 |
|---------|------|---------|
| 2.0.46 | 2026-04-18 | サーバー適用デフォルト、ガイドワークフロー統合、および最小構成セクションの拡張を追加 |
