...
HTTP ロードバランサースキーマのエンリッチメントメタデータです。拡張の定義については [OpenAPI 拡張](../development/#openapi-extensions) を参照してください。
...
`x-f5xc-minimum-configuration` 拡張は、最小限で動作可能な HTTP ロードバランサーを作成するための包括的な CLI メタデータを提供します。
...
以下のロードバランサータイプのいずれか1つを指定する必要があります：

| バリアント | 説明 |
|---------|-------------|
| `spec.http` | HTTP のみ |
| `spec.https` | 手動証明書による HTTPS |
| `spec.https_auto_cert` | 自動証明書による HTTPS |
| `spec.http_https` | HTTP と HTTPS の両方 |

**注意**: ロードバランサータイプの OneOf グループは以下のように呼ばれます：
- 構成ファイルでは `lb_type`（`config/minimum_configs.yaml` での短縮形）
- OpenAPI スペックのネイティブ拡張では `loadbalancer_type`（ネイティブフィールド名）
...
`x-f5xc-server-default: true` とマークされたフィールドは、リクエストから省略された場合に F5 XC API サーバーによって `default` 値が適用されます。HTTP ロードバランサーには多数のサーバー適用デフォルトがあります。これは、ほとんどのセキュリティ機能やプロトコルオプションがデフォルトで無効または安全な状態になるためです。

### HTTPS Auto-Cert のデフォルト値
...
| `tls_config.default_security` | `{}` | object | 強力な暗号による TLS 1.2 以上 |
...
| `disable_malware_protection` | `{}` | マルウェア保護無効 |
| `disable_client_side_defense` | `{}` | クライアントサイド防御無効 |
| `disable_ip_reputation` | `{}` | IP レピュテーション無効 |
| `disable_threat_mesh` | `{}` | 脅威メッシュ無効 |
| `disable_malicious_user_detection` | `{}` | 悪意あるユーザー検出無効 |

### DDoS およびアクセス制御のデフォルト値
...
`x-f5xc-conflicts-with` とマークされたフィールドは OneOf パターンを示します。各グループから1つのフィールドのみ指定できます。
...
| `mtls` | `no_mtls`, `use_mtls` | 相互 TLS の有効/無効 |
...
| グループ名 | フィールド | 説明 |
|-----------|--------|-------------|
| `path_normalize` | `enable_path_normalize`, `disable_path_normalize` | パス正規化の有効/無効 |
...
| `malware_protection` | `disable_malware_protection`, `enable_malware_protection` | マルウェア保護 |
...
| `threat_mesh` | `disable_threat_mesh`, `enable_threat_mesh` | 脅威メッシュ |
| `malicious_user_detection` | `disable_malicious_user_detection`, `enable_malicious_user_detection` | 悪意あるユーザー検出 |
...
| `ddos_rps_threshold` | `default_rps_threshold`, `custom_rps_threshold` | 秒間リクエスト数しきい値 |
...
| グループ名 | フィールド | 説明 |
|-----------|--------|-------------|
| `challenge` | `no_challenge`, `js_challenge`, `captcha_challenge` | ボット検出用クライアントチャレンジタイプ |
| `user_identification` | `user_id_client_ip`, `user_identification` | ユーザー識別方法 |
| `client_ip_headers` | `disable_trust_client_ip_headers`, `enable_trust_client_ip_headers` | クライアント IP ヘッダーの信頼有無 |
| `timeouts` | `system_default_timeouts`, `custom_timeouts` | システムデフォルトまたはカスタムタイムアウトの使用 |
| `service_policies_source` | `service_policies_from_namespace`, `active_service_policies` | 名前空間からのサービスポリシーまたはアクティブリスト |
| `sensitive_data_policy` | `default_sensitive_data_policy`, `custom_sensitive_data_policy` | デフォルトまたはカスタム機密データポリシーの使用 |

## OneOf バリアント推奨（将来の拡張）

**ステータス**: http_loadbalancer スキーマにはまだ適用されていません。

ヘルスチェックには OneOf グループの最も一般的な選択肢を示す `x-f5xc-recommended-oneof-variant` が含まれていますが、http_loadbalancer には現在この拡張がありません。
...
1. F5 XC コンソールのデフォルト選択を観察する（例：UI でどの lb_type がプリセレクトされているか）
2. `config/discovered_defaults.yaml` に構成を追加する
...
viewshttp_loadbalancerCreateSpecType:
  x-f5xc-recommended-oneof-variant:
    loadbalancer_type: "https_auto_cert"  # 例 - 検証が必要
```

この拡張により、ダウンストリームツールがユーザーに構成オプションを提示する際に、最も一般的に使用されるバリアントをプリセレクトできるようになります。

## 制約メタデータ

`x-f5xc-constraints` とマークされたフィールドには、API 分析から発見されたバリデーション制約が含まれています。
...
| `spec.l7_ddos_protection.mitigation` | `mitigation_block`, `mitigation_challenge`, `mitigation_none` | `mitigation_block` | レイヤー7 DDoS 緩和アクション |
| `spec.l7_ddos_protection.rps_threshold` | `default_rps_threshold`, `custom_rps_threshold` | `default_rps_threshold` | DDoS 検出用 RPS しきい値 |
...
| `spec.advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | `advertise_on_public_default_vip` | ロードバランサーアドバタイズ |
...
バックエンドオリジンプールを含む完全に構成された HTTP ロードバランサーを作成するための5ステップワークフロー（`config/guided_workflows.yaml` で定義）：
...
**前提条件**: ターゲットテナントの有効な名前空間、IP または DNS で到達可能なバックエンドアプリケーション、SSL 証明書（HTTPS の場合はオプション）。
...
**前提条件**: ターゲットテナントの有効な名前空間、SSL 証明書と秘密鍵、到達可能なバックエンドアプリケーション。

### ワークフローの利用

ダウンストリームツールは、これらのワークフローを使用してマルチステップデプロイメントをガイドします：

- **xcsh CLI**: ワークフローをインタラクティブなガイド付きコマンドとして提示（例：`xcsh deploy http-lb`）
- **VS Code 拡張機能**: サイドバーパネルに進捗追跡付きのワークフローステップを表示
- **AI アシスタント**: ワークフローステップを順番に実行し、各リソースを作成して依存関係を検証

## OpenAPI 拡張リファレンス

これらのベンダー拡張は、F5 XC 固有のメタデータを伝えるために標準 OpenAPI スキーマに追加されます。

### x-f5xc-cli-domain

**型**: `string`

CLI およびツール組織化のためのドメイン分類を示します。
...
配列サイズ制限、整数範囲、一意性要件、ディスカバリーメタデータを含むバリデーション制約を提供します。
...
ドキュメントおよびツール用の具体的なフィールド例を提供します。
...
- [開発ガイド - OpenAPI 拡張](../development/#openapi-extensions) - 拡張の定義と使用法
- [ヘルスチェック拡張](./healthcheck-enhancements/) - ヘルスチェックスキーマのエンリッチメント（`x-f5xc-recommended-oneof-variant` を含む）
- [オリジンプール拡張](./originpool-enhancements/) - オリジンプールスキーマのエンリッチメント
- [制約メタデータガイド](../constraint-metadata/) - 制約バリデーションの詳細ドキュメント

## 変更履歴

| バージョン | 日付 | 変更内容 |
|---------|------|---------|
| 2.0.46 | 2026-04-18 | サーバー適用デフォルト値、ガイド付きワークフロー統合、最小構成セクションの拡張を追加 |
