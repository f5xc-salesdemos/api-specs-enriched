...
origin_pool 関連スキーマのエンリッチメントメタデータです。拡張機能の定義については [OpenAPI 拡張](../development/#openapi-extensions) を参照してください。
...
| `disable_lb_source_ip_persistance` | `{}` | object | LB ソース IP 永続性無効 |
...
|-------|-----------------|----------------|------|
| `loadbalancer_algorithm` | `LB_OVERRIDE` | `ROUND_ROBIN` | フィールドが省略された場合、UI 作成リソースと API 作成リソースで異なる |
...
| 値 | 説明 | 備考 |
|-------|-------------|-------|
| `ROUND_ROBIN` | 各正常なエンドポイントをラウンドロビン順に選択 | サーバーデフォルト |
...
相互に排他的なフィールドグループです。各グループから1つのフィールドのみ指定できます：

| グループ | フィールド | サーバーデフォルト |
|-------|--------|----------------|
| ポート構成 | `port`, `automatic_port`, `lb_port` | `port`（明示的） |
...
F5 XC Web コンソールは、オリジンプールに対して15の設定オプションを提供します：
...
| 15 | LB ソース IP 永続性 | `spec.advanced_options.[disable_lb_source_ip_persistance\|enable_lb_source_ip_persistance]` | OneOf | `disable_lb_source_ip_persistance` |
...
1. **サイトリソースの発見** - `site` リソーススキーマと site_locator パターンのドキュメント化
2. **ネットワーク選択列挙** - `network_choice` の制約された値（site_local_inside、site_local_outside など）のドキュメント化
...
| バージョン | 日付 | 変更内容 |
|---------|------|---------|
| 2.1.2 | 2026-01-18 | public_name/public_ip のオリジンサーバータイプセクションを追加。サイト依存タイプの TODO マーカーを追加 |
| 2.1.1 | 2026-01-18 | 純粋な API リファレンスとして書き直し。ダウンストリームコード例を削除 |
| 2.1.0 | 2026-01-18 | validation.json の統一されたデフォルト構造に更新 |
