...
- **ここで注入** — エンリッチャーが追加する拡張（`x-f5xc-*` および `x-ves-cli-*` / `x-ves-field-*` / `x-ves-operation-*` / ディスカバリーバリアント）。ダウンストリームツールが利用すべきものです。
- **アップストリームパススルー** — F5がソース仕様で発行し、変更せずに保持する拡張（`x-ves-proto-*`、`x-displayname` など）。透明性のために文書化していますが、このリポジトリでは管理していません。
- **将来注入** — まだ出力されていません。エンリッチャーが生成を開始した時点でここに文書化されます（初期登録時点では該当なし）。

## エントリスキーマ

以下のすべてのエントリはこの形式に従います。`tests/test_extension_catalog.py` のパリティテストは、`### x-name` ヘッダーが存在し、`Pass-through from upstream:` フラグが `yes` または `no` の値で存在する限り、セクション本体が簡略化されていても許容します。
...
- **Applied at:** info
- **Purpose:** エンリッチされた仕様のCLIドメインスラグ（例：`http_loadbalancer`）を識別します。
- **Consumers:** CLI
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** CLI全体のメタデータブロック（ツール名、バージョンヒント、ドメイングループ化）。
- **Consumers:** CLI
- **Value type:** object
...
- **Applied at:** info
- **Purpose:** エンリッチファイルの構築元となったアップストリームソース仕様のタイムスタンプ。
- **Consumers:** multiple
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** アップストリームソース仕様のリリースアセットのETag。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** パイプラインによってエンリッチされた仕様にスタンプされるセマンティックバージョン。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** ライブAPIディスカバリーパスが実行されたタイムスタンプ。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string", "format": "date-time"}`
...
- **Applied at:** info
- **Purpose:** ディスカバリー中にプローブされたライブAPIのベースURL。
- **Consumers:** multiple
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** このドメインのホスティングされたAPIリファレンスドキュメントページへのURL。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string", "format": "uri"}`
...
- **Applied at:** info
- **Purpose:** ディスカバリー中にプローブされたAPIの観測応答時間（ms）。
- **Consumers:** multiple
- **Value type:** number
...
- **Applied at:** schema
- **Purpose:** このリソースのPOST/PUTを成功させるために必要な最小限のフィールドセット。
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** info
- **Purpose:** リソースの名前空間制約、推奨事項、分類メタデータを提供します。
- **Consumers:** multiple
- **Value type:** object
...
- **Applied at:** schema
- **Purpose:** UI/CLIプレゼンテーションのためのプロパティの推奨順序。
- **Consumers:** multiple
- **Value type:** array
...
- **Applied at:** schema
- **Purpose:** このスキーマにマッピングされるTerraformリソースタイプ名。
- **Consumers:** Terraform
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema
- **Purpose:** リソーススキーマの人間が読める表示名（自動生成をオーバーライド）。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** アップストリームのprotobuf `ves.io.schema.rules` から導出された宣言的バリデーションルール。
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** schema property
- **Purpose:** シェル補完ヒント（静的enum または動的コマンド）。
- **Consumers:** CLI
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** 生成されたドキュメントやUIに表示するデフォルト値。
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** このプロパティを必要とするHTTPオペレーション（POST/PUT/...）をリストします。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** このプロパティを必要とする名前付き機能の組み合わせをリストします。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/utils/minimum_configuration_enricher.py
...
- **Applied at:** schema property
- **Purpose:** 条件付き要件（例：兄弟フィールドがXに等しい場合に必須）。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** サーバーデフォルトが最適でない場合のフィールドの推奨本番値。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{}`
- **Injected by:** scripts/utils/default_value_enricher.py
...
- **Applied at:** schema property
- **Purpose:** `oneOf` ブロックの場合、推奨されるバリアントを示します。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/default_value_enricher.py
...
- **Applied at:** schema property
- **Purpose:** ライブAPIプローブまたは静的パターンから導出された数値/文字列制約。
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Purpose:** 成功のために提供する必要があるオペレーションボディのフィールド名を指定します。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
...
- **Applied at:** operation
- **Purpose:** CLI/UIが実行前にユーザーに確認を求めるべきかどうか。
- **Consumers:** multiple
- **Value type:** boolean
...
- **Applied at:** operation
- **Purpose:** オペレーションの観測可能な副作用（再起動、再構成など）をリストします。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/utils/operation_metadata_enricher.py
...
- **Applied at:** operation
- **Purpose:** ディスカバリー中にこのオペレーションについて実測された応答時間。
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** operation
- **Purpose:** ライブAPIから表面化された観測されたレート制限ヘッダー/動作。
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** operation
- **Purpose:** ライブディスカバリー中に観測されたエラーレスポンスのカタログとサンプルペイロード。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** info
- **Purpose:** ドメインのトップレベルCLI / UI / ドキュメント / Terraformグループ化カテゴリ。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** ドメインを定義するプライマリリソースタイプのリスト。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** 高度な注意が必要なリソース（本番環境でクリティカル）。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** このドメインで構成を作成する際の相対的な複雑さのティア。
- **Consumers:** multiple
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** 必要な最小F5 XCサブスクリプションティア。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** UIでこのドメインをレンダリングする際に使用するアイコン識別子。
- **Consumers:** Web UI
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** ドメインを表すブランドロゴのインラインSVG（またはパス）。
- **Consumers:** Web UI
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** このドメインと一緒に使用されることが多い他のドメインへのクロスリンク。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** レンダリングされたドキュメントのドキュメントセクション/ナビゲーショングループ化スラグ。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
...
- **Injected by:** upstream
- **Driven by config:** upstream
- **Example:** F5アップストリームドキュメントを参照してください。
- **Pass-through from upstream:** yes
