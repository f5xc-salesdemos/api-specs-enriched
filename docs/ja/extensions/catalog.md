...
ここでは3つのクラスの拡張がドキュメント化されています：

- **ここで注入** — エンリッチャーが追加する拡張（`x-f5xc-*` および `x-ves-cli-*` / `x-ves-field-*` / `x-ves-operation-*` / ディスカバリーバリアント）。これらはダウンストリームツールが利用すべきものです。
- **アップストリームパススルー** — F5がソース仕様で出力し、変更せずに保持する拡張（`x-ves-proto-*`、`x-displayname` など）。透明性のためにドキュメント化されていますが、このリポジトリでは制御されません。
- **将来注入** — まだ出力されていません。エンリッチャーが生成を開始した時点でここにドキュメント化されます（初期登録時点では該当なし）。

## エントリスキーマ

以下のすべてのエントリは正確にこの形式を持ちます。`tests/test_extension_catalog.py` のパリティテストは、`### x-name` ヘッダーが存在し、`Pass-through from upstream:` フラグが `yes` または `no` の値で存在する限り、セクション本文が簡略化されていても許容します。

    ### x-<name>
    - **適用先:** <schema | parameter | operation | path-item | info | response>
    - **目的:** <一文>
    - **利用者:** <CLI | VSCode | Terraform | Web UI | 複数 | N/A>
    - **値の型:** <string | number | boolean | object | array>
    - **値スキーマ:** <JSON Schemaスニペット、またはN/A>
...
- **適用先:** info
- **目的:** エンリッチされた仕様のCLIドメインスラグ（例：`http_loadbalancer`）を識別します。
- **利用者:** CLI
- **値の型:** string
- **値スキーマ:** `{"type": "string"}`
...
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{"type": "string", "format": "date-time"}`
- **注入元:** scripts/merge_specs.py
- **設定による駆動:** hardcoded
...
- **適用先:** info
- **目的:** パイプラインによってエンリッチされた仕様にスタンプされるセマンティックバージョン。
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{"type": "string"}`
- **注入元:** scripts/merge_specs.py
...
- **適用先:** info
- **目的:** 各ドメイン仕様に適用されるブランディング/用語集ブロック。
- **利用者:** 複数
- **値の型:** object
- **値スキーマ:** `{"type": "object"}`
...
- **適用先:** info
- **目的:** このドメインのホストされたAPIリファレンスドキュメントページへのURL。
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{"type": "string", "format": "uri"}`
- **注入元:** scripts/utils/external_docs_enricher.py
- **設定による駆動:** なし（ドメイン名から派生）
...
- **適用先:** info
- **目的:** ディスカバリー中にプローブされたAPIの観測された応答時間（ミリ秒）。
- **利用者:** 複数
- **値の型:** number
- **値スキーマ:** `{"type": "number"}`
- **注入元:** scripts/utils/discovery_enricher.py
...
- **適用先:** info
- **目的:** ドメインのキュレーションされたベストプラクティスガイダンス。
- **利用者:** 複数
- **値の型:** array
- **値スキーマ:** `{"type": "array", "items": {"type": "object"}}`
...
- **適用先:** info
- **目的:** ドメインで一般的なタスクを達成するための名前付きステップバイステップワークフロー。
- **利用者:** 複数
- **値の型:** array
- **値スキーマ:** `{"type": "array", "items": {"type": "object"}}`
...
- **利用者:** 複数
- **値の型:** object
- **値スキーマ:** `{"type": "object", "additionalProperties": {"type": "string"}}`
...
- **適用先:** info
- **目的:** リソースのネームスペース制約、推奨事項、および分類メタデータを提供します。
- **利用者:** 複数
- **値の型:** object
- **値スキーマ:** `{"type": "object", "properties": {"constraint": {"type": "object"}, "recommendation": {"type": "object"}, "classification": {"type": "object"}}}`
...
- **適用先:** schema
- **目的:** UI/CLIプレゼンテーション用のプロパティの推奨表示順序。
- **利用者:** 複数
- **値の型:** array
- **値スキーマ:** `{"type": "array", "items": {"type": "string"}}`
...
- **適用先:** schema
- **目的:** リソーススキーマの人間が読める表示名（自動生成を上書き）。
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{"type": "string"}`
- **注入元:** scripts/utils/field_metadata_enricher.py
...
- **適用先:** スキーマプロパティ
- **目的:** アップストリームの `description` を補足するエンリッチされたプロパティ説明。
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{"type": "string"}`
- **注入元:** scripts/utils/field_metadata_enricher.py
...
- **適用先:** スキーマプロパティ
- **目的:** このプロパティを必要とするHTTPオペレーション（POST/PUT/...）を一覧表示。
- **利用者:** 複数
- **値の型:** array
- **値スキーマ:** `{"type": "array", "items": {"type": "string"}}`
- **注入元:** scripts/utils/field_metadata_enricher.py
...
- **適用先:** スキーマプロパティ
- **目的:** このプロパティを必要とする名前付き機能の組み合わせを一覧表示。
- **利用者:** 複数
- **値の型:** array
- **値スキーマ:** `{"type": "array", "items": {"type": "string"}}`
- **注入元:** scripts/utils/minimum_configuration_enricher.py
...
- **適用先:** スキーマプロパティ
- **目的:** サーバーデフォルトが最適でない場合のフィールドの推奨本番値。
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{}`
- **注入元:** scripts/utils/default_value_enricher.py
...
- **適用先:** スキーマプロパティ
- **目的:** `oneOf` ブロックにおいて、推奨されるバリアントを示します。
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{"type": "string"}`
- **注入元:** scripts/utils/default_value_enricher.py
...
- **適用先:** スキーマプロパティ
- **目的:** このプロパティと同時に設定できない兄弟プロパティを一覧表示。
- **利用者:** 複数
- **値の型:** array
- **値スキーマ:** `{"type": "array", "items": {"type": "string"}}`
...
- **適用先:** スキーマプロパティ
- **目的:** あるフィールドが別のフィールドの設定を必要とするクロスフィールド依存関係をドキュメント化。
- **利用者:** compile_catalog.py、xcsh CLI
- **値の型:** array
- **値スキーマ:** `{"type": "array", "items": {"type": "object", "properties": {"field": {"type": "string"}, "required": {"type": "boolean"}, "reason": {"type": "string"}}}}`
...
- **適用先:** スキーマプロパティ
- **目的:** ライブAPIプローブまたは静的パターンから派生した数値/文字列制約。
- **利用者:** 複数
- **値の型:** object
- **値スキーマ:** `{"type": "object"}`
...
- **適用先:** スキーマプロパティ
- **目的:** フィールドがそのスコープ内で一意でなければならないかどうかを宣言。
- **利用者:** 複数
- **値の型:** object
- **値スキーマ:** `{"type": "object"}`
...
- **適用先:** operation
- **目的:** 成功のためにオペレーションボディで提供する必要があるフィールドの名前。
- **利用者:** 複数
- **値の型:** array
- **値スキーマ:** `{"type": "array", "items": {"type": "string"}}`
...
- **適用先:** operation
- **目的:** オペレーションの影響範囲を分類（low/medium/high/critical）。
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{"type": "string", "enum": ["low", "medium", "high", "critical"]}`
- **注入元:** scripts/utils/operation_metadata_enricher.py
- **設定による駆動:** config/operation_metadata.yaml
...
- **適用先:** operation
- **目的:** オペレーションの観測可能な副作用（再起動、再構成など）を一覧表示。
- **利用者:** 複数
- **値の型:** array
- **値スキーマ:** `{"type": "array", "items": {"type": "string"}}`
- **注入元:** scripts/utils/operation_metadata_enricher.py
...
- **適用先:** operation
- **目的:** ライブAPIから表出された観測されたレートリミットヘッダー/動作。
- **利用者:** 複数
- **値の型:** object
- **値スキーマ:** `{"type": "object"}`
- **注入元:** scripts/utils/discovery_enricher.py
...
- **適用先:** info
- **目的:** ドメインのトップレベルCLI / UI / ドキュメント / Terraformグルーピングカテゴリ。
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{"type": "string"}`
- **注入元:** scripts/merge_specs.py
...
- **適用先:** info
- **目的:** このドメインでの設定作成における相対的な複雑さの段階。
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{"type": "string", "enum": ["low", "medium", "high"]}`
- **注入元:** scripts/merge_specs.py
- **設定による駆動:** config/domain_patterns.yaml
...
- **適用先:** info
- **目的:** このドメインと一般的に一緒に使用される他のドメインへのクロスリンク。
- **利用者:** 複数
- **値の型:** array
- **値スキーマ:** `{"type": "array", "items": {"type": "string"}}`
- **注入元:** scripts/merge_specs.py
...
- **適用先:** info
- **目的:** レンダリングされたドキュメントのドキュメントセクション/ナビゲーショングルーピングスラグ。
- **利用者:** 複数
- **値の型:** string
- **値スキーマ:** `{"type": "string"}`
- **注入元:** scripts/merge_specs.py
