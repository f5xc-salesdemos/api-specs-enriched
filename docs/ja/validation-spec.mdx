...
バリデーション仕様は、API バリデーション制約、デフォルト値、および設定メタデータを単一の JSON ファイルに統合したものです。この仕様はエンリッチされた OpenAPI 仕様から生成され、それらと共に公開されます。
...
型レベルのバリデーションデフォルトおよびパターンベースのルールです。
...
信頼度スコア付きのフィールド名パターンベースのバリデーションルールです。
...
すべてのデフォルト値を統一された構造でリソースタイプ別に整理したものです。
...
| `advanced_options` | advanced_options オブジェクト内のネストされたデフォルト | API ディスカバリ |
...
エンリッチされた OpenAPI 仕様は、以下の拡張を使用してバリデーションメタデータを埋め込みます：
...
| `x-f5xc-minimum-configuration` | 最小構成のサンプル | スキーマ定義 |
...
| 変更タイプ | バージョンバンプ | 例 |
|-------------|--------------|---------|
| 破壊的な構造変更 | メジャー | フィールドのリネーム、セクションの削除 |
| 新しいバリデーションルールまたはリソース | マイナー | 新しいリソースのデフォルト |
| バグ修正または説明の更新 | パッチ | タイプミスの修正 |

## 調整の優先順位

複数のソースから制約が提供される場合、調整の優先順位は以下の通りです：
...
- [ヘルスチェックの強化](./Enhancements/healthcheck-enhancements/) - ヘルスチェックスキーマのエンリッチメント
- [オリジンプールの強化](./Enhancements/originpool-enhancements/) - オリジンプールスキーマのエンリッチメント
...
| 2.1.0 | 2026-01-18 | 分散していたセクションを置き換える統一されたデフォルト構造 |
