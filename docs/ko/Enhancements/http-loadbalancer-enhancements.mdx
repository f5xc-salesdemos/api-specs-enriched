...
`x-f5xc-minimum-configuration` 확장은 최소한의 실행 가능한 HTTP 로드밸런서를 생성하기 위한 포괄적인 CLI 메타데이터를 제공합니다.
...
|---------|-------------|
| `spec.http` | HTTP 전용 |
| `spec.https` | 수동 인증서를 사용한 HTTPS |
| `spec.https_auto_cert` | 자동 인증서를 사용한 HTTPS |
| `spec.http_https` | HTTP와 HTTPS 모두 |

**참고**: 로드밸런서 유형 OneOf 그룹은 다음과 같이 참조됩니다:
- 구성 파일에서는 `lb_type` (`config/minimum_configs.yaml`의 짧은 형식)
- OpenAPI 스펙 네이티브 확장에서는 `loadbalancer_type` (네이티브 필드 이름)
...
`x-f5xc-server-default: true`로 표시된 필드는 요청에서 생략될 때 F5 XC API 서버에 의해 `default` 값이 적용됩니다. HTTP 로드밸런서는 대부분의 보안 기능과 프로토콜 옵션이 비활성화 또는 안전한 상태로 기본 설정되기 때문에 서버 적용 기본값이 많습니다.

### HTTPS 자동 인증서 기본값

`https_auto_cert`가 지정된 경우, 생략된 필드에 대해 서버가 다음 기본값을 적용합니다:
...
| `tls_config.default_security` | `{}` | object | 강력한 암호를 사용한 TLS 1.2+ |
...
| `disable_trust_client_ip_headers` | `{}` | 클라이언트 IP 헤더 신뢰하지 않음 |

### 기타 서버 적용 기본값

| 필드 | 기본값 | 유형 | 설명 |
|-------|---------------|------|-------------|
| `advertise_on_public_default_vip` | `{}` | object | 공개 기본 VIP에 공고 |
...
| `advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | 로드밸런서 공고 방법 |
...
| `timeouts` | `system_default_timeouts`, `custom_timeouts` | 시스템 기본 또는 사용자 정의 타임아웃 사용 |
| `service_policies_source` | `service_policies_from_namespace`, `active_service_policies` | 네임스페이스의 서비스 정책 또는 활성 목록 |
...
1. F5 XC 콘솔 기본 선택 사항을 관찰합니다 (예: UI에서 어떤 lb_type이 미리 선택되는지)
...
`x-f5xc-constraints`로 표시된 필드는 API 분석에서 발견된 유효성 검사 제약 조건을 포함합니다.
...
| `spec.advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | `advertise_on_public_default_vip` | 로드밸런서 공고 |

## 가이드 워크플로우 통합

보강 파이프라인은 스펙 수준에서 `x-f5xc-guided-workflows` 메타데이터를 추가합니다. 이러한 워크플로우는 AI 어시스턴트와 CLI 도구에서 사용하는 단계별 배포 안내를 제공합니다.

### HTTP 로드밸런서 배포

백엔드 오리진 풀을 포함한 완전히 구성된 HTTP 로드밸런서를 생성하기 위한 5단계 워크플로우입니다 (`config/guided_workflows.yaml`에 정의됨):
...
**사전 요구 사항**: 대상 테넌트의 유효한 네임스페이스, IP 또는 DNS를 통해 접근 가능한 백엔드 애플리케이션, SSL 인증서 (HTTPS의 경우 선택 사항).

### TLS를 포함한 HTTPS 로드밸런서 배포

SSL/TLS 종료를 포함한 HTTPS 로드밸런서를 생성하기 위한 7단계 워크플로우입니다:
...
**사전 요구 사항**: 대상 테넌트의 유효한 네임스페이스, SSL 인증서 및 개인 키, 접근 가능한 백엔드 애플리케이션.

### 워크플로우 활용

다운스트림 도구는 이러한 워크플로우를 사용하여 사용자에게 다단계 배포를 안내합니다:

- **xcsh CLI**: 워크플로우를 대화형 가이드 명령으로 제시합니다 (예: `xcsh deploy http-lb`)
- **VS Code 확장**: 진행 추적과 함께 사이드바 패널에 워크플로우 단계를 표시합니다
- **AI 어시스턴트**: 워크플로우 단계를 순차적으로 따라가며 각 리소스를 생성하고 종속성을 검증합니다
...
최소한의 실행 가능한 구성을 생성하기 위한 포괄적인 메타데이터를 제공합니다. 포함 내용:
- 설명
- 제약 조건이 있는 필수 필드
- 상호 배타적 그룹
- 예제 구성 (YAML, JSON)
...
`true`인 경우, 동반되는 `default` 값이 F5 XC API 서버에 의해 적용됨을 나타냅니다. 이 확장이 있는 필드는 API 요청에서 안전하게 생략할 수 있습니다 -- 서버가 자동으로 기본값을 적용합니다.
...
현재 필드와 상호 배타적인 필드 이름을 나열하며, OneOf 패턴을 나타냅니다.
...
| `config/guided_workflows.yaml` | 가이드 워크플로우 단계 정의 |
...
| 버전 | 날짜 | 변경 사항 |
|---------|------|---------|
| 2.0.46 | 2026-04-18 | 서버 적용 기본값, 가이드 워크플로우 통합 및 확장된 최소 구성 섹션 추가 |
