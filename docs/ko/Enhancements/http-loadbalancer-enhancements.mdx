---
title: HTTP 로드밸런서 개선사항
description: HTTP 로드밸런서 스키마를 위한 보강 메타데이터
sidebar:
  order: 2
i18n:
...
HTTP 로드밸런서 스키마를 위한 보강 메타데이터입니다. 확장 정의에 대해서는 [OpenAPI 확장](../development/#openapi-extensions)을 참조하세요.

## 보강된 스키마

### 패턴

`viewshttp_loadbalancer.*SpecType`과 일치하는 모든 스키마가 보강을 받습니다:
...
`x-f5xc-minimum-configuration` 확장은 최소한으로 실행 가능한 HTTP 로드밸런서를 생성하기 위한 포괄적인 CLI 메타데이터를 제공합니다.
...
- `lb_type` — 구성 파일에서 사용 (`config/minimum_configs.yaml`의 약식 형태)
- `loadbalancer_type` — OpenAPI 스펙 네이티브 확장에서 사용 (네이티브 필드 이름)
...
`x-f5xc-server-default: true`로 표시된 필드는 요청에서 생략될 때 F5 XC API 서버에 의해 `default` 값이 적용됩니다. HTTP 로드밸런서는 대부분의 보안 기능과 프로토콜 옵션이 비활성화 또는 안전한 상태로 기본 설정되기 때문에 서버 적용 기본값이 매우 많습니다.
...
| `tls_config.default_security` | `{}` | object | 강력한 암호화를 사용하는 TLS 1.2+ |
...
| `connection_idle_timeout` | `120000` | integer | 연결 유휴 시간 제한(밀리초, 2분) |
...
| 필드 | 기본값 | 유형 | 설명 |
|-------|---------------|------|-------------|
| `advertise_on_public_default_vip` | `{}` | object | 퍼블릭 기본 VIP에 광고 |
| `round_robin` | `{}` | object | 라운드 로빈 로드밸런싱 알고리즘 |
| `add_location` | `true` | boolean | 응답에 위치 헤더 추가 |
| `system_default_timeouts` | `{}` | object | 시스템 기본 시간 제한 사용 |
...
| `api_discovery` | `disable_api_discovery`, `enable_api_discovery` | API 탐색 |
...
| `malware_protection` | `disable_malware_protection`, `enable_malware_protection` | 악성코드 보호 |
...
| `malicious_user_detection` | `disable_malicious_user_detection`, `enable_malicious_user_detection` | 악의적 사용자 탐지 |
...
| `timeouts` | `system_default_timeouts`, `custom_timeouts` | 시스템 기본 또는 사용자 정의 시간 제한 사용 |
| `service_policies_source` | `service_policies_from_namespace`, `active_service_policies` | 네임스페이스 또는 활성 목록의 서비스 정책 |
| `sensitive_data_policy` | `default_sensitive_data_policy`, `custom_sensitive_data_policy` | 기본 또는 사용자 정의 민감 데이터 정책 사용 |
...
**HTTP 로드밸런서** (아직 구현되지 않음):
```yaml
viewshttp_loadbalancerCreateSpecType:
  x-f5xc-recommended-oneof-variant: null  # 권장 lb_type 변형을 나타냄
```

### 향후 구현

http_loadbalancer에 이 확장을 추가하려면:
1. F5 XC 콘솔 기본 선택 항목을 관찰합니다 (예: UI에서 사전 선택되는 lb_type)
2. `config/discovered_defaults.yaml`에 구성을 추가합니다
3. 보강 파이프라인을 다시 실행하여 확장을 적용합니다
...
이 확장을 사용하면 다운스트림 도구가 사용자에게 구성 옵션을 제시할 때 가장 일반적으로 사용되는 변형을 사전 선택할 수 있습니다.
...
| `spec.https_auto_cert.connection_idle_timeout` | 1000 | 3600000 | 120000 | 연결 유휴 시간 제한 (밀리초, 1초~1시간) |
...
보강 파이프라인은 스펙 수준에서 `x-f5xc-guided-workflows` 메타데이터를 추가합니다. 이러한 워크플로는 AI 어시스턴트와 CLI 도구에서 사용되는 단계별 배포 가이드를 제공합니다.

### HTTP 로드밸런서 배포

백엔드 오리진 풀을 포함하여 완전히 구성된 HTTP 로드밸런서를 생성하기 위한 5단계 워크플로입니다 (`config/guided_workflows.yaml`에 정의됨):

| 단계 | 작업 | 리소스 | 필수 필드 | 선택 사항 |
|------|--------|----------|-----------------|----------|
| 1 | 오리진 풀 생성 | `origin_pool` | `name`, `origin_servers`, `port` | 아니요 |
| 2 | 헬스 체크 구성 | `healthcheck` | `name`, `http_health_check` | 예 |
| 3 | 헬스 체크 연결 | - | - | 예 |
| 4 | HTTP 로드밸런서 생성 | `http_loadbalancer` | `name`, `domains`, `http.port` | 아니요 |
| 5 | 배포 확인 | - | - | 아니요 |

**전제 조건**: 대상 테넌트의 유효한 네임스페이스, IP 또는 DNS를 통해 접근 가능한 백엔드 애플리케이션, SSL 인증서(HTTPS의 경우 선택 사항).

### TLS를 포함한 HTTPS 로드밸런서 배포

SSL/TLS 종료를 포함한 HTTPS 로드밸런서를 생성하기 위한 7단계 워크플로입니다:

| 단계 | 작업 | 리소스 | 필수 필드 | 선택 사항 |
|------|--------|----------|-----------------|----------|
| 1 | SSL 인증서 업로드 | `certificate` | `name`, `certificate_chain`, `private_key` | 아니요 |
| 2 | 오리진 풀 생성 | `origin_pool` | `name`, `origin_servers` | 아니요 |
| 3 | 헬스 체크 구성 | `healthcheck` | - | 예 |
| 4 | WAF 정책 구성 | `app_firewall` | - | 예 |
| 5 | HTTPS 로드밸런서 생성 | `http_loadbalancer` | `name`, `domains`, `https.tls_parameters` | 아니요 |
| 6 | WAF 정책 연결 | - | - | 예 |
| 7 | HTTPS 배포 확인 | - | - | 아니요 |

**전제 조건**: 대상 테넌트의 유효한 네임스페이스, SSL 인증서 및 개인 키, 접근 가능한 백엔드 애플리케이션.

### 워크플로 사용

다운스트림 도구는 이러한 워크플로를 사용하여 다단계 배포 과정에서 사용자를 안내합니다:

- **xcsh CLI**: 워크플로를 대화형 가이드 명령으로 제시합니다 (예: `xcsh deploy http-lb`)
- **VS Code 확장**: 진행 상황 추적과 함께 사이드바 패널에 워크플로 단계를 표시합니다
- **AI 어시스턴트**: 워크플로 단계를 순차적으로 따라가며 각 리소스를 생성하고 종속성을 검증합니다
...
최소 실행 가능 구성을 생성하기 위한 포괄적인 메타데이터를 제공합니다. 포함 항목:
- 설명
- 제약 조건이 있는 필수 필드
- 상호 배타적 그룹
- 예제 구성 (YAML, JSON)
...
배열 크기 제한, 정수 범위, 고유성 요구사항 및 탐색 메타데이터를 포함한 유효성 검사 제약 조건을 제공합니다.
...
플래그를 사용하여 컨텍스트별 요구사항을 나타냅니다: `minimum_config`, `create`, `update`, `read`.
...
| 파일 | 내용 |
|------|---------|
| `docs/specifications/api/virtual.json` | 보강이 포함된 모든 http_loadbalancer 스키마 |
...
- [Healthcheck 개선사항](./healthcheck-enhancements/) - Healthcheck 스키마 보강 (`x-f5xc-recommended-oneof-variant` 포함)
- [오리진 풀 개선사항](./originpool-enhancements/) - 오리진 풀 스키마 보강
- [제약 조건 메타데이터 가이드](../constraint-metadata/) - 상세 제약 조건 유효성 검사 문서
