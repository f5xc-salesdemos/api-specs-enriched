...
origin_pool 관련 스키마를 위한 보강 메타데이터입니다. 확장 정의에 대해서는 [OpenAPI 확장](../development/#openapi-extensions)을 참조하십시오.

## 보강된 스키마

### 패턴

`origin_pool.*SpecType`과 일치하는 모든 스키마가 보강을 받습니다:
...
| `disable_outlier_detection` | `{}` | object | 이상치 탐지 비활성화 |
...
F5 XC 웹 UI는 필드가 생략될 때 API가 적용하는 것과 다른 값을 미리 선택합니다.

| 필드 | UI 미리 선택 값 | 서버 적용 값 | 참고 |
|-------|-----------------|----------------|------|
...
| `LEAST_REQUEST` | 활성 요청이 가장 적은 엔드포인트가 선택됨 | |
| `RING_HASH` | 엔드포인트 이름의 링 해시를 사용한 일관된 해싱 | |
| `RANDOM` | 무작위 정상 엔드포인트 선택 | |
| `LB_OVERRIDE` | 상위 로드 밸런서에서 해시 정책을 상속 | UI 기본값 |
...
상호 배타적인 필드 그룹입니다. 각 그룹에서 하나의 필드만 지정할 수 있습니다:
...
| 이상치 탐지 | `disable_outlier_detection`, `outlier_detection` | `disable_outlier_detection` |
...
| 10 | 이상치 탐지 | `spec.advanced_options.[disable_outlier_detection\|outlier_detection]` | OneOf | `disable_outlier_detection` |
...
| `docs/specifications/api/openapi.json` | 모든 스키마가 포함된 통합 명세 |

### validation.json 구조

```text
defaults.resources.origin_pool
├── server_applied      # x-f5xc-server-default: true 가 지정된 필드
├── recommended         # x-f5xc-recommended-value 가 지정된 필드
├── advanced_options    # 중첩된 advanced_options 기본값
├── oneof_choices       # OneOf 기본 선택
└── ui_vs_server        # UI와 서버 간 불일치
```

## 오리진 서버 타입

오리진 서버는 다양한 타입 변형을 사용하여 지정할 수 있습니다. 타입에 따라 필요한 추가 필드가 결정됩니다.

### 구현된 타입

| 타입 | 설명 | 필수 필드 | 상태 |
|------|-------------|-----------------|--------|
| `public_name` | 공용 DNS 이름을 가진 오리진 서버 | `dns_name` | ✅ 완료 |
| `public_ip` | 공용 IP 주소를 가진 오리진 서버 | `ip` (IPv4) | ✅ 완료 |
...
## TODO: 사이트 종속 오리진 서버 타입

다음 오리진 서버 타입은 구현 전에 "site" 리소스 로직 개발이 필요합니다:

| 타입 | 설명 | 필수 필드 | 상태 |
|------|-------------|-----------------|--------|
| `private_ip` | 사설/공용 IP 및 사이트 정보를 가진 오리진 서버 | `ip`, `site_locator`, `network_choice`, `snat_pool` | 🔲 사이트 로직 대기 중 |
...
이러한 타입을 완전히 문서화하기 전에 다음 사항이 필요합니다:

1. **사이트 리소스 디스커버리** - `site` 리소스 스키마 및 site_locator 패턴 문서화
2. **네트워크 선택 열거형** - `network_choice` 제약 값 문서화 (site_local_inside, site_local_outside 등)
3. **SNAT 풀 옵션** - SNAT 풀 구성 패턴 문서화
4. **교차 리소스 참조** - origin_pool이 site 리소스를 참조하는 방식 정의
...
- [유효성 검사 명세](../validation-spec/) - validation.json 형식 및 구조
...
| 버전 | 날짜 | 변경 사항 |
|---------|------|---------|
| 2.1.2 | 2026-01-18 | public_name/public_ip를 포함한 오리진 서버 타입 섹션 추가; 사이트 종속 타입에 대한 TODO 마커 |
| 2.1.1 | 2026-01-18 | 순수 API 레퍼런스로 재작성; 다운스트림 코드 예시 제거 |
| 2.1.0 | 2026-01-18 | validation.json의 통합 기본값 구조로 업데이트 |
| 2.0.33 | 2026-01-17 | 오리진 풀 개선 사항 초기 문서화 |
