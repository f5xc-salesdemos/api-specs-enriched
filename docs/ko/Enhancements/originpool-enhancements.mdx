...
`origin_pool.*SpecType`과 일치하는 모든 스키마는 보강을 수신합니다:
...
| `spec.origin_servers` | Create | 최소 1개 항목 |
| `spec.port` | Create | 1-65535, 서버 기본값 없음 |
...
`advanced_options`가 지정되지 않은 경우, 서버는 이 값들이 설정된 것처럼 동작합니다:
...
| `origin_servers[].public_name.refresh_interval` | `0` | 시스템 기본 DNS 새로고침 사용 |
...
|-------|-----------------|----------------|------|
| `loadbalancer_algorithm` | `LB_OVERRIDE` | `ROUND_ROBIN` | 필드가 생략될 때 UI 생성 리소스와 API 생성 리소스가 다름 |
...
F5 XC 웹 콘솔은 오리진 풀에 대해 15개의 구성 옵션을 제공합니다:
...
| `docs/specifications/api/openapi.json` | 모든 스키마가 포함된 통합 사양 |
...
## TODO: 사이트 의존 오리진 서버 유형

다음 오리진 서버 유형은 구현 전에 "site" 리소스 로직 개발이 필요합니다:

| 유형 | 설명 | 필수 필드 | 상태 |
|------|-------------|-----------------|--------|
| `private_ip` | 프라이빗/퍼블릭 IP 및 사이트 정보가 있는 오리진 서버 | `ip`, `site_locator`, `network_choice`, `snat_pool` | 🔲 사이트 로직 대기 중 |
| `private_name` | DNS 이름 및 사이트 정보가 있는 오리진 서버 | `dns_name`, `site_locator`, `network_choice`, `snat_pool` | 🔲 사이트 로직 대기 중 |
| `k8s_service` | 사이트 정보가 있는 Kubernetes 서비스 | `service_name`, `site_locator`, `network_choice` | 🔲 사이트 로직 대기 중 |
| `consul_service` | 사이트 정보가 있는 HashiCorp Consul 서비스 | `service_name`, `site_locator`, `network_choice` | 🔲 사이트 로직 대기 중 |

### 구현 요구 사항

이러한 유형을 완전히 문서화하기 전에 필요한 작업:
...
4. **교차 리소스 참조** - origin_pool이 사이트 리소스를 참조하는 방식 정의
...
- [검증 사양](../validation-spec/) - validation.json 형식 및 구조
- [헬스체크 향상](./healthcheck-enhancements/) - 헬스체크 스키마 보강

## 변경 이력

| 버전 | 날짜 | 변경 사항 |
|---------|------|---------|
| 2.1.2 | 2026-01-18 | public_name/public_ip가 포함된 오리진 서버 유형 섹션 추가; 사이트 의존 유형에 대한 TODO 마커 |
| 2.1.1 | 2026-01-18 | 순수 API 레퍼런스로 재작성; 다운스트림 코드 예시 제거 |
| 2.1.0 | 2026-01-18 | validation.json의 통합 기본값 구조로 업데이트 |
| 2.0.33 | 2026-01-17 | 오리진 풀 향상 초기 문서 |
