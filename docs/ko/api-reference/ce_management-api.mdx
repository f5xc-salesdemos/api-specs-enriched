...
이미지 다운로드 및 업그레이드 전 유효성 검사를 포함한 토큰 기반 프로비저닝. 플릿 그룹화를 통해 분산된 위치에 걸쳐 일괄 작업을 수행할 수 있습니다.
...
| GET | `/api/config/namespaces/&#123;namespace&#125;/fleets` | 플릿 목록. |
...
| GET | `/api/config/namespaces/&#123;namespace&#125;/network_interfaces` | 네트워크 인터페이스 목록. |
...
| POST | `/api/register/namespaces/&#123;namespace&#125;/listregistrationsbystate` | 상태별 등록 목록. |
| POST | `/api/register/namespaces/&#123;namespace&#125;/registration/&#123;name&#125;/approve` | 등록 승인. |
| GET | `/api/register/namespaces/&#123;namespace&#125;/registrations` | 등록 목록. |
...
| GET | `/api/register/namespaces/&#123;namespace&#125;/registrations_by_site/&#123;site_name&#125;` | 사이트별 등록 목록. |
...
| GET | `/api/config/namespaces/&#123;namespace&#125;/usb_policys` | USB 정책 목록. |
