...
AWS, Azure, GCP VPC와 트랜짓 게이트웨이 통합. 리전 간 정책 적용을 위한 레이블 기반 선택.
...
- 클라우드 공급자(AWS, Azure, GCP) 간 F5 XC 배포
...
- **virtual_site**: 레이블 셀렉터를 사용한 사이트의 논리적 그룹핑
...
| POST | `/api/data/namespaces/system/site/&#123;name&#125;/status` | 사이트 존재 확인. |
...
| GET | `/api/config/namespaces/&#123;namespace&#125;/virtual_k8ss` | Virtual Kubernetes 목록. |
