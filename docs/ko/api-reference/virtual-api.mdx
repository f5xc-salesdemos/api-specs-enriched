...
라우팅 규칙을 사용한 리전 간 트래픽 분산. 헬스 체크 및 장애 조치 정책.
...
- **http_loadbalancer**: 애플리케이션 트래픽 분산을 위한 레이어 7 HTTP/HTTPS 로드 밸런서
...
| POST | `/api/data/namespaces/&#123;namespace&#125;/enhanced_firewall_policy/hits` | 향상된 방화벽 정책 히트. |
...
| POST | `/api/config/namespaces/system/malware_protection/addon/subscribe` | 악성코드 보호 구독. |
| POST | `/api/config/namespaces/system/malware_protection/addon/unsubscribe` | 악성코드 보호 구독 해제. |
...
| POST | `/api/ml/data/namespaces/&#123;namespace&#125;/virtual_hosts/&#123;name&#125;/api_endpoints/summary/calls_by_response_code` | 가상 호스트의 전체 API 호출 조회. |
| POST | `/api/ml/data/namespaces/&#123;namespace&#125;/virtual_hosts/&#123;name&#125;/api_endpoints/summary/top_active` | 가상 호스트의 상위 API 엔드포인트 조회. |
| POST | `/api/ml/data/namespaces/&#123;namespace&#125;/virtual_hosts/&#123;name&#125;/api_endpoints/summary/top_sensitive_data` | 가상 호스트의 민감 데이터 요약 조회. |
