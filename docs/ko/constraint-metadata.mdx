...
description: 결정론적 AI 생성 및 유효성 검사를 위한 종합 제약 조건 메타데이터
sidebar:
  order: 2
i18n:
...
**목적**: 결정론적 AI 생성, CLI 유효성 검사 및 Terraform 스키마 적용을 위한 종합 제약 조건 메타데이터.

**확장**: `x-f5xc-constraints`
**버전**: 3.3.0
**상태**: 프로덕션
**소비자**: AI 어시스턴트, CLI 도구, Terraform 프로바이더, IDE 확장, API 유효성 검사기
...
5. [유효성 검사 예제](#validation-examples)
...
- **3계층 조정**: EXISTING > DISCOVERY > INFERRED
...
| `category` | string | 그룹화 및 리포팅을 위한 제약 조건 카테고리 |
| *(제약 조건 키)* | various | 최상위에 평면화된 유형별 제약 조건 키 (예: `minLength`, `maxItems`, `minimum`) |
...
- DNS 레이블 이름은 1-63자의 소문자 영숫자 문자여야 합니다
- 하이픈은 허용되지만 시작/끝에는 사용할 수 없습니다 (RFC 1123)
- 예: `my-service`, `api-gateway`, `lb-prod-01`

**이메일 주소** (`format: "email"`):
- 이메일 주소는 RFC 5322에 따라 유효해야 합니다
- 최대 254자
- 예: `admin@example.com`, `api+webhook@service.io`

**FQDN** (`format: "fqdn"`):
- 정규화된 도메인 이름은 1-253자여야 합니다
- 점으로 구분된 DNS 레이블
- 예: `api.example.com`, `service.production.internal`

**URL** (`format: "url"`):
- URL은 RFC 3986에 따라 유효한 URI여야 합니다
- 스킴(http/https)을 포함해야 합니다
- 예: `https://api.example.com/v1`, `http://localhost:8080`

**IPv4 주소** (`format: "ipv4"`):
- IPv4 주소는 점으로 구분된 10진수 표기법이어야 합니다
...
- 예: `2026-01-19T12:00:00Z`, `2026-01-19T12:00:00+00:00`
...
**오리진 풀**:
- 오리진 풀 배열은 최소 1개, 최대 50개 항목이 필요합니다
- 각 항목은 고유해야 합니다
...
**유효하지 않은 값**:
- ❌ `My-Service` (대문자 허용되지 않음)
- ❌ `-my-service` (시작에 하이픈)
- ❌ `my-service-` (끝에 하이픈)
- ❌ `my_service` (밑줄 허용되지 않음)
...
3. **INFERRED** (신뢰도: 0.50-0.95)
   - 필드 이름에서 패턴 매칭됨
   - 가장 낮은 우선순위, 디스커버리 또는 명시적 제약 조건에 의해 재정의됨
...
func generateTerraformSchema(property map[string]interface{}) *schema.Schema {
    ...
    // 제약 조건 키는 최상위에 위치 (중첩된 "string" 래퍼 없음)
...
- **이슈**: 제약 조건 정확도 문제는 https://github.com/f5xc-salesdemos/api-specs-enriched/issues 에 보고해 주세요
