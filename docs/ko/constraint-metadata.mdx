...
| *(제약 조건 키)* | various | 최상위 수준에 평탄화된 유형별 제약 조건 키 (예: `minLength`, `maxItems`, `minimum`) |
...
| 형식 | 설명 | 예시 |
|--------|-------------|---------|
| `dns-label` | RFC 1123 DNS 라벨 (1-63자) | `my-service` |
...
**DNS 라벨** (`format: "dns-label"`):
- DNS 라벨 이름은 1-63자의 소문자 영숫자 문자여야 합니다
...
| 0.50-0.69 | **낮은 신뢰도** | 정보 제공용, 사용자 유효성 검사 필요 |
...
- 신뢰도 점수 >= 0.9
- 패턴이 디스커버리 데이터에 대해 검증되었거나 또는
- 패턴이 잘 확립된 표준(RFC, ISO 등)과 일치
...
|---------|------------------|-----------------|---------|
| DNS 라벨 | `\bname$` | String (1-63자, dns-label) | `my-service` |
...
- **이슈**: 제약 조건 정확도 문제는 https://github.com/f5xc-salesdemos/api-specs-enriched/issues 에서 보고해 주세요
- **문서**: 전체 API 문서는 https://f5xc-salesdemos.github.io/api-specs-enriched/ 에서 확인하세요
- **MCP 통합**: MCP 서버 통합 예시는 `f5xc-api-mcp` 리포지토리를 참조하세요
