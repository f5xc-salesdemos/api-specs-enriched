...
description: 결정론적 AI 생성 및 유효성 검증을 위한 포괄적 제약 조건 메타데이터
sidebar:
  order: 2
i18n:
...
**목적**: 결정론적 AI 생성, CLI 유효성 검증 및 Terraform 스키마 적용을 위한 포괄적 제약 조건 메타데이터.

**확장자**: `x-f5xc-constraints`
**버전**: 3.3.0
**상태**: 프로덕션
**소비자**: AI 어시스턴트, CLI 도구, Terraform 프로바이더, IDE 확장 프로그램, API 유효성 검증기
...
5. [유효성 검증 예제](#validation-examples)
...
`x-f5xc-constraints` 확장은 필드 유효성 검증 제약 조건에 대한 결정론적 지식을 제공하여 다음을 가능하게 합니다:

- **AI 어시스턴트**: 제약 조건 세부 사항에 대한 사용자 입력 없이 유효한 구성 생성
- **CLI 도구**: 정확한 입력 힌트, 유효성 검증 및 오류 메시지 제공
- **Terraform**: 적용 시점이 아닌 계획 시점에서 제약 조건 적용
- **IDE 확장 프로그램**: 실시간 유효성 검증 및 자동 완성 제안
- **API 유효성 검증기**: API 오류를 줄이기 위한 제출 전 유효성 검증

### 커버리지 통계

- 모든 F5 XC API 스펙에 걸쳐 **9,851개의 제약 조건**
- **50개 이상의 패턴 유형** (문자열, 배열, 숫자)
- 높은 우선순위 필드(이름, 포트, 식별자)에 대해 **95% 신뢰도**
...
| `deterministic` | boolean | AI 생성을 위한 높은 신뢰도 플래그 (confidence >= 0.9) |
| `category` | string | 그룹화 및 보고를 위한 제약 조건 카테고리 |
| *(constraint keys)* | various | 최상위 수준에 평탄화된 유형별 제약 조건 키 (예: `minLength`, `maxItems`, `minimum`) |
| `metadata` | object | 소스 추적, 신뢰도, 유효성 검증 타임스탬프 |
...
| `fqdn` | 정규화된 도메인 이름 | `api.example.com` |
...
| `base64` | Base64 인코딩 문자열 | `SGVsbG8gV29ybGQ=` |
...
- 하이픈은 허용되지만 시작/끝에는 사용할 수 없습니다 (RFC 1123)
- 예제: `my-service`, `api-gateway`, `lb-prod-01`

**이메일 주소** (`format: "email"`):
- 이메일 주소는 RFC 5322에 따라 유효해야 합니다
- 최대 254자
- 예제: `admin@example.com`, `api+webhook@service.io`

**FQDN** (`format: "fqdn"`):
- 정규화된 도메인 이름은 1-253자여야 합니다
- 점으로 구분된 DNS 레이블
- 예제: `api.example.com`, `service.production.internal`

**URL** (`format: "url"`):
- URL은 RFC 3986에 따라 유효한 URI여야 합니다
- 스킴(http/https)을 포함해야 합니다
- 예제: `https://api.example.com/v1`, `http://localhost:8080`

**IPv4 주소** (`format: "ipv4"`):
- IPv4 주소는 점으로 구분된 십진수 표기법이어야 합니다
- 예제: `192.168.1.1`, `10.0.0.1`
...
- 예제: `2026-01-19T12:00:00Z`, `2026-01-19T12:00:00+00:00`
...
**타임아웃**:
- 타임아웃은 1-3600초(최대 1시간)여야 합니다

### 배열 제약 조건

**오리진 풀**:
- 오리진 풀 배열은 최소 1개, 최대 50개의 항목이 필요합니다
- 각 항목은 고유해야 합니다

**태그**:
- 태그 배열은 0-100개의 항목을 허용합니다
- 항목이 고유할 필요는 없습니다
...
**유효하지 않은 값**:
- ❌ `My-Service` (대문자 허용 안 됨)
- ❌ `-my-service` (시작 부분에 하이픈)
- ❌ `my-service-` (끝 부분에 하이픈)
- ❌ `my_service` (밑줄 허용 안 됨)
- ❌ `this-is-a-very-long-name-that-exceeds-the-sixty-three-character-limit` (너무 김)
...
| 0.70-0.89 | **중간 신뢰도** | 권고 수준, 사용자 확인 권장 |
| 0.50-0.69 | **낮은 신뢰도** | 정보 제공 목적, 사용자 유효성 검증 필요 |
| < 0.50 | **매우 낮음** | 제약 조건에 포함되지 않음 |

### 결정론적 플래그

`deterministic` 불리언 플래그는 다음 조건에서 설정됩니다:
- 신뢰도 점수 >= 0.9
- 디스커버리 데이터에 대해 패턴이 검증되었거나
- 잘 확립된 표준(RFC, ISO 등)과 패턴이 일치하는 경우
...
1. **EXISTING** (신뢰도: 1.0)
   - 원본 OpenAPI 스펙에 명시적으로 설정됨
   - 절대 재정의되지 않음

2. **DISCOVERY** (신뢰도: 0.99)
   - 라이브 F5 XC API 응답에서 추출됨
   - x-ves-validation-rules의 유효성 검증 규칙 포함
   - 추론된 제약 조건을 재정의함

3. **INFERRED** (신뢰도: 0.50-0.95)
   - 필드 이름에서 패턴 매칭됨
   - 가장 낮은 우선순위, 디스커버리 또는 명시적 제약에 의해 재정의됨

### 디스커버리 데이터 소스

디스커버리 제약 조건은 다음에서 추출됩니다:

1. **x-ves-validation-rules**: 네이티브 F5 유효성 검증 확장
...
3. **응답 유효성 검증**: 유효한 값을 보여주는 성공적인 API 응답
   - 관찰된 값 범위
   - 실제 형식 패턴
...
| 패턴 | 필드 이름 정규식 | 제약 조건 유형 | 예제 |
|---------|------------------|-----------------|---------|
| DNS 레이블 | `\bname$` | String (1-63자, dns-label) | `my-service` |
| 네임스페이스 | `\bnamespace$` | String (1-63자, dns-label) | `production` |
| 포트 | `\bport$` | Number (1-65535) | `443` |
| VLAN ID | `\b(vlan)?_?id$` | Number (1-4094) | `100` |
| UUID | `\buuid$` | String (36자, uuid 형식) | `550e8400-...` |
| 이메일 | `\bemail$` | String (최대 254자, email) | `user@example.com` |
| 타임스탬프 | `\b(timestamp\|created_at\|updated_at)$` | String (ISO 8601) | `2026-01-19T12:00:00Z` |

### 중간 신뢰도 패턴 (0.80-0.94)

| 패턴 | 필드 이름 정규식 | 제약 조건 유형 | 예제 |
|---------|------------------|-----------------|---------|
| 사용자 이름 | `\b(user\|username)$` | String (1-64자, 영숫자) | `admin` |
| 표시 이름 | `\b(display_?)?name$` | String (1-256자, 자유 텍스트) | `My Service` |
| 설명 | `\b(comment\|description\|note)$` | String (최대 4096자) | `Service description` |
| 경로 | `\bpath$` | String (최대 2048자) | `/api/v1/resources` |

---

## 지원 및 피드백

- **이슈**: 제약 조건 정확성 문제는 https://github.com/f5xc-salesdemos/api-specs-enriched/issues 에서 보고해 주세요
- **문서**: 전체 API 문서는 https://f5xc-salesdemos.github.io/api-specs-enriched/ 에서 확인할 수 있습니다
- **MCP 통합**: MCP 서버 통합 예제는 `f5xc-api-mcp` 리포지토리를 참조하세요
