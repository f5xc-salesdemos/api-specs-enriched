...
**버전**: 3.3.0
**상태**: 프로덕션
**사용자**: AI 어시스턴트, CLI 도구, Terraform 프로바이더, IDE 확장, API 유효성 검사기
...
- 높은 우선순위 필드(이름, 포트, 식별자)에 대한 **95% 신뢰도**
...
| `deterministic` | boolean | AI 생성을 위한 높은 신뢰도 플래그 (신뢰도 >= 0.9) |
...
| `fqdn` | 완전한 정규화된 도메인 이름 | `api.example.com` |
...
**오리진 풀**:
- 오리진 풀 배열은 최소 1개 항목, 최대 50개 항목을 필요로 합니다
- 각 항목은 고유해야 합니다
...
| 0.50-0.69 | **낮은 신뢰도** | 정보 제공 목적만, 사용자 유효성 검사 필요 |
...
- 신뢰도 점수 >= 0.9
- 패턴이 디스커버리 데이터에 대해 유효성 검사된 경우 또는
- 패턴이 잘 확립된 표준(RFC, ISO 등)과 일치하는 경우
...
3. **INFERRED** (신뢰도: 0.50-0.95)
   - 필드 이름에서 패턴 매칭됨
   - 가장 낮은 우선순위, 디스커버리 또는 명시적 설정에 의해 재정의됨
...
- **이슈**: https://github.com/f5xc-salesdemos/api-specs-enriched/issues 에서 제약 조건 정확도 문제 보고
- **문서**: https://f5xc-salesdemos.github.io/api-specs-enriched/ 에서 전체 API 문서 확인
- **MCP 통합**: MCP 서버 통합 예시는 `f5xc-api-mcp` 리포지토리 참조
