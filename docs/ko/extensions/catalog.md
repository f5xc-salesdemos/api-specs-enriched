---
title: 보강 확장(Extension) 카탈로그
description: 보강된 OpenAPI 사양의 모든 x-* 확장에 대한 단일 정보 소스
i18n:
  sourceHash: 7fde0afb4dac
  translator: machine
---

# 보강 확장(Extension) 카탈로그

`docs/specifications/api/*.json`에 나타나는 모든 `x-*` 확장에 대한 단일 정보 소스입니다.
`scripts/utils/extension_constants.py`와의 동일성은
`tests/test_extension_catalog.py`에 의해 강제됩니다.

여기에는 세 가지 클래스의 확장이 문서화되어 있습니다:

- **여기서 주입됨** — 보강기(enricher)가 추가하는 확장(`x-f5xc-*` 및
  `x-ves-cli-*` / `x-ves-field-*` / `x-ves-operation-*` / 디스커버리
  변형). 다운스트림 도구가 소비해야 하는 확장입니다.
- **업스트림 패스스루** — F5가 소스 사양에서 발행하고
  변경 없이 보존하는 확장(`x-ves-proto-*`, `x-displayname` 등).
  투명성을 위해 문서화되었지만 이 저장소에서 제어하지 않습니다.
- **향후 주입 예정** — 아직 발행되지 않았으며, 보강기가 생성하기 시작하는
  시점에 여기 문서화됩니다(초기 작성 시점에는 해당 없음).

## 항목 스키마

아래의 모든 항목은 정확히 다음과 같은 형태를 가집니다. `tests/test_extension_catalog.py`의
동일성 테스트는 `### x-name` 헤더가 존재하고
...
- **적용 위치:** info
- **목적:** 보강된 사양의 CLI 도메인 슬러그(예: `http_loadbalancer`)를 식별합니다.
- **소비자:** CLI
- **값 타입:** string
- **값 스키마:** `{"type": "string"}`
...
- **값 스키마:** `{"type": "object"}`
- **주입 주체:** scripts/merge_specs.py
- **설정 근거:** config/cli_metadata.yaml
...
- **적용 위치:** info
- **목적:** 보강된 파일이 빌드된 업스트림 소스 사양의 타임스탬프.
- **소비자:** multiple
- **값 타입:** string
...
- **적용 위치:** info
- **목적:** 파이프라인에 의해 보강된 사양에 찍히는 시맨틱 버전.
- **소비자:** multiple
- **값 타입:** string
- **값 스키마:** `{"type": "string"}`
- **주입 주체:** scripts/merge_specs.py
...
- **적용 위치:** info
- **목적:** 각 도메인 사양에 적용되는 브랜딩/용어 용어집 블록.
- **소비자:** multiple
- **값 타입:** object
- **값 스키마:** `{"type": "object"}`
- **주입 주체:** scripts/utils/branding.py
- **설정 근거:** config/branding.yaml
...
- **적용 위치:** info
- **목적:** 이 도메인에 대한 호스팅된 API 참조 문서 페이지의 URL.
- **소비자:** multiple
- **값 타입:** string
- **값 스키마:** `{"type": "string", "format": "uri"}`
- **주입 주체:** scripts/utils/external_docs_enricher.py
- **설정 근거:** none (도메인 이름에서 파생)
...
- **적용 위치:** info
- **목적:** 도메인에서 일반적인 작업을 수행하기 위한 이름이 지정된 단계별 워크플로.
- **소비자:** multiple
- **값 타입:** array
- **값 스키마:** `{"type": "array", "items": {"type": "object"}}`
- **주입 주체:** scripts/utils/guided_workflow_enricher.py
- **설정 근거:** config/guided_workflows.yaml
...
- **설정 근거:** config/minimum_configs.yaml
...
- **적용 위치:** info
- **목적:** 리소스에 대한 네임스페이스 제약 조건, 권장 사항 및 분류 메타데이터를 제공합니다.
- **소비자:** multiple
- **값 타입:** object
...
- **설정 근거:** config/namespace_profile.yaml
...
- **적용 위치:** schema
- **목적:** 리소스 스키마의 사람이 읽을 수 있는 표시 이름(자동 생성을 재정의).
- **소비자:** multiple
- **값 타입:** string
- **값 스키마:** `{"type": "string"}`
- **주입 주체:** scripts/utils/field_metadata_enricher.py
...
- **값 스키마:** `{"type": "string"}`
- **주입 주체:** scripts/utils/field_metadata_enricher.py
- **설정 근거:** config/field_descriptions.yaml
...
- **적용 위치:** schema property
- **목적:** 업스트림 protobuf `ves.io.schema.rules`에서 파생된 선언적 유효성 검사 규칙.
- **소비자:** multiple
- **값 타입:** object
- **값 스키마:** `{"type": "object"}`
- **주입 주체:** scripts/utils/validation_enricher.py
- **설정 근거:** config/validation_rules.yaml
...
- **설정 근거:** config/resource_examples.yaml
...
- **적용 위치:** schema property
- **목적:** 이 속성을 요구하는 HTTP 작업(POST/PUT/...)을 나열합니다.
- **소비자:** multiple
- **값 타입:** array
- **값 스키마:** `{"type": "array", "items": {"type": "string"}}`
- **주입 주체:** scripts/utils/field_metadata_enricher.py
...
- **적용 위치:** schema property
- **목적:** 이 속성을 요구하는 명명된 기능 조합을 나열합니다.
- **소비자:** multiple
- **값 타입:** array
- **값 스키마:** `{"type": "array", "items": {"type": "string"}}`
- **주입 주체:** scripts/utils/minimum_configuration_enricher.py
...
- **설정 근거:** config/discovered_defaults.yaml
...
- **적용 위치:** schema property
- **목적:** `oneOf` 블록에서 어떤 변형이 권장되는지 나타냅니다.
- **소비자:** multiple
- **값 타입:** string
- **값 스키마:** `{"type": "string"}`
- **주입 주체:** scripts/utils/default_value_enricher.py
...
- **적용 위치:** schema property
- **목적:** 한 필드가 다른 필드의 설정을 요구하는 교차 필드 종속성을 문서화합니다.
...
- **설정 근거:** config/minimum_configs.yaml (dependencies 섹션)
...
- **설정 근거:** config/constraint_patterns.yaml
...
- **적용 위치:** operation
- **목적:** 성공을 위해 제공해야 하는 작업 본문 필드의 이름을 지정합니다.
- **소비자:** multiple
- **값 타입:** array
- **값 스키마:** `{"type": "array", "items": {"type": "string"}}`
- **주입 주체:** scripts/utils/operation_metadata_enricher.py
- **설정 근거:** config/operation_metadata.yaml
...
- **적용 위치:** operation
- **목적:** 작업의 영향 범위를 분류합니다(low/medium/high/critical).
- **소비자:** multiple
- **값 타입:** string
...
- **적용 위치:** operation
- **목적:** 작업의 관측 가능한 부수 효과를 나열합니다(재시작, 재구성 등).
- **소비자:** multiple
- **값 타입:** array
- **값 스키마:** `{"type": "array", "items": {"type": "string"}}`
- **주입 주체:** scripts/utils/operation_metadata_enricher.py
...
- **적용 위치:** operation
- **목적:** 디스커버리 중 이 작업에 대해 경험적으로 측정된 응답 시간.
- **소비자:** multiple
- **값 타입:** object
- **값 스키마:** `{"type": "object"}`
- **주입 주체:** scripts/utils/discovery_enricher.py
- **설정 근거:** config/discovery_enrichment.yaml
...
- **적용 위치:** operation
- **목적:** 라이브 디스커버리 중 관측된 오류 응답 카탈로그(샘플 페이로드 포함).
- **소비자:** multiple
- **값 타입:** array
- **값 스키마:** `{"type": "array", "items": {"type": "object"}}`
- **주입 주체:** scripts/utils/discovery_enricher.py
...
- **적용 위치:** info
- **목적:** 도메인에 대한 최상위 CLI / UI / 문서 / Terraform 그룹화 카테고리.
- **소비자:** multiple
- **값 타입:** string
- **값 스키마:** `{"type": "string"}`
- **주입 주체:** scripts/merge_specs.py
- **설정 근거:** config/domain_patterns.yaml
...
- **적용 위치:** info
- **목적:** 높은 수준의 주의가 필요한 리소스(프로덕션 핵심).
- **소비자:** multiple
- **값 타입:** array
- **값 스키마:** `{"type": "array", "items": {"type": "string"}}`
- **주입 주체:** scripts/merge_specs.py
- **설정 근거:** config/critical_resources.yaml
...
- **적용 위치:** info
- **목적:** 짧은(~60자) 도메인 설명. 긴 설명에 대해서는 속성 수준에서도 적용됩니다.
- **소비자:** multiple
- **값 타입:** string
- **값 스키마:** `{"type": "string"}`
- **주입 주체:** scripts/utils/property_description_short_enricher.py
- **설정 근거:** config/property_description_short.yaml
...
- **적용 위치:** info
- **목적:** 중간 길이(~150자) 도메인 설명. 속성 수준에서도 적용됩니다.
- **소비자:** multiple
- **값 타입:** string
- **값 스키마:** `{"type": "string"}`
- **주입 주체:** scripts/utils/property_description_short_enricher.py
...
- **설정 근거:** config/domain_descriptions.yaml
...
- **적용 위치:** info
- **목적:** 이 도메인에서 구성을 작성하기 위한 상대적 복잡성 등급.
- **소비자:** multiple
- **값 타입:** string
...
- **적용 위치:** info
- **목적:** 이 도메인과 함께 일반적으로 사용되는 다른 도메인으로의 교차 링크.
- **소비자:** multiple
- **값 타입:** array
- **값 스키마:** `{"type": "array", "items": {"type": "string"}}`
- **주입 주체:** scripts/merge_specs.py
...
- **적용 위치:** info
- **목적:** 렌더링된 문서의 문서 섹션/내비게이션 그룹화 슬러그.
- **소비자:** multiple
- **값 타입:** string
- **값 스키마:** `{"type": "string"}`
- **주입 주체:** scripts/merge_specs.py
