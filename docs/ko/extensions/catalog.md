---
title: 확장 기능 보강 카탈로그
description: 보강된 OpenAPI 명세에 포함된 모든 x-* 확장 기능의 정보 원본
i18n:
...
`docs/specifications/api/*.json`에 나타나는 모든 `x-*` 확장 기능의 정보 원본입니다.
`scripts/utils/extension_constants.py`와의 일관성은
`tests/test_extension_catalog.py`에 의해 검증됩니다.

여기에는 세 가지 유형의 확장 기능이 문서화되어 있습니다:

- **여기서 주입됨** — 보강기(enricher)가 추가하는 확장 기능(`x-f5xc-*` 및
  `x-ves-cli-*` / `x-ves-field-*` / `x-ves-operation-*` / 디스커버리
  변형). 이러한 확장 기능은 다운스트림 도구에서 사용해야 합니다.
- **업스트림 패스스루** — F5가 소스 명세에서 내보내고
  우리가 변경 없이 보존하는 확장 기능(`x-ves-proto-*`, `x-displayname` 등).
  투명성을 위해 문서화하지만 이 저장소에서 관리하지 않습니다.
- **향후 주입 예정** — 아직 생성되지 않았으며, 보강기가 생성을 시작하는 시점에
  여기에 문서화됩니다(초기 구성 시점에는 해당 없음).

## 항목 스키마

아래의 모든 항목은 정확히 이 형태를 따릅니다. `tests/test_extension_catalog.py`의
일관성 테스트는 `### x-name` 헤더가 존재하고 `Pass-through from upstream:` 플래그가
`yes` 또는 `no` 값으로 존재하는 한, 섹션 본문이 간략해도 허용합니다.

    ### x-<name>
    - **적용 위치:** <schema | parameter | operation | path-item | info | response>
    - **목적:** <한 문장>
    - **소비자:** <CLI | VSCode | Terraform | Web UI | multiple | N/A>
    - **값 유형:** <string | number | boolean | object | array>
...
- **적용 위치:** info
- **목적:** 보강된 파일이 빌드된 업스트림 소스 명세의 타임스탬프.
- **소비자:** multiple
- **값 유형:** string
...
- **적용 위치:** schema
- **목적:** 이 리소스를 성공적으로 POST/PUT하는 데 필요한 최소한의 필드 집합.
- **소비자:** multiple
- **값 유형:** object
- **값 스키마:** `{"type": "object"}`
...
- **적용 위치:** schema
- **목적:** 이 스키마에 매핑되는 Terraform 리소스 유형 이름.
- **소비자:** Terraform
- **값 유형:** string
- **값 스키마:** `{"type": "string"}`
- **주입 주체:** scripts/utils/field_metadata_enricher.py
...
- **적용 위치:** schema
- **목적:** 리소스 스키마에 대한 사람이 읽을 수 있는 표시 이름(자동 생성을 대체).
- **소비자:** multiple
- **값 유형:** string
- **값 스키마:** `{"type": "string"}`
- **주입 주체:** scripts/utils/field_metadata_enricher.py
...
- **적용 위치:** schema property
- **목적:** 이 속성이 필요한 이름이 지정된 기능 조합을 나열합니다.
- **소비자:** multiple
- **값 유형:** array
- **값 스키마:** `{"type": "array", "items": {"type": "string"}}`
- **주입 주체:** scripts/utils/minimum_configuration_enricher.py
...
- **적용 위치:** schema property
- **목적:** 클라이언트가 속성을 생략했을 때 서버가 할당하는 기본값.
- **소비자:** multiple
- **값 유형:** string
- **값 스키마:** `{}`
...
- **적용 위치:** schema property
- **목적:** 서버 기본값이 최적이 아닌 필드에 대한 권장 프로덕션 값.
- **소비자:** multiple
- **값 유형:** string
- **값 스키마:** `{}`
- **주입 주체:** scripts/utils/default_value_enricher.py
...
- **적용 위치:** schema property
- **목적:** 한 필드가 다른 필드의 설정을 필요로 하는 필드 간 종속성을 문서화합니다.
...
- **적용 위치:** schema property
- **목적:** 필드가 해당 범위 내에서 고유해야 하는지 여부를 선언합니다.
- **소비자:** multiple
- **값 유형:** object
- **값 스키마:** `{"type": "object"}`
...
- **적용 위치:** operation
- **목적:** 성공적인 실행을 위해 제공해야 하는 작업 본문 필드를 지정합니다.
- **소비자:** multiple
- **값 유형:** array
- **값 스키마:** `{"type": "array", "items": {"type": "string"}}`
...
- **적용 위치:** operation
- **목적:** 작업의 관찰 가능한 부작용을 나열합니다(재시작, 재구성 등).
- **소비자:** multiple
- **값 유형:** array
- **값 스키마:** `{"type": "array", "items": {"type": "string"}}`
- **주입 주체:** scripts/utils/operation_metadata_enricher.py
...
- **적용 위치:** operation
- **목적:** 디스커버리 중 이 작업에 대해 실험적으로 측정된 응답 시간.
- **소비자:** multiple
- **값 유형:** object
- **값 스키마:** `{"type": "object"}`
- **주입 주체:** scripts/utils/discovery_enricher.py
...
- **적용 위치:** info
- **목적:** 짧은(~60자) 도메인 설명. 긴 설명의 속성 수준에도 적용됩니다.
- **소비자:** multiple
- **값 유형:** string
- **값 스키마:** `{"type": "string"}`
