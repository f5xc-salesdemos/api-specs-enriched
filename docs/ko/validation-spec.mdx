...
이 문서는 api-specs-enriched 프로젝트에서 게시하는 검증 사양의 구조와 내용을 설명합니다.

## 개요

검증 사양은 API 검증 제약 조건, 기본값 및 구성 메타데이터를 단일 JSON 파일로 통합합니다. 이 사양은 보강된 OpenAPI 사양에서 생성되며 함께 게시됩니다.
...
신뢰도 점수가 포함된 필드 이름 패턴 기반 검증 규칙입니다.
...
리소스 유형별로 통합된 구조로 정리된 모든 기본값입니다.
...
| `recommended` | F5 XC 웹 콘솔에서 사전 입력되는 값 | UI 분석 |
| `advanced_options` | advanced_options 객체 내의 중첩된 기본값 | API 디스커버리 |
...
보강된 OpenAPI 사양은 다음 확장을 사용하여 검증 메타데이터를 포함합니다:
...
| `x-f5xc-validation` | 디스커버리 파생 제약 조건 | 스키마 속성 |
...
검증 사양은 `version` 필드에서 시맨틱 버전 관리를 따릅니다:

| 변경 유형 | 버전 증가 | 예시 |
|-------------|--------------|---------|
| 호환성을 깨는 구조 변경 | 메이저 | 필드 이름 변경, 섹션 제거 |
| 새로운 검증 규칙 또는 리소스 | 마이너 | 새로운 리소스 기본값 |
...
2. **디스커버리(Discovery)** - 실시간 API 디스커버리에서 얻은 제약 조건
3. **추론(Inferred)** - 패턴 매칭에서 얻은 제약 조건 (최저 우선순위)

## 관련 문서

- [헬스체크 보강](./Enhancements/healthcheck-enhancements/) - 헬스체크 스키마 보강
- [오리진 풀 보강](./Enhancements/originpool-enhancements/) - 오리진 풀 스키마 보강
