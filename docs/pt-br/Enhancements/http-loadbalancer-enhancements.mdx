...
Todos os esquemas que correspondem a `viewshttp_loadbalancer.*SpecType` recebem enriquecimentos:
...
A extensão `x-f5xc-minimum-configuration` fornece metadados CLI abrangentes para criar HTTP load balancers minimamente viáveis.

### Campos Obrigatórios

| Campo | Restrição | Descrição |
|-------|-----------|-----------|
| `metadata.name` | Formato de label DNS: `[a-z0-9]([-a-z0-9]*[a-z0-9])?` | Nome do recurso |
| `metadata.namespace` | Formato de label DNS | Namespace |
...
- `loadbalancer_type` em extensões nativas da especificação OpenAPI (nome nativo do campo)

**Referência do Schema da API**: `x-ves-oneof-field-loadbalancer_type: ["http", "https", "https_auto_cert", "http_https"]`
...
Campos marcados com `x-f5xc-server-default: true` têm seu valor `default` aplicado pelo servidor da API F5 XC quando omitidos das requisições. O HTTP load balancer possui um grande número de valores padrão aplicados pelo servidor porque a maioria dos recursos de segurança e opções de protocolo são configurados por padrão em um estado desabilitado ou seguro.

### Valores Padrão do HTTPS Auto-Cert

Quando `https_auto_cert` é especificado, o servidor aplica estes valores padrão para campos omitidos:
...
| `connection_idle_timeout` | `120000` | integer | Tempo limite de inatividade da conexão em milissegundos (2 minutos) |
...
### Valores Padrão de Recursos de Segurança
...
### Valores Padrão de DDoS e Controle de Acesso
...
| `l7_ddos_protection.ddos_policy_none` | `{}` | Sem referência a política DDoS |
| `no_challenge` | `{}` | Sem desafio ao cliente |
| `user_id_client_ip` | `{}` | Identificar usuários por IP do cliente |
| `disable_trust_client_ip_headers` | `{}` | Não confiar nos cabeçalhos de IP do cliente |

### Outros Valores Padrão Aplicados pelo Servidor

| Campo | Valor Padrão | Tipo | Descrição |
|-------|--------------|------|-----------|
| `advertise_on_public_default_vip` | `{}` | object | Divulgar no VIP público padrão |
...
| `advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | Como divulgar o load balancer |
...
| `ddos_policy` | `ddos_policy_none`, `ddos_policy_ref` | Referência a política DDoS ou nenhuma |
...
**Status**: Ainda não aplicado aos esquemas http_loadbalancer.
...
Esta extensão permitiria que ferramentas downstream pré-selecionassem a variante mais comumente usada ao apresentar opções de configuração aos usuários.

## Metadados de Restrição
...
| `spec.https_auto_cert.connection_idle_timeout` | 1000 | 3600000 | 120000 | Tempo limite de inatividade da conexão (milissegundos, 1s a 1h) |
...
| `spec.l7_ddos_protection.mitigation` | `mitigation_block`, `mitigation_challenge`, `mitigation_none` | `mitigation_block` | Ação de mitigação DDoS de camada 7 |
...
| `spec.advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | `advertise_on_public_default_vip` | Divulgação do load balancer |

## Integração de Fluxo de Trabalho Guiado

O pipeline de enriquecimento adiciona metadados `x-f5xc-guided-workflows` no nível da especificação. Esses fluxos de trabalho fornecem orientação de implantação passo a passo consumida por assistentes de IA e ferramentas CLI.
...
| Etapa | Ação | Recurso | Campos Obrigatórios | Opcional |
|-------|------|---------|---------------------|----------|
| 1 | Enviar Certificado SSL | `certificate` | `name`, `certificate_chain`, `private_key` | Não |
...
Ferramentas downstream utilizam esses fluxos de trabalho para guiar os usuários através de implantações em múltiplas etapas:
...
Estas extensões de fornecedor são adicionadas ao schema OpenAPI padrão para transmitir metadados específicos do F5 XC.
...
Fornece metadados abrangentes para criar configurações minimamente viáveis. Inclui:
- Descrição
...
Quando `true`, indica que o valor `default` acompanhante é aplicado pelo servidor da API F5 XC. Campos com esta extensão podem ser omitidos com segurança das requisições da API -- o servidor aplica o valor padrão automaticamente.
...
| Arquivo | Finalidade |
|---------|------------|
| `config/minimum_configs.yaml` | Fonte de metadados de configuração mínima |
| `config/constraint_patterns.yaml` | Definições de padrões de restrição |
| `config/guided_workflows.yaml` | Definições de etapas de fluxos de trabalho guiados |
| `config/domain_descriptions.yaml` | Metadados de descrição em nível de domínio |

### Esquemas Enriquecidos

- `viewshttp_loadbalancerCreateSpecType` - Schema da operação de criação
- `viewshttp_loadbalancerReplaceSpecType` - Schema da operação de atualização
- `viewshttp_loadbalancerGetSpecType` - Schema da operação de leitura

## Documentação Relacionada

- [Guia de Desenvolvimento - Extensões OpenAPI](../development/#openapi-extensions) - Definições e uso de extensões
- [Aprimoramentos de Healthcheck](./healthcheck-enhancements/) - Enriquecimentos de esquema de healthcheck (inclui `x-f5xc-recommended-oneof-variant`)
- [Aprimoramentos de Origin Pool](./originpool-enhancements/) - Enriquecimentos de esquema de origin pool
- [Guia de Metadados de Restrição](../constraint-metadata/) - Documentação detalhada de validação de restrições

## Registro de Alterações

| Versão | Data | Alterações |
|--------|------|------------|
| 2.0.46 | 2026-04-18 | Adicionados valores padrão aplicados pelo servidor, integração de fluxo de trabalho guiado e seção expandida de configuração mínima |
