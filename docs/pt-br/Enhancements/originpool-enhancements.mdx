...
Todos os esquemas correspondentes a `origin_pool.*SpecType` recebem enriquecimentos:
...
| `spec.origin_servers` | Create | Mínimo de 1 item |
| `spec.port` | Create | 1-65535, sem valor padrão do servidor |
...
| `disable_subsets` | `{}` | object | Balanceamento de carga por subconjuntos desabilitado |
| `auto_http_config` | `{}` | object | Negociação automática do protocolo HTTP |
...
| `origin_servers[].public_name.refresh_interval` | `0` | Usar intervalo padrão do sistema para atualização DNS |
...
| Valor | Descrição | Observações |
|-------|-----------|-------------|
| `DISTRIBUTED` | Considera tanto endpoints remotos quanto locais | Padrão do servidor |
| `LOCAL_ONLY` | Apenas endpoints locais são utilizados | |
| `LOCAL_PREFERRED` | Prefere locais, recorre a remotos se indisponível | |
...
## Tipos de Servidor de Origem

Servidores de origem podem ser especificados usando diferentes variantes de tipo. O tipo determina quais campos adicionais são obrigatórios.
...
## A FAZER: Tipos de Servidor de Origem Dependentes de Site

Os seguintes tipos de servidor de origem requerem desenvolvimento de lógica de recurso "site" antes da implementação:

| Tipo | Descrição | Campos Obrigatórios | Status |
|------|-----------|---------------------|--------|
| `private_ip` | Servidor de origem com IP privado/público e informações de site | `ip`, `site_locator`, `network_choice`, `snat_pool` | 🔲 Pendente de lógica de site |
| `private_name` | Servidor de origem com nome DNS e informações de site | `dns_name`, `site_locator`, `network_choice`, `snat_pool` | 🔲 Pendente de lógica de site |
...
1. **Descoberta de Recurso Site** - Documentar o esquema do recurso `site` e padrões de site_locator
2. **Enum de Network Choice** - Documentar valores restritos de `network_choice` (site_local_inside, site_local_outside, etc.)
3. **Opções de Pool SNAT** - Documentar padrões de configuração do pool SNAT
...
- Sprint relacionada: Enriquecimentos de esquema do recurso site
...
|--------|------|------------|
| 2.1.2 | 18/01/2026 | Adicionada seção de tipos de servidor de origem com public_name/public_ip; marcadores A FAZER para tipos dependentes de site |
| 2.1.1 | 18/01/2026 | Reescrito como referência de API pura; removidos exemplos de código downstream |
| 2.1.0 | 18/01/2026 | Atualizado para estrutura unificada de valores padrão no validation.json |
| 2.0.33 | 17/01/2026 | Documentação inicial de aprimoramentos do origin pool |
