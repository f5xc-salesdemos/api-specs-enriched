...
description: Metadados de enriquecimento para schemas de origin pool
sidebar:
  order: 3
i18n:
...
Metadados de enriquecimento para schemas relacionados a origin_pool. Consulte [Extensões OpenAPI](../development/#openapi-extensions) para definições de extensões.

## Schemas Enriquecidos

### Padrão

Todos os schemas correspondentes a `origin_pool.*SpecType` recebem enriquecimentos:
...
| `spec.origin_servers` | Criação | Mínimo 1 item |
...
| `healthcheck` | `[]` | array | Nenhuma verificação de saúde configurada |
...
| `same_as_endpoint_port` | `{}` | object | Verificação de saúde usa a porta do endpoint |
| `default_circuit_breaker` | `{}` | object | Configurações padrão do circuit breaker |
| `disable_outlier_detection` | `{}` | object | Detecção de outlier desabilitada |
...
| `origin_servers[].public_name.refresh_interval` | `0` | Usar intervalo padrão de atualização DNS do sistema |
...
|-------|-----------|-------------|
| `ROUND_ROBIN` | Cada endpoint saudável selecionado em ordem round-robin | Padrão do servidor |
| `LEAST_REQUEST` | Endpoint com menos requisições ativas selecionado | |
...
| `LOCAL_ONLY` | Apenas endpoints locais utilizados | |
| `LOCAL_PREFERRED` | Preferir local, recorrer a remoto se indisponível | |
...
| Porta de Verificação de Saúde | `same_as_endpoint_port`, `health_check_port` | `same_as_endpoint_port` |
...
| `docs/specifications/api/openapi.json` | Especificação combinada com todos os schemas |
...
├── advanced_options    # Valores padrão de advanced_options aninhados
...
Os seguintes tipos de origin server requerem desenvolvimento da lógica do recurso "site" antes da implementação:

| Tipo | Descrição | Campos Obrigatórios | Status |
|------|-----------|---------------------|--------|
| `private_ip` | Origin server com IP privado/público e informações do site | `ip`, `site_locator`, `network_choice`, `snat_pool` | 🔲 Pendente lógica de site |
| `private_name` | Origin server com nome DNS e informações do site | `dns_name`, `site_locator`, `network_choice`, `snat_pool` | 🔲 Pendente lógica de site |
| `k8s_service` | Serviço Kubernetes com informações do site | `service_name`, `site_locator`, `network_choice` | 🔲 Pendente lógica de site |
| `consul_service` | Serviço HashiCorp Consul com informações do site | `service_name`, `site_locator`, `network_choice` | 🔲 Pendente lógica de site |

### Requisitos de Implementação

Antes que esses tipos possam ser completamente documentados:

1. **Descoberta de Recursos Site** - Documentar o schema do recurso `site` e os padrões de site_locator
2. **Enum de Network Choice** - Documentar os valores restritos de `network_choice` (site_local_inside, site_local_outside, etc.)
...
- Sprint relacionada: Enriquecimentos de schema de recursos site
...
- [Aprimoramentos de Healthcheck](./healthcheck-enhancements/) - Enriquecimentos de schema de healthcheck
...
| 2.0.33 | 2026-01-17 | Documentação inicial de aprimoramentos do origin pool |
