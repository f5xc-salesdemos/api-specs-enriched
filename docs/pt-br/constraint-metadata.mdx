...
A extensão `x-f5xc-constraints` fornece conhecimento determinístico sobre restrições de validação de campos, permitindo:

- **Assistentes de IA**: Gerar configurações válidas sem necessidade de entrada do usuário para detalhes de restrições
...
- **50+ tipos de padrão** (string, array, numérico)
- **95% de confiança** em campos de alta prioridade (nomes, portas, identificadores)
- **Reconciliação em 3 níveis**: EXISTENTE > DISCOVERY > INFERIDO
...
| *(chaves de restrição)* | variados | Chaves de restrição específicas por tipo, achatadas no nível superior (ex.: `minLength`, `maxItems`, `minimum`) |
...
- Rótulos DNS separados por pontos
...
**Origin Pools**:
- Array de origin pools requer pelo menos 1 item, máximo de 50 itens
...
### Exemplo 1: Validação de Nome como Rótulo DNS
...
| 0.70-0.89 | **Confiança média** | Consultivo, recomenda confirmação do usuário |
...
3. **Validação de Respostas**: Respostas de API bem-sucedidas demonstrando valores válidos
