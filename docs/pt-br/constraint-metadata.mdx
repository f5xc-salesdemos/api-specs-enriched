...
description: >-
  Metadados abrangentes de restrições para geração e validação determinística de
  IA
sidebar:
  order: 2
i18n:
...
**Objetivo**: Metadados abrangentes de restrições para geração determinística de IA, validação por CLI e imposição de esquema Terraform.

**Extensão**: `x-f5xc-constraints`
**Versão**: 3.3.0
**Status**: Produção
**Consumidores**: Assistentes de IA, ferramentas de CLI, provedores Terraform, extensões de IDE, validadores de API
...
- **Ferramentas de CLI**: Fornecer dicas de entrada precisas, validação e mensagens de erro
- **Terraform**: Impor restrições no momento do plano, não no momento de aplicação
...
| `deterministic` | boolean | Indicador de alta confiança (confiança >= 0,9) para geração por IA |
| `category` | string | Categoria de restrição para agrupamento e relatórios |
| *(chaves de restrição)* | variado | Chaves de restrição específicas do tipo niveladas no nível superior (ex.: `minLength`, `maxItems`, `minimum`) |
...
- Nomes de rótulos DNS devem ter entre 1 e 63 caracteres alfanuméricos minúsculos
...
**FQDNs** (`format: "fqdn"`):
- Nomes de Domínio Totalmente Qualificados devem ter entre 1 e 253 caracteres
...
**Pools de Origem**:
- O array de pools de origem requer no mínimo 1 item e no máximo 50 itens
...
|-------|---------------|-----|
| 0,90-1,00 | **Alta confiança** | Indicador determinístico habilitado, use para geração por IA |
| 0,70-0,89 | **Confiança média** | Consultivo, recomende confirmação do usuário |
| 0,50-0,69 | **Baixa confiança** | Apenas informativo, exija validação do usuário |
...
- O padrão foi validado com dados de descoberta OU
...
2. **DISCOVERY** (Confiança: 0,99)
   - Extraído de respostas da API F5 XC em tempo real
...
1. **x-ves-validation-rules**: Extensões de validação nativas do F5
...
3. **Validação de Resposta**: Respostas de API bem-sucedidas demonstrando valores válidos
...
### Integração com Ferramenta de CLI
