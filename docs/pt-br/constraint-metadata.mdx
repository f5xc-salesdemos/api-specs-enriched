...
**Propósito**: Metadados abrangentes de restrições para geração determinística por IA, validação por CLI e aplicação de esquema Terraform.
...
2. [Estrutura da Extensão](#estrutura-da-extensão)
3. [Tipos de Restrições](#tipos-de-restrições)
4. [Descrições em Linguagem Natural](#descrições-em-linguagem-natural)
5. [Exemplos de Validação](#exemplos-de-validação)
6. [Pontuações de Confiança](#pontuações-de-confiança)
7. [Integração com Descoberta](#integração-com-descoberta)
8. [Guia de Integração de Ferramentas](#guia-de-integração-de-ferramentas)
...
| *(chaves de restrição)* | diversos | Chaves de restrição específicas do tipo achatadas no nível superior (ex.: `minLength`, `maxItems`, `minimum`) |
...
- Nomes de rótulos DNS devem ter de 1 a 63 caracteres alfanuméricos em minúsculas
- Hífens permitidos, mas não no início/fim (RFC 1123)
- Exemplo: `my-service`, `api-gateway`, `lb-prod-01`

**Endereços de E-mail** (`format: "email"`):
- Endereços de e-mail devem ser válidos conforme RFC 5322
...
**URLs** (`format: "url"`):
- URLs devem ser URIs válidos conforme RFC 3986
...
**Pools de Origem**:
- O array de pools de origem requer no mínimo 1 item, máximo de 50 itens
...
### Exemplo 1: Validação de Nome com Rótulo DNS
...
- ✅ `80` (HTTP)
- ✅ `443` (HTTPS)
- ✅ `8080` (alternativa comum para HTTP)
- ✅ `65535` (porta máxima)
...
| 0.70-0.89 | **Confiança média** | Consultivo, recomendar confirmação do usuário |
...
2. **Catálogo de Erros**: Mensagens de erro da API indicando violações de restrições
   - "name must be 1-63 characters" → minLength/maxLength
   - "port must be between 1 and 65535" → minimum/maximum

3. **Validação de Resposta**: Respostas bem-sucedidas da API demonstrando valores válidos
   - Faixas de valores observados
...
## Apêndice: Catálogo de Padrões de Restrições
...
### Padrões de Confiança Média (0.80-0.94)
...
- **Problemas**: Reporte problemas de precisão de restrições em https://github.com/f5xc-salesdemos/api-specs-enriched/issues
