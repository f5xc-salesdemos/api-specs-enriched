...
|-----------|---------|------------|
| `specs/original/` | Specs fonte do F5 | Ignorado pelo Git |
| `specs/discovered/` | Saída da descoberta de API | Rastreado (openapi.json, session.json) |
| `docs/specifications/api/` | Specs de domínio geradas | Ignorado pelo Git |
...
- Valores enum de respostas em produção
- Valores padrão
...
2. Verificação de ETag determina se as specs do F5 mudaram
3. Pipeline processa e enriquece as specs
4. Versão calculada a partir de tags git: `git describe --tags --abbrev=0`
...
A Descoberta explora a API F5 XC em produção para encontrar:

- **Restrições não documentadas**: Campos obrigatórios não marcados no OpenAPI
- **Valores enum reais**: Valores reais observados em produção
...
| `x-f5xc-recommended-value` | O console web do F5 XC pré-preenche este valor para novos recursos | O campo não possui valor padrão do servidor, mas este valor representa a configuração típica |
| `x-f5xc-recommended-oneof-variant` | O console do F5 XC pré-seleciona esta variante OneOf | Identifica a escolha típica quando múltiplas opções mutuamente exclusivas existem |
...
### Extensões de Campos Obrigatórios

| Extensão | Origem | Finalidade |
|-----------|--------|---------|
| `x-ves-required: "true"` | Spec original do F5 XC | O campo requer valor não-zero/não-vazio |
...
Quando `true`, indica que o valor `default` acompanhante é aplicado pelo servidor da API F5 XC. Campos com esta extensão podem ser omitidos com segurança das requisições à API — o servidor aplica o valor padrão automaticamente.
...
Especifica um valor que o console web do F5 XC usa como padrão pré-preenchido. Este valor não é aplicado pelo servidor, mas representa a configuração inicial típica para novos recursos criados via console.
...
**Tipo**: `object` (mapa do nome do grupo para o nome da variante)
...
Declara relações de exclusividade mútua entre membros de grupos OneOf. Derivado automaticamente das extensões `x-ves-oneof-field-*`. Isso permite que ferramentas downstream (Terraform, CLI, MCP) validem conflitos no nível do schema em vez de em tempo de execução.
...
**Origem**: Derivado automaticamente das extensões nativas `x-ves-oneof-field-*` do F5 durante o enriquecimento do pipeline.
...
| Regra | Implicação |
|------|-------------|
| `ves.io.schema.rules.message.required: "true"` | O campo é obrigatório |
| `ves.io.schema.rules.uint32.gte: N` | Se N >= 1 e sem valor padrão do servidor, o campo é obrigatório |
...
O pipeline é executado a cada commit para garantir consistência. Para commits de trabalho em progresso:
...
A versão é derivada de tags git (ex.: `v2.0.38`), eliminando condições de corrida que causavam conflitos de merge em PRs concorrentes.
...
As specs geradas são ignoradas pelo git:
...
Adicione uma exclusão ao `.pre-commit-config.yaml`:
...
| `make download` | Baixar specs (cache por ETag) |
...
| `make push-discovery` | Comitar dados de descoberta |
...
**Nota**: A versão é derivada de tags git (ex.: `v2.0.38`), não de um arquivo `.version`.
