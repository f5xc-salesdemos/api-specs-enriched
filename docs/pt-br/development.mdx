...
|-----------|---------|------------|
| `specs/original/` | Specs fonte da F5 | Ignorado pelo Git |
...
4. A versão é calculada a partir das tags git: `git describe --tags --abbrev=0`
...
| `x-f5xc-recommended-value` | O console web F5 XC pré-preenche este valor para novos recursos | O campo não tem valor padrão do servidor, mas este valor representa a configuração típica |
...
Especifica um valor que o console web F5 XC usa como padrão pré-preenchido. Este valor não é imposto pelo servidor, mas representa a configuração inicial típica para novos recursos criados via console.
...
Para estes recursos, `x-f5xc-required-for.create` pode ser `false` mesmo quando `x-ves-required` é `true`.
