...
A especificação de validação consolida restrições de validação de API, valores padrão e metadados de configuração em um único arquivo JSON. Esta especificação é gerada a partir das especificações OpenAPI enriquecidas e publicada junto com elas.
...
Padrões de validação em nível de tipo e regras baseadas em padrões.
...
|----------|-------------|--------|
| `server_applied` | Valores que a API aplica quando campos são omitidos | Testes em API ao vivo |
| `recommended` | Valores pré-preenchidos no console web F5 XC | Análise da interface |
...
| Correções de bugs ou atualizações de descrição | Patch | Correções de digitação |
...
2. **Descoberta** - Restrições provenientes da descoberta de API ao vivo
3. **Inferida** - Restrições provenientes de correspondência de padrões (prioridade mais baixa)
