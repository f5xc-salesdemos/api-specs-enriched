...
A especificação de validação consolida restrições de validação de API, valores padrão e metadados de configuração em um único arquivo JSON. Essa especificação é gerada a partir das specs OpenAPI enriquecidas e publicada junto a elas.
...
Padrões de validação padrão por tipo e regras baseadas em padrões.
...
|----------|-------------|--------|
| `server_applied` | Valores que a API aplica quando os campos são omitidos | Testes de API ao vivo |
...
| `nested_recommended` | Valores recomendados para esquemas aninhados | Análise de UI |
| `ui_vs_server` | Casos em que os padrões da UI diferem dos padrões da API | Análise comparativa |
...
As specs OpenAPI enriquecidas incorporam metadados de validação usando estas extensões:

| Extensão | Finalidade | Localização |
|-----------|---------|----------|
| `x-f5xc-required-for` | Campos obrigatórios específicos por contexto | Propriedades do esquema |
| `x-f5xc-server-default` | Marca padrões aplicados pelo servidor | Propriedades do esquema |
| `x-f5xc-recommended-value` | Valor padrão recomendado | Propriedades do esquema |
| `x-f5xc-recommended-oneof-variant` | Variante OneOf recomendada | Definições do esquema |
| `x-f5xc-conditions` | Requisitos condicionais | Propriedades do esquema |
| `x-f5xc-minimum-configuration` | Exemplos de configuração mínima | Definições do esquema |
| `x-f5xc-validation` | Restrições derivadas de descoberta | Propriedades do esquema |
...
| Novas regras de validação ou recursos | Minor | Novos padrões de recurso |
| Correções de bugs ou atualizações de descrição | Patch | Correções de erros tipográficos |

## Prioridade de Reconciliação

Quando múltiplas fontes fornecem restrições, a prioridade de reconciliação é:

1. **Existente** - Restrições na spec OpenAPI original (maior prioridade)
2. **Descoberta** - Restrições provenientes da descoberta de API ao vivo
3. **Inferida** - Restrições provenientes de correspondência de padrões (menor prioridade)

## Documentação Relacionada

- [Healthcheck Enhancements](./Enhancements/healthcheck-enhancements/) - Enriquecimentos do esquema de healthcheck
- [Origin Pool Enhancements](./Enhancements/originpool-enhancements/) - Enriquecimentos do esquema de origin pool
...
|---------|------|---------|
| 2.1.2 | 2026-01-18 | Reescrito como referência de API pura; categorias oneof_recommended e nested_recommended adicionadas |
| 2.1.0 | 2026-01-18 | Estrutura de padrões unificada substituindo seções fragmentadas |
