...
description: เมตาดาต้าข้อจำกัดที่ครอบคลุมสำหรับการสร้างและตรวจสอบ AI แบบกำหนดได้
sidebar:
  order: 2
i18n:
...
**วัตถุประสงค์**: เมตาดาต้าข้อจำกัดที่ครอบคลุมสำหรับการสร้าง AI แบบกำหนดได้, การตรวจสอบ CLI และการบังคับใช้ Terraform schema
...
4. [คำอธิบายด้วยภาษาธรรมชาติ](#natural-language-descriptions)
5. [ตัวอย่างการตรวจสอบ](#validation-examples)
...
ส่วนขยาย `x-f5xc-constraints` ให้ความรู้แบบกำหนดได้เกี่ยวกับข้อจำกัดการตรวจสอบฟิลด์ ซึ่งช่วยให้:

- **ผู้ช่วย AI**: สร้างการกำหนดค่าที่ถูกต้องโดยไม่ต้องให้ผู้ใช้ป้อนข้อมูลรายละเอียดข้อจำกัด
- **เครื่องมือ CLI**: ให้คำแนะนำการป้อนข้อมูลที่แม่นยำ, การตรวจสอบ และข้อความแสดงข้อผิดพลาด
- **Terraform**: บังคับใช้ข้อจำกัดในขั้นตอน plan ไม่ใช่ขั้นตอน apply
- **ส่วนขยาย IDE**: การตรวจสอบแบบเรียลไทม์และข้อเสนอแนะการเติมอัตโนมัติ
- **ตัวตรวจสอบ API**: การตรวจสอบก่อนส่งเพื่อลดข้อผิดพลาดของ API

### สถิติความครอบคลุม

- **ข้อจำกัด 9,851 รายการ** ครอบคลุมสเปก F5 XC API ทั้งหมด
- **รูปแบบมากกว่า 50 ประเภท** (string, array, numeric)
- **ความเชื่อมั่น 95%** สำหรับฟิลด์ที่มีความสำคัญสูง (names, ports, identifiers)
- **การปรับข้อมูล 3 ระดับ**: EXISTING > DISCOVERY > INFERRED
...
| `deterministic` | boolean | แฟล็กความเชื่อมั่นสูง (confidence >= 0.9) สำหรับการสร้าง AI |
| `category` | string | หมวดหมู่ข้อจำกัดสำหรับการจัดกลุ่มและรายงาน |
| *(constraint keys)* | ต่างๆ | คีย์ข้อจำกัดเฉพาะประเภทที่แบนอยู่ระดับบนสุด (เช่น `minLength`, `maxItems`, `minimum`) |
| `metadata` | object | การติดตามแหล่งที่มา, ความเชื่อมั่น, ประทับเวลาการตรวจสอบ |
...
| `date-time` | ประทับเวลา ISO 8601 | `2026-01-19T12:00:00Z` |
| `date` | วันที่ ISO 8601 | `2026-01-19` |
| `time` | เวลา ISO 8601 | `12:00:00` |
...
- อนุญาตให้ใช้เครื่องหมายยัติภังค์แต่ไม่ใช่ที่จุดเริ่มต้น/สิ้นสุด (RFC 1123)
...
**FQDNs** (`format: "fqdn"`):
- ชื่อโดเมนแบบเต็ม (Fully Qualified Domain Names) ต้องมีความยาว 1-253 ตัวอักษร
...
**ประทับเวลา** (`format: "date-time"`):
- ประทับเวลาต้องอยู่ในรูปแบบ ISO 8601
...
**ค่า Timeout**:
- ค่า Timeout ต้องเป็น 1-3600 วินาที (สูงสุด 1 ชั่วโมง)
...
- ❌ `-my-service` (เครื่องหมายยัติภังค์ที่จุดเริ่มต้น)
- ❌ `my-service-` (เครื่องหมายยัติภังค์ที่จุดสิ้นสุด)
...
| 0.50-0.69 | **ความเชื่อมั่นต่ำ** | เพื่อข้อมูลเท่านั้น ต้องให้ผู้ใช้ตรวจสอบ |
| < 0.50 | **ต่ำมาก** | ไม่รวมในข้อจำกัด |

### แฟล็ก Deterministic

แฟล็กบูลีน `deterministic` จะถูกตั้งค่าเมื่อ:
- คะแนนความเชื่อมั่น >= 0.9
- รูปแบบได้รับการตรวจสอบกับข้อมูล discovery แล้ว หรือ
- รูปแบบตรงกับมาตรฐานที่เป็นที่ยอมรับ (RFC, ISO, ฯลฯ)
...
ข้อจำกัดจะถูกปรับข้อมูลโดยใช้ลำดับความสำคัญ 3 ระดับ:

1. **EXISTING** (ความเชื่อมั่น: 1.0)
   - กำหนดไว้อย่างชัดเจนในสเปก OpenAPI ต้นฉบับ
   - ไม่มีการแทนที่

2. **DISCOVERY** (ความเชื่อมั่น: 0.99)
   - สกัดจากการตอบกลับ API ของ F5 XC ที่ใช้งานจริง
...
2. **Error Catalog**: ข้อความแสดงข้อผิดพลาดของ API ที่ระบุการละเมิดข้อจำกัด
...
| UUID | `\buuid$` | String (36 ตัวอักษร, uuid format) | `550e8400-...` |
...
- **ปัญหา**: รายงานปัญหาความถูกต้องของข้อจำกัดได้ที่ https://github.com/f5xc-salesdemos/api-specs-enriched/issues
...
**ข้อจำกัดที่ปรับข้อมูลแล้ว**: 9,851
