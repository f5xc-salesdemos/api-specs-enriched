---
title: HTTP 负载均衡器增强
description: HTTP 负载均衡器模式的增强元数据
sidebar:
  order: 2
i18n:
...
HTTP 负载均衡器模式的增强元数据。有关扩展定义，请参阅 [OpenAPI 扩展](../development/#openapi-extensions)。

## 增强模式

### 匹配模式

所有匹配 `viewshttp_loadbalancer.*SpecType` 的模式都会接收增强：
...
`x-f5xc-minimum-configuration` 扩展为创建最小可用的 HTTP 负载均衡器提供了全面的 CLI 元数据。
...
- `loadbalancer_type` 在 OpenAPI 规范原生扩展中（原生字段名称）
...
标记为 `x-f5xc-server-default: true` 的字段，其 `default` 值会在请求中省略时由 F5 XC API 服务器自动应用。HTTP 负载均衡器拥有大量服务器应用的默认值，因为大多数安全功能和协议选项默认处于禁用或安全状态。
...
| `header_transformation_type.legacy_header_transformation` | `{}` | object | 传统头转换 |
| `connection_idle_timeout` | `120000` | integer | 连接空闲超时（毫秒）（2 分钟） |
| `http_protocol_options.http_protocol_enable_v1_v2` | `{}` | object | HTTP/1.1 和 HTTP/2 已启用 |
...
标记为 `x-f5xc-conflicts-with` 的字段表示 OneOf 模式。每个组中只能指定一个字段。
...
与包含 `x-f5xc-recommended-oneof-variant` 来指示 OneOf 组中最常见选择的 healthcheck 不同，http_loadbalancer 目前缺少此扩展。
...
1. 观察 F5 XC 控制台的默认选择（例如，UI 中预选哪个 lb_type）
...
增强管道在 spec 级别添加 `x-f5xc-guided-workflows` 元数据。这些工作流提供逐步部署指导，供 AI 助手和 CLI 工具使用。

### 部署 HTTP 负载均衡器

一个 5 步工作流，用于创建带有后端源池的完整配置的 HTTP 负载均衡器（定义在 `config/guided_workflows.yaml` 中）：
...
- **VS Code 扩展**：在侧边栏面板中显示工作流步骤并提供进度跟踪
...
当为 `true` 时，表示伴随的 `default` 值由 F5 XC API 服务器强制执行。具有此扩展的字段可以安全地从 API 请求中省略——服务器会自动应用默认值。
...
| 文件 | 内容 |
|------|---------|
| `docs/specifications/api/virtual.json` | 包含增强的所有 http_loadbalancer 模式 |
...
- [健康检查增强](./healthcheck-enhancements/) - 健康检查模式增强（包含 `x-f5xc-recommended-oneof-variant`）
- [源池增强](./originpool-enhancements/) - 源池模式增强
...
| 版本 | 日期 | 变更 |
|---------|------|---------|
| 2.0.46 | 2026-04-18 | 添加了服务器应用的默认值、引导式工作流集成以及扩展的最小配置部分 |
| 2.0.45 | 2026-01-20 | 初始 http_loadbalancer 增强文档 |
