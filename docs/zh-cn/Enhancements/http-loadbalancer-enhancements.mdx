---
title: HTTP LoadBalancer 增强
description: HTTP 负载均衡器模式的富化元数据
sidebar:
  order: 2
  label: HTTP LoadBalancer 增强
i18n:
...
`x-f5xc-minimum-configuration` 扩展提供了全面的 CLI 元数据，用于创建最小可行的 HTTP 负载均衡器。
...
| `spec.domains` | 数组，min_items: 1 | 要提供服务的域名 |
...
|---------|-------------|
| `spec.http` | 仅 HTTP |
| `spec.https` | 带手动证书的 HTTPS |
| `spec.https_auto_cert` | 带自动证书的 HTTPS |
| `spec.http_https` | HTTP 和 HTTPS 兼有 |

**注意**：负载均衡器类型 OneOf 组的引用方式为：
- 在配置文件中为 `lb_type`（`config/minimum_configs.yaml` 中的简短形式）
- 在 OpenAPI 规范原生扩展中为 `loadbalancer_type`（原生字段名称）
...
标记为 `x-f5xc-server-default: true` 的字段，其 `default` 值会在请求中省略时由 F5 XC API 服务器自动应用。HTTP 负载均衡器具有大量服务器应用的默认值，因为大多数安全功能和协议选项默认为禁用或安全状态。
...
| `tls_config.default_security` | `{}` | object | TLS 1.2+ 使用强加密套件 |
...
| 字段 | 默认值 | 描述 |
|-------|---------------|-------------|
| `l7_ddos_protection.mitigation_block` | `{}` | 阻止 DDoS 流量（默认缓解操作） |
...
| `load_balancing_algorithm` | `round_robin`, `least_request`, `ring_hash`, `random` | 流量分发算法 |
...
| `bot_defense` | `disable_bot_defense`, `enable_bot_defense` | 机器人检测和缓解 |
...
| 组名 | 字段 | 描述 |
|-----------|--------|-------------|
| `ddos_mitigation` | `mitigation_block`, `mitigation_challenge`, `mitigation_none` | DDoS 缓解操作 |
| `ddos_rps_threshold` | `default_rps_threshold`, `custom_rps_threshold` | 每秒请求数阈值 |
| `ddos_clientside_action` | `clientside_action_none`, `clientside_action_javascript`, `clientside_action_captcha` | 客户端 DDoS 验证操作 |
...
| `timeouts` | `system_default_timeouts`, `custom_timeouts` | 使用系统默认超时或自定义超时 |
| `service_policies_source` | `service_policies_from_namespace`, `active_service_policies` | 来自命名空间或活跃列表的服务策略 |
...
与 healthcheck 不同（后者包含 `x-f5xc-recommended-oneof-variant` 来指示 OneOf 组最常见的选择），http_loadbalancer 目前缺少此扩展。
...
此扩展将使下游工具能够在向用户呈现配置选项时预选最常用的变体。

## 约束元数据

标记为 `x-f5xc-constraints` 的字段包含从 API 分析中发现的验证约束。
...
| `spec.l7_ddos_protection.mitigation` | `mitigation_block`, `mitigation_challenge`, `mitigation_none` | `mitigation_block` | 第 7 层 DDoS 缓解操作 |
...
| `spec.advertising` | `advertise_on_public_default_vip`, `advertise_on_public`, `advertise_custom`, `do_not_advertise` | `advertise_on_public_default_vip` | 负载均衡器发布 |

## 引导式工作流集成

富化管道在规范级别添加 `x-f5xc-guided-workflows` 元数据。这些工作流提供逐步部署指导，供 AI 助手和 CLI 工具使用。

### 部署 HTTP 负载均衡器

一个 5 步工作流，用于创建带有后端源池的完全配置的 HTTP 负载均衡器（定义在 `config/guided_workflows.yaml` 中）：
...
**前提条件**：目标租户中的有效命名空间、后端应用可通过 IP 或 DNS 访问、SSL 证书（HTTPS 可选）。
...
**前提条件**：目标租户中的有效命名空间、SSL 证书和私钥、后端应用可访问。
...
使用标志指示上下文特定的要求：`minimum_config`、`create`、`update`、`read`。
...
| 文件 | 内容 |
|------|---------|
| `docs/specifications/api/virtual.json` | 所有带富化的 http_loadbalancer 模式 |
...
- [Healthcheck 增强](./healthcheck-enhancements/) - 健康检查模式富化（包含 `x-f5xc-recommended-oneof-variant`）
- [Origin Pool 增强](./originpool-enhancements/) - 源池模式富化
...
| 版本 | 日期 | 变更 |
|---------|------|---------|
| 2.0.46 | 2026-04-18 | 添加了服务器应用的默认值、引导式工作流集成，并扩展了最小配置部分 |
