...
origin_pool 相关模式的元数据增强。有关扩展定义，请参阅 [OpenAPI 扩展](../development/#openapi-extensions)。

## 增强的模式

### 模式匹配

所有匹配 `origin_pool.*SpecType` 的模式都会接收增强：
...
标记为 `x-f5xc-server-default: true` 的字段，在请求中省略时，其 `default` 值由 F5 XC API 服务器应用。
...
当未指定 `advanced_options` 时，服务器的行为与设置了以下值相同：
...
| `disable_outlier_detection` | `{}` | object | 禁用异常值检测 |
...
| `disable_lb_source_ip_persistance` | `{}` | object | 禁用 LB 源 IP 持久性 |
...
| 字段 | UI 预选值 | 服务器应用值 | 说明 |
|-------|-----------------|----------------|------|
| `loadbalancer_algorithm` | `LB_OVERRIDE` | `ROUND_ROBIN` | 省略字段时，UI 创建的资源与 API 创建的资源不同 |
...
| 组 | 字段 | 服务器默认值 |
|-------|--------|----------------|
| 端口配置 | `port`, `automatic_port`, `lb_port` | `port`（显式指定） |
...
| 异常值检测 | `disable_outlier_detection`, `outlier_detection` | `disable_outlier_detection` |
...
| LB 源 IP | `disable_lb_source_ip_persistance`, `enable_lb_source_ip_persistance` | `disable_lb_source_ip_persistance` |
...
F5 XC Web 控制台为源服务器池提供 15 个配置选项：

| # | UI 标签 | API 字段路径 | 类型 | 服务器默认值 |
|---|----------|----------------|------|----------------|
| 1 | 源服务器端口 | `spec.[port\|automatic_port\|lb_port]` | OneOf | `port`（显式指定） |
...
| 10 | 异常值检测 | `spec.advanced_options.[disable_outlier_detection\|outlier_detection]` | OneOf | `disable_outlier_detection` |
...
| 15 | LB 源 IP 持久性 | `spec.advanced_options.[disable_lb_source_ip_persistance\|enable_lb_source_ip_persistance]` | OneOf | `disable_lb_source_ip_persistance` |
...
├── advanced_options    # 嵌套 advanced_options 默认值
...
源服务器可以使用不同的类型变体来指定。类型决定了需要哪些额外的必填字段。
...
2. **网络选择枚举** - 记录 `network_choice` 约束值（site_local_inside、site_local_outside 等）
...
- [健康检查增强](./healthcheck-enhancements/) - 健康检查模式增强

## 变更日志

| 版本 | 日期 | 变更内容 |
|---------|------|---------|
| 2.1.2 | 2026-01-18 | 添加了源服务器类型章节，包含 public_name/public_ip；为依赖站点的类型添加了 TODO 标记 |
