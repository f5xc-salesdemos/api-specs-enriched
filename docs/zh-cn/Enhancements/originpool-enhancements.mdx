...
origin_pool 相关模式的富化元数据。扩展定义请参见 [OpenAPI 扩展](../development/#openapi-extensions)。
...
| 字段 | 适用操作 | 说明 |
|-------|--------------|-------|
| `metadata.name` | 所有操作 | 1-63 个字符，小写字母数字，匹配模式：`^[a-z0-9]([-a-z0-9]*[a-z0-9])?$` |
...
| `disable_lb_source_ip_persistance` | `{}` | object | 禁用负载均衡源 IP 持久性 |
...
F5 XC Web UI 预选的值与省略字段时 API 应用的值有所不同。

| 字段 | UI 预选值 | 服务器应用值 | 说明 |
|-------|-----------------|----------------|------|
| `loadbalancer_algorithm` | `LB_OVERRIDE` | `ROUND_ROBIN` | 省略字段时，通过 UI 创建和通过 API 创建的资源会有差异 |
...
| `metadata.name` | string | 1-63 个字符，匹配模式：`^[a-z0-9]([-a-z0-9]*[a-z0-9])?$` |
...
| 15 | 负载均衡源 IP 持久性 | `spec.advanced_options.[disable_lb_source_ip_persistance\|enable_lb_source_ip_persistance]` | OneOf | `disable_lb_source_ip_persistance` |
...
├── server_applied      # 标记为 x-f5xc-server-default: true 的字段
├── recommended         # 标记为 x-f5xc-recommended-value 的字段
...
源服务器可以使用不同的类型变体进行指定。类型决定了需要哪些额外的必填字段。
...
| 类型 | 描述 | 必填字段 | 状态 |
|------|-------------|-----------------|--------|
| `private_ip` | 使用私有/公共 IP 和站点信息的源服务器 | `ip`、`site_locator`、`network_choice`、`snat_pool` | 🔲 等待站点逻辑 |
| `private_name` | 使用 DNS 名称和站点信息的源服务器 | `dns_name`、`site_locator`、`network_choice`、`snat_pool` | 🔲 等待站点逻辑 |
| `k8s_service` | 带有站点信息的 Kubernetes 服务 | `service_name`、`site_locator`、`network_choice` | 🔲 等待站点逻辑 |
| `consul_service` | 带有站点信息的 HashiCorp Consul 服务 | `service_name`、`site_locator`、`network_choice` | 🔲 等待站点逻辑 |
...
| 2.1.1 | 2026-01-18 | 重写为纯 API 参考；移除了下游代码示例 |
