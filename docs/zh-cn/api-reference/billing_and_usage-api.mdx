...
计划变更、发票开具和资源消耗。命名空间级别的配额限制与用量跟踪。
...
- **subscription**：用于服务权益的订阅
- **quota**：用于使用限制的资源配额
- **usage_report**：用于消耗跟踪的使用报告
...
| POST | `/api/web/namespaces/&#123;namespace&#125;/billing/payment_methods` | 创建支付方式规格。 |
...
| GET | `/api/web/namespaces/system/usage_plans/current` | 获取当前使用计划。 |
| GET | `/api/web/namespaces/system/usage_plans/custom_list` | 列出使用计划。 |
