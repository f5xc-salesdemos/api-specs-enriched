...
跨区域的流量分发与路由规则。健康检查与故障转移策略。

- **类别**：网络
- **复杂度**：高级
- **路径数**：133 | **Schema 数**：903
- **层级**：高级
- **相关领域**：`dns`、`service_policy`、`network`
...
- **http_loadbalancer**：第 7 层 HTTP/HTTPS 负载均衡器，用于应用流量分发
- **tcp_loadbalancer**：第 4 层 TCP/UDP 负载均衡器，用于非 HTTP 协议流量
- **origin_pool**：后端服务器组，用于负载均衡器流量分发
- **healthcheck**：源站服务器可用性的健康监控配置
