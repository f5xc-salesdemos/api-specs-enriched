...
**目的**：用于确定性 AI 生成、CLI 验证和 Terraform 模式强制执行的全面约束元数据。

**扩展名**：`x-f5xc-constraints`
**版本**：3.3.0
**状态**：生产环境
**使用者**：AI 助手、CLI 工具、Terraform 提供程序、IDE 扩展、API 验证器
...
- **50+ 种模式类型**（字符串、数组、数值）
- 高优先级字段（名称、端口、标识符）的 **置信度达 95%**
- **3 层协调**：EXISTING > DISCOVERY > INFERRED
...
| *(约束键)* | 各种 | 特定类型的约束键扁平化在顶级（如 `minLength`、`maxItems`、`minimum`） |
| `metadata` | object | 来源跟踪、置信度、验证时间戳 |
...
当满足以下条件时，将设置 `deterministic` 布尔标志：
- 置信度分数 >= 0.9
- 模式已通过发现数据验证，或者
- 模式符合公认标准（RFC、ISO 等）
...
- **问题**：请在 https://github.com/f5xc-salesdemos/api-specs-enriched/issues 报告约束准确性问题
