...
| `docs/specifications/api/` | 生成的域规范 | 已忽略 |
...
发现功能探索实时 F5 XC API 以查找：

- **未文档化的约束**：OpenAPI 中未标记的必需字段
- **实际枚举值**：生产环境中看到的真实值
- **默认行为**：省略字段时服务器应用的值
- **响应模式**：实际的数据结构
...
| 扩展 | 含义 | 影响 |
|-----------|---------|-------------|
| `x-f5xc-server-default` | F5 XC API 服务器在省略该字段时应用此值 | 字段为可选；省略它将产生文档中记录的默认行为 |
| `x-f5xc-recommended-value` | F5 XC Web 控制台为新资源预填充此值 | 字段没有服务器默认值，但此值代表典型配置 |
| `x-f5xc-recommended-oneof-variant` | F5 XC 控制台预选此 OneOf 变体 | 标识当存在多个互斥选项时的典型选择 |
...
| 扩展 | 来源 | 用途 |
|-----------|--------|---------|
| `x-ves-required: "true"` | F5 XC 原始规范 | 字段要求非零/非空值 |
| `x-f5xc-required-for` | 增强管道 | 上下文相关的必需状态 |

**x-f5xc-required-for 上下文**：
- `create`：创建资源时必需
- `update`：更新资源时必需
- `minimum_config`：最小可行配置所必需
...
指定 F5 XC Web 控制台用作预填充默认值的值。此值不由服务器强制执行，但代表通过控制台创建新资源时的典型初始配置。
...
对于具有互斥字段组的 schema，标识哪个变体是默认或最常用的选择。键是 OneOf 组名，值是推荐的变体字段名。
...
增强器检查 `x-ves-validation-rules` 以推断必需状态：

| 规则 | 影响 |
|------|-------------|
| `ves.io.schema.rules.message.required: "true"` | 字段为必需 |
| `ves.io.schema.rules.uint32.gte: N` | 如果 N >= 1 且没有服务器默认值，则字段为必需 |
...
在增强/规范化配置中修复问题，而非在输出文件中修改。

### 版本系统

版本从 git 标签派生（例如 `v2.0.38`），消除了并发 PR 上合并冲突导致的竞争条件。
