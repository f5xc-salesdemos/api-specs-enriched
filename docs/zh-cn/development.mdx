...
| 目录 | 用途 | Git 状态 |
|-----------|---------|------------|
| `specs/original/` | F5 源规范 | 已 Gitignore |
| `specs/discovered/` | API 发现输出 | 已跟踪 (openapi.json, session.json) |
| `docs/specifications/api/` | 生成的领域规范 | 已 Gitignore |
...
1. 每日定时（UTC 6:00 AM）或推送到 main 分支触发工作流
2. ETag 检查确定 F5 规范是否发生变化
3. 管道处理和增强规范
4. 版本从 git 标签计算：`git describe --tags --abbrev=0`
5. 直接提交 + 创建标签（无版本升级 PR）
6. 创建带变更日志的 GitHub Release
...
| 扩展 | 含义 | 影响 |
|-----------|---------|-------------|
| `x-f5xc-server-default` | F5 XC API 服务器在省略字段时应用此值 | 字段为可选；省略它将产生文档中描述的默认行为 |
...
对于具有互斥字段组的模式，标识哪个变体是默认或最常见的选择。键是 OneOf 组名，值是推荐的变体字段名。
...
声明 OneOf 组成员之间的互斥关系。从 `x-ves-oneof-field-*` 扩展自动推导。这使得下游工具（Terraform、CLI、MCP）能够在模式级别而非运行时验证冲突。
...
**来源**：在管道增强过程中从 F5 原生 `x-ves-oneof-field-*` 扩展自动推导。
...
| `ves.io.schema.rules.uint32.gte: N` | 如果 N >= 1 且无服务器默认值，字段为必填 |
...
版本从 git 标签推导（例如 `v2.0.38`），消除了并发 PR 上因合并冲突导致的竞态条件。
...
**注意**：版本从 git 标签推导（例如 `v2.0.38`），而非从 `.version` 文件获取。
