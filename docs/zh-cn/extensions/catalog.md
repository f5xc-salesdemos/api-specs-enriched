...
- **此处注入** — 我们的增强器添加的扩展（`x-f5xc-*` 以及
  `x-ves-cli-*` / `x-ves-field-*` / `x-ves-operation-*` / 发现变体）。这些是下游工具应使用的扩展。
- **上游透传** — F5 在源规范中发出的扩展，我们原样保留（`x-ves-proto-*`、`x-displayname` 等）。为透明性而记录，但不受本仓库控制。
- **未来注入** — 尚未发出；在增强器开始生成时立即记录于此（初始填充时不适用）。

## 条目模式

以下每个条目都严格遵循此格式。`tests/test_extension_catalog.py` 中的一致性测试允许章节正文较为简略，只要 `### x-name` 标题存在且 `Pass-through from upstream:` 标志以 `yes` 或 `no` 值出现即可。
...
- **Applied at:** info
- **Purpose:** 标识增强规范的 CLI 域名标识（例如 `http_loadbalancer`）。
- **Consumers:** CLI
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** 发现过程中探测 API 时观察到的响应时间（毫秒）。
- **Consumers:** multiple
- **Value type:** number
...
- **Applied at:** schema
- **Purpose:** 成功执行 POST/PUT 此资源所需的最小可行字段集。
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
...
- **Applied at:** schema
- **Purpose:** 资源模式的人类可读显示名称（覆盖自动生成的名称）。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** 条件性要求（例如，当兄弟字段等于 X 时为必填）。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** 当服务器默认值不够理想时，推荐的生产环境字段值。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{}`
- **Injected by:** scripts/utils/default_value_enricher.py
...
- **Applied at:** schema property
- **Purpose:** 记录跨字段依赖关系，即某个字段需要另一个字段被设置。
...
- **Purpose:** 指定操作请求体中必须提供的字段名称才能成功执行。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
...
- **Applied at:** operation
- **Purpose:** 实时发现过程中观察到的错误响应目录，包含示例负载。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** info
- **Purpose:** 简短（约 60 字符）的域描述。也适用于长描述的属性级别。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
