...
验证规范将 API 验证约束、默认值和配置元数据整合到一个 JSON 文件中。该规范从增强的 OpenAPI 规范生成，并与其一起发布。
...
| 类别 | 描述 | 来源 |
|----------|-------------|--------|
| `server_applied` | 省略字段时 API 应用的值 | 实时 API 测试 |
...
| 扩展 | 用途 | 位置 |
|-----------|---------|----------|
| `x-f5xc-required-for` | 上下文特定的必需字段 | Schema 属性 |
...
| 变更类型 | 版本升级 | 示例 |
|-------------|--------------|---------|
| 破坏性结构变更 | 主版本号 | 字段重命名、移除的部分 |
| 新增验证规则或资源 | 次版本号 | 新增资源默认值 |
| 错误修复或描述更新 | 补丁版本号 | 拼写错误修正 |

## 协调优先级

当多个来源提供约束时，协调优先级如下：

1. **现有** - 原始 OpenAPI 规范中的约束（最高优先级）
2. **发现** - 来自实时 API 发现的约束
3. **推断** - 来自模式匹配的约束（最低优先级）
...
| 版本 | 日期 | 变更内容 |
|---------|------|---------|
| 2.1.2 | 2026-01-18 | 重写为纯 API 参考文档；新增 oneof_recommended 和 nested_recommended 类别 |
| 2.1.0 | 2026-01-18 | 统一默认值结构，替代分散的各部分 |
