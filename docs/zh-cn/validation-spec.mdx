...
验证规范将 API 验证约束、默认值和配置元数据整合到一个 JSON 文件中。该规范从增强的 OpenAPI 规范生成，并与之一起发布。
...
| `nested_recommended` | 嵌套模式的推荐值 | UI 分析 |
...
| 变更类型 | 版本升级 | 示例 |
|-------------|--------------|---------|
| 破坏性结构变更 | 主版本号 | 字段重命名、删除章节 |
| 新增验证规则或资源 | 次版本号 | 新增资源默认值 |
| 错误修复或描述更新 | 修订版本号 | 拼写纠正 |
...
- [健康检查增强](./Enhancements/healthcheck-enhancements/) - 健康检查模式增强
- [源站池增强](./Enhancements/originpool-enhancements/) - 源站池模式增强
...
| 版本 | 日期 | 变更内容 |
|---------|------|---------|
| 2.1.2 | 2026-01-18 | 重写为纯 API 参考；新增 oneof_recommended 和 nested_recommended 类别 |
| 2.1.0 | 2026-01-18 | 统一默认值结构，替代分散的章节 |
