...
健康檢查相關結構描述的擴充中繼資料。請參閱 [OpenAPI 擴充](../development/#openapi-extensions) 以取得擴充定義。

## 擴充結構描述

### 模式

所有符合 `healthcheck.*SpecType` 的結構描述都會接收擴充：
...
| `jitter_percent` | `0` | integer | 用於時間隨機化的百分比抖動 |
...
| `headers` | `{}` | object | 自訂標頭 |
| `request_headers_to_remove` | `[]` | array | 要從請求中移除的標頭 |
...
| `interval` | `15` | integer | 健康檢查之間的間隔（秒） |
...
包含互斥欄位群組（OneOf）的結構描述包含 `x-f5xc-recommended-oneof-variant`，以指示最常使用的變體。此中繼資料根據 F5 XC 主控台預設值和常見使用模式，識別存在多個選項時的典型選擇。
...
這些供應商擴充被新增到標準 OpenAPI 結構描述中，以傳達 F5 XC 特定的預設行為。

### x-f5xc-server-default

**類型**：`boolean`

當值為 `true` 時，表示附帶的 `default` 值由 F5 XC API 伺服器強制執行。具有此擴充的欄位可以安全地從 API 請求中省略——伺服器會自動套用預設值。
...
指定 F5 XC 網頁主控台用作預先填入預設值的值。此值並非由伺服器強制執行，而是代表透過主控台建立新資源時的典型起始設定。
