...
description: Origin Pool 結構描述的補充中繼資料
sidebar:
  order: 3
i18n:
...
origin_pool 相關結構描述的補充中繼資料。有關擴充定義，請參閱 [OpenAPI 擴充](../development/#openapi-extensions)。

## 已補充的結構描述

### 模式

所有匹配 `origin_pool.*SpecType` 的結構描述都會接收補充資料：
...
| 欄位 | 適用操作 | 備註 |
|-------|--------------|-------|
| `metadata.name` | 所有操作 | 1-63 字元，小寫字母數字，模式：`^[a-z0-9]([-a-z0-9]*[a-z0-9])?$` |
...
標記為 `x-f5xc-server-default: true` 的欄位，在請求中省略時會由 F5 XC API 伺服器套用其 `default` 值。
...
| `same_as_endpoint_port` | `{}` | object | 健康檢查使用端點埠 |
| `default_circuit_breaker` | `{}` | object | 預設斷路器設定 |
| `disable_outlier_detection` | `{}` | object | 停用異常偵測 |
...
F5 XC 網頁 UI 預選的值與省略欄位時 API 套用的值不同。

| 欄位 | UI 預選值 | 伺服器套用值 | 備註 |
|-------|-----------------|----------------|------|
| `loadbalancer_algorithm` | `LB_OVERRIDE` | `ROUND_ROBIN` | 省略欄位時，UI 建立和 API 建立的資源會有差異 |
...
| 值 | 說明 | 備註 |
|-------|-------------|-------|
| `ROUND_ROBIN` | 依輪詢順序選擇每個健康端點 | 伺服器預設值 |
| `LEAST_REQUEST` | 選擇活躍請求最少的端點 | |
...
| `LOCAL_ONLY` | 僅使用本地端點 | |
| `LOCAL_PREFERRED` | 優先使用本地端點，不可用時回退至遠端 | |
...
| 群組 | 欄位 | 伺服器預設值 |
|-------|--------|----------------|
| 埠配置 | `port`, `automatic_port`, `lb_port` | `port`（明確指定） |
| 至來源的 TLS | `no_tls`, `use_tls` | `no_tls` |
| 健康檢查埠 | `same_as_endpoint_port`, `health_check_port` | `same_as_endpoint_port` |
| 斷路器 | `default_circuit_breaker`, `disable_circuit_breaker`, `circuit_breaker` | `default_circuit_breaker` |
| 異常偵測 | `disable_outlier_detection`, `outlier_detection` | `disable_outlier_detection` |
...
| `metadata.name` | string | 1-63 字元，模式：`^[a-z0-9]([-a-z0-9]*[a-z0-9])?$` |

## UI 配置選項

F5 XC 網頁控制台為 Origin Pool 提供 15 個配置選項：
...
以下 Origin Server 類型需要在實作前完成「site」資源邏輯開發：
...
2. **Network Choice 列舉** - 記錄 `network_choice` 約束值（site_local_inside、site_local_outside 等）
...
- [Healthcheck 增強功能](./healthcheck-enhancements/) - Healthcheck 結構描述補充

## 變更日誌

| 版本 | 日期 | 變更內容 |
|---------|------|---------|
| 2.1.2 | 2026-01-18 | 新增 Origin Server 類型章節，包含 public_name/public_ip；為依賴站點的類型新增 TODO 標記 |
