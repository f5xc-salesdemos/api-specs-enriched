---
title: 約束條件元資料系統
description: 用於確定性 AI 生成和驗證的完整約束條件元資料
sidebar:
  order: 2
i18n:
...
**目的**：用於確定性 AI 生成、CLI 驗證和 Terraform 架構強制執行的完整約束條件元資料。

**擴展**：`x-f5xc-constraints`
**版本**：3.3.0
**狀態**：正式環境
**使用者**：AI 助手、CLI 工具、Terraform 提供者、IDE 擴展、API 驗證器
...
`x-f5xc-constraints` 擴展提供有關欄位驗證約束條件的確定性知識，實現以下功能：

- **AI 助手**：無需使用者輸入約束條件細節即可生成有效配置
- **CLI 工具**：提供準確的輸入提示、驗證和錯誤訊息
- **Terraform**：在規劃階段而非套用階段強制執行約束條件
- **IDE 擴展**：即時驗證和自動完成建議
- **API 驗證器**：提交前驗證以減少 API 錯誤

### 覆蓋統計

- 跨所有 F5 XC API 規格共 **9,851 個約束條件**
- **50+ 種模式類型**（字串、陣列、數值）
- 高優先欄位（名稱、連接埠、識別碼）達 **95% 信心度**
- **三層協調**：EXISTING > DISCOVERY > INFERRED
...
| `category` | string | 約束條件類別，用於分組和報告 |
| *（約束條件鍵）* | various | 類型特定的約束條件鍵，扁平化於頂層（例如 `minLength`、`maxItems`、`minimum`） |
...
- **文件**：完整 API 文件位於 https://f5xc-salesdemos.github.io/api-specs-enriched/
