---
title: 約束條件元資料系統
description: 用於確定性 AI 生成與驗證的完整約束條件元資料
sidebar:
  order: 2
i18n:
...
**目的**：用於確定性 AI 生成、CLI 驗證及 Terraform schema 強制執行的完整約束條件元資料。

**擴充欄位**：`x-f5xc-constraints`
**版本**：3.3.0
**狀態**：Production
**使用者**：AI 助理、CLI 工具、Terraform 提供者、IDE 擴充套件、API 驗證器
...
1. [概觀](#overview)
2. [擴充結構](#extension-structure)
3. [約束條件類型](#constraint-types)
...
`x-f5xc-constraints` 擴充欄位提供關於欄位驗證約束條件的確定性知識，實現以下功能：

- **AI 助理**：無需使用者輸入約束條件細節即可生成有效配置
- **CLI 工具**：提供準確的輸入提示、驗證及錯誤訊息
- **Terraform**：在 plan 階段而非 apply 階段強制執行約束條件
- **IDE 擴充套件**：即時驗證與自動完成建議
- **API 驗證器**：提交前驗證以減少 API 錯誤

### 涵蓋統計

- 所有 F5 XC API 規格中有 **9,851 個約束條件**
- **50+ 種模式類型**（字串、陣列、數值）
- 高優先欄位（名稱、連接埠、識別碼）達 **95% 信心度**
- **三層調和**：EXISTING > DISCOVERY > INFERRED
...
| 欄位 | 類型 | 描述 |
|-------|------|-------------|
| `constraintType` | string | 資料型別（string、array、number、object） |
| `deterministic` | boolean | 高信心度旗標（信心度 >= 0.9），用於 AI 生成 |
| `category` | string | 約束條件分類，用於分組與報告 |
| *（約束條件鍵）* | 各種 | 型別特定的約束條件鍵，平展於頂層（例如 `minLength`、`maxItems`、`minimum`） |
...
| 格式 | 描述 | 範例 |
|--------|-------------|---------|
| `dns-label` | RFC 1123 DNS 標籤（1-63 字元） | `my-service` |
...
**連接埠號碼**：
- 連接埠號碼必須為 1 到 65535 之間的整數
- 標準連接埠：80（HTTP）、443（HTTPS）、22（SSH）

**VLAN ID**：
- VLAN ID 必須為 1 到 4094 之間的整數（802.1Q 標準）

**逾時時間**：
- 逾時時間必須為 1-3600 秒（最長 1 小時）
...
- ❌ `my-service-`（結尾為連字號）
- ❌ `my_service`（不允許底線）
- ❌ `this-is-a-very-long-name-that-exceeds-the-sixty-three-character-limit`（過長）
...
| 範圍 | 解讀 | 用途 |
|-------|----------------|-------|
| 0.90-1.00 | **高信心度** | 啟用確定性旗標，用於 AI 生成 |
| 0.70-0.89 | **中等信心度** | 建議性質，建議使用者確認 |
| 0.50-0.69 | **低信心度** | 僅供參考，需要使用者驗證 |
| < 0.50 | **極低** | 不納入約束條件 |

### 確定性旗標

`deterministic` 布林旗標在以下情況下設為啟用：
- 信心分數 >= 0.9
- 模式已根據探索資料驗證，或
- 模式符合已確立的標準（RFC、ISO 等）
...
1. **x-ves-validation-rules**：原生 F5 驗證擴充
...
| 模式 | 欄位名稱正規表達式 | 約束條件類型 | 範例 |
|---------|------------------|-----------------|---------|
| DNS 標籤 | `\bname$` | String（1-63 字元，dns-label） | `my-service` |
| 命名空間 | `\bnamespace$` | String（1-63 字元，dns-label） | `production` |
| 連接埠 | `\bport$` | Number（1-65535） | `443` |
| VLAN ID | `\b(vlan)?_?id$` | Number（1-4094） | `100` |
| UUID | `\buuid$` | String（36 字元，uuid 格式） | `550e8400-...` |
| 電子郵件 | `\bemail$` | String（最長 254 字元，email） | `user@example.com` |
| 時間戳記 | `\b(timestamp\|created_at\|updated_at)$` | String（ISO 8601） | `2026-01-19T12:00:00Z` |

### 中等信心度模式（0.80-0.94）

| 模式 | 欄位名稱正規表達式 | 約束條件類型 | 範例 |
|---------|------------------|-----------------|---------|
| 使用者名稱 | `\b(user\|username)$` | String（1-64 字元，英數字元） | `admin` |
| 顯示名稱 | `\b(display_?)?name$` | String（1-256 字元，自由文字） | `My Service` |
| 描述 | `\b(comment\|description\|note)$` | String（最長 4096 字元） | `Service description` |
| 路徑 | `\bpath$` | String（最長 2048 字元） | `/api/v1/resources` |

---

## 支援與意見回饋

- **問題回報**：在 https://github.com/f5xc-salesdemos/api-specs-enriched/issues 回報約束條件準確性問題
- **文件**：完整 API 文件請參閱 https://f5xc-salesdemos.github.io/api-specs-enriched/
- **MCP 整合**：請參閱 `f5xc-api-mcp` 儲存庫以取得 MCP 伺服器整合範例
