---
title: 擴充延伸目錄
description: 豐富化 OpenAPI 規格中每個 x-* 延伸的權威來源
i18n:
...
`docs/specifications/api/*.json` 中出現的每個 `x-*` 延伸的權威來源。與
`scripts/utils/extension_constants.py` 的一致性由
`tests/test_extension_catalog.py` 強制執行。

本文件記錄三類延伸：

- **此處注入** — 我們的豐富化工具添加的延伸（`x-f5xc-*` 和
  `x-ves-cli-*` / `x-ves-field-*` / `x-ves-operation-*` / 探索
  變體）。這些是下游工具應該使用的延伸。
- **上游直通** — F5 在來源規格中發出的延伸，
  我們原封不動地保留（`x-ves-proto-*`、`x-displayname` 等）。
  為透明度而記錄，但不由本儲存庫控制。
- **未來注入** — 尚未發出；在豐富化工具開始產生它們的那一刻
  記錄於此（在初始填充時不適用）。

## 條目結構

以下每個條目都具有完全相同的格式。
`tests/test_extension_catalog.py` 中的一致性測試容許區段本體
較為簡略，只要存在 `### x-name` 標頭以及
`Pass-through from upstream:` 旗標且值為 `yes` 或 `no` 即可。
...
- **Applied at:** info
- **Purpose:** 識別豐富化規格的 CLI 領域代碼（例如 `http_loadbalancer`）。
- **Consumers:** CLI
- **Value type:** string
...
- **Applied at:** info
- **Purpose:** 探索期間探測 API 觀察到的回應時間（毫秒）。
- **Consumers:** multiple
- **Value type:** number
...
- **Applied at:** info
- **Purpose:** 具名的逐步工作流程，用於完成領域中的常見任務。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
...
- **Applied at:** schema
- **Purpose:** 對應至此結構的 Terraform 資源類型名稱。
- **Consumers:** Terraform
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema
- **Purpose:** 資源結構的人類可讀顯示名稱（覆蓋自動產生的名稱）。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** 條件式需求（例如當兄弟欄位等於 X 時為必填）。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** 當伺服器預設值不是最佳選擇時，建議的生產環境值。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{}`
- **Injected by:** scripts/utils/default_value_enricher.py
...
- **Applied at:** schema property
- **Purpose:** 記錄跨欄位相依性，其中一個欄位需要另一個欄位已設定。
...
- **Applied at:** operation
- **Purpose:** 即時探索期間觀察到的錯誤回應目錄，附帶範例負載。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "object"}}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** info
- **Purpose:** 簡短（約 60 字元）的領域描述。也適用於屬性層級的長描述。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
...
- **Applied at:** info
- **Purpose:** 中等長度（約 150 字元）的領域描述。也適用於屬性層級。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/property_description_short_enricher.py
