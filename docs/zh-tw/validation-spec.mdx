...
本文件描述由 api-specs-enriched 專案發布的驗證規格結構與內容。

## 概述

驗證規格將 API 驗證約束、預設值和組態中繼資料整合為單一 JSON 檔案。此規格從擴充的 OpenAPI 規格生成，並與其一同發布。

**發布位置**：`docs/specifications/api/validation.json`
...
| 類別 | 說明 | 來源 |
|----------|-------------|--------|
| `server_applied` | 當欄位被省略時 API 套用的值 | 即時 API 測試 |
| `recommended` | F5 XC 網頁主控台預先填入的值 | UI 分析 |
| `advanced_options` | advanced_options 物件中的巢狀預設值 | API 探索 |
...
擴充的 OpenAPI 規格使用以下擴充嵌入驗證中繼資料：

| 擴充 | 用途 | 位置 |
|-----------|---------|----------|
| `x-f5xc-required-for` | 上下文相關的必填欄位 | Schema 屬性 |
...
| `x-f5xc-minimum-configuration` | 最小組態範例 | Schema 定義 |
...
- [健康檢查增強](./Enhancements/healthcheck-enhancements/) - 健康檢查結構描述擴充
- [來源池增強](./Enhancements/originpool-enhancements/) - 來源池結構描述擴充
...
| 版本 | 日期 | 變更內容 |
|---------|------|---------|
| 2.1.2 | 2026-01-18 | 重寫為純 API 參考；新增 oneof_recommended 和 nested_recommended 類別 |
| 2.1.0 | 2026-01-18 | 統一預設值結構，取代分散的章節 |
