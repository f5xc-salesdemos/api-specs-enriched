...
本文件描述由 api-specs-enriched 專案發佈的驗證規格之結構與內容。

## 概述

驗證規格將 API 驗證約束、預設值和配置中繼資料整合到單一 JSON 檔案中。此規格從增強型 OpenAPI 規格產生，並與其一同發佈。
...
增強型 OpenAPI 規格使用以下擴充嵌入驗證中繼資料：

| 擴充 | 用途 | 位置 |
|-----------|---------|----------|
| `x-f5xc-required-for` | 特定情境所需的欄位 | Schema 屬性 |
...
1. **既有** - 原始 OpenAPI 規格中的約束（最高優先）
2. **探索** - 來自即時 API 探索的約束
3. **推斷** - 來自模式比對的約束（最低優先）

## 相關文件

- [健康檢查增強](./Enhancements/healthcheck-enhancements/) - 健康檢查結構描述增強
- [Origin Pool 增強](./Enhancements/originpool-enhancements/) - Origin pool 結構描述增強
...
| 版本 | 日期 | 變更內容 |
|---------|------|---------|
| 2.1.2 | 2026-01-18 | 重新撰寫為純 API 參考文件；新增 oneof_recommended 和 nested_recommended 類別 |
| 2.1.0 | 2026-01-18 | 統一預設值結構，取代分散的區段 |
